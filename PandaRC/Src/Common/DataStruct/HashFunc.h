...
#include <stdint.h>
 ...
 /* Max elements per slot should be (16~20)
  * linux set 4096 slots default 
  */ 

#define DEFAULT_SLOT_NUM 4096
#define MAX_ELEM_PERSLOT 16
...
hashFunc(const void *buf, register size_t len) {
	register uint32_t hash = FNV_32_INIT;
	unsigned char *bp = (unsigned char*)buf;
	unsigned char *ep = bp + len;
	while (bp < ep) {
		hash *= FNV_32_PRIME;
		hash ^= (uint32_t)*bp++;
	}    
