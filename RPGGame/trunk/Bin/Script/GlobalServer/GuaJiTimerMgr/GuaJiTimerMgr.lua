...
function CGuaJiTimerObj:OnRelease()
...
function CGuaJiTimerObj:RegAutoReward(nRoleID, nGuanQia)
    goTimerMgr:Clear(self.m_nAutoRewardTimer)
    local tGuanQiaConf = ctGuaJiConf:GetGuanQiaConf(nGuanQia)
    self.m_nAutoRewardTimer = goTimerMgr:Interval(3*tGuanQiaConf.nPatrolSec, function() self:AutoReward() end)
end
...
function CGuaJiTimerObj:AutoReward()
    ...
    goRemoteCall:CallWait("GuaJiReward", CallBack, oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetID())
end
...
goGuaJiTimerMgr = goGuaJiTimerMgr or CGuaJiTimerMgr:new()