...
function CHallFame:RegZeroTimer()
	goTimerMgr:Clear(self.m_nZeroTimer)
	local nNextZeroTime = os.NextHourTime1(0)
	self.m_nZeroTimer = goTimerMgr:Interval(nNextZeroTime, function() self:OnZeroTimer() end)
end
...
function CHallFame:LoadData()
	local sData = goDBMgr:GetSSDB(gnServerID, "global", GF.GetServiceID()):HGet(gtDBDef.sHallFameDB, "data")
	if sData ~= "" then
...
function CHallFame:SaveData()
	...
	goDBMgr:GetSSDB(gnServerID, "global", GF.GetServiceID()):HSet(gtDBDef.sHallFameDB, "data", cjson.encode(tData))
...
function CHallFame:CongratReq(oRole, nTitleID)
	...
	oRole:Tips(sTips)
	self:SyncInfo(oRole)
	goRemoteCall:Call("OnCongratulate", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetID(), {})
end
...
function CHallFame:SetCongratTipsReq(oRole, nTitleID, sTips)
	...
	GF.HasBadWord(sTips, fnCallback)
end
...
goHallFame = goHallFame or CHallFame:new()