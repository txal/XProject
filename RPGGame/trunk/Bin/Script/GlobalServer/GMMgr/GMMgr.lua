...
function CGMMgr:Ctor()
	...
	self.m_nWhiteTimer = goTimerMgr:Interval(60, function() self:UpdateWhiteAccount() end)
...
function CGMMgr:OnRelease()
	goTimerMgr:Clear(self.m_nWhiteTimer)
...
--发送到ROUTER的GM
CGMMgr["cgm"] = function(self, nServer, nService, nSession, tArgs)
	local sCmd = table.concat(tArgs, " ")
	local tConf = goServerMgr:GetRouterService()
	goRemoteCall:Call("GMCommandReq", gnServerID, tConf.nID, nSession, sCmd)
end
	...
	for _, tConf in pairs(tList) do
		if sFlag == "local" then
			if tConf.nServer == gnServerID then
				...
				goRemoteCall:Call("GMCommandReq", tConf.nServer, tConf.nID, nSession, sCmd)
			end
		else
			goRemoteCall:Call("GMCommandReq", tConf.nServer, tConf.nID, nSession, sCmd)
		end
...
CGMMgr["rgm"] = function(self, nServer, nService, nSession, tArgs)
	...
	for _, tConf in pairs(tList) do
		goRemoteCall:CallWait("GMCommandReq", function(bRes) 
...
CGMMgr["wgm"] = function(self, nServer, nService, nSession, tArgs)
	...
	goRemoteCall:Call("GMCommandReq", gnWorldServerID, nServiceID1, nSession, sCmd)
end
...
CGMMgr["wgm2"] = function (self,nServer,nService,nSession,tArgs)
	...
	goRemoteCall:Call("GMCommandReq", gnWorldServerID, nServiceID2, nSession, sCmd)
end
...
CGMMgr["agm"] = function(self, nServer, nService, nSession, tArgs)
	...
	goRemoteCall:Call("GMCommandReq", gnServerID, nServiceID, nSession, sCmd)
end
...
CGMMgr["arenarobot"] = function(self, nServer, nService, nSession, tArgs)
	...
	local fnArenaCheckCallback = function (bRet, sReason)
		...
		goRemoteCall:Call("ArenaBattleReq", oRole:GetStayServer(), oRole:GetLogic(), 
			oRole:GetSession(), oRole:GetID(), tEnemyData, goArenaMgr:GetArenaSeason())
	end
	--取玩家当前所在逻辑服
	goRemoteCall:CallWait("ArenaBattleCheckReq", fnArenaCheckCallback, oRole:GetStayServer(), 
...
--关服
CGMMgr['closeserver'] = function(self, nServer, nService, nSession, tArgs)
	local tRouter = goServerMgr:GetRouterService()
	CmdNet.Srv2Srv("CloseServerReq", gnWorldServerID, tRouter.nID, 0, gnServerID)
end
...
CGMMgr['yyhd'] = function (self,nServer,nService,nSession,tArgs)
	...
	elseif nOperaType == 4 then
		local tSCConf = goBackstage:GetConf(gnServerID, gtBackstageType.eSingleRecharge)
		print("---------yyhd-----------",tSCConf)
