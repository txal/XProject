...
function CColligatePowerRanking:LoadData()
	...
	for k = 1, 10 do 
		...
		if nRoleID > 0 then 
			goRemoteCall:Call("RemoveRoleFromRobotPoolReq", gnWorldServerID, 
...
function CColligatePowerRanking:OnRelease()
	CRankingBase.OnRelease(self)
	goTimerMgr:Clear(self.m_nZeroTimer)
end

function CColligatePowerRanking:OnZeroTimer()
	goTimerMgr:Clear(self.m_nZeroTimer)
	self.m_nZeroTimer = goTimerMgr:Interval(os.NextHourTime1(0), function() self:OnZeroTimer() end)
...
function CColligatePowerRanking:BroadcastCongrat()
	...
	CmdNet.PBSrv2All("RankingRedpointRet", tMsg) 
end
...
function CColligatePowerRanking:OnRankChange(nRoleID, nNewRank, nOldRank)
	...
	if nNewRank <= nFilterRank and (nOldRank > nFilterRank or nOldRank <= 0) then 
		...
		if nTarRoleID and nTarRoleID > 0 then 
			goRemoteCall:Call("AddRole2RobotPoolReq", gnWorldServerID, 
