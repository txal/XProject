function CltPBProc.RankingListReq(nCmd, nServer, nService, nSession, tData)
	...
	print("CltPBProc.RankingListReq***", tData.nRankID)
end

function CltPBProc.RankingCongratReq(nCmd, nServer, nService, nSession, tData)
	local oRole = goGPlayerMgr:GetRoleBySS(nServer, nSession)
	if not oRole then return end
	local oRanking = goRankingMgr:GetRanking(tData.nRankID)
...
-----服务器内部
------推送科舉答題信息
function Srv2Srv.PushKejuRank(nSrcServer, nSrcService, nTarSession,nRoleID,nKejuType,tData)
...
function Srv2Srv.KejuRankingCheckJoinDianshiReq(nSrcServer,nSrcService,nTarSession,nRoleID)
...
function Srv2Srv.KejuRankingTest(nSrcServer,nSrcService,nTarSession,nOperaType,nRoleID,tData)
	local oRole = goGPlayerMgr:GetRoleByID(nRoleID)
	if not oRole then return end
...
--最高宠物评分变化
function Srv2Srv.PetScoreChangeReq(nSrcServer, nSrcService, nTarSession, nRoleID, nPetPos, sPetName, nValue)
...
--家园资产评分变化
function Srv2Srv.HouseAssetsChangeReq(nSrcServer, nSrcService, nTarSession, nRoleID, nValue)
	local oRole = goGPlayerMgr:GetRoleByID(nRoleID)
	if not oRole or oRole:IsRobot() then 
		return 
	end
...
--竞技场积分变化
function Srv2Srv.ArenaScoreChangeReq(nSrcServer, nSrcService, nTarSession, nRoleID, nValue)
	local oRole = goGPlayerMgr:GetRoleByID(nRoleID)
	if not oRole or oRole:IsRobot() then 
		return 
	end
...
--人气变化
function Srv2Srv.PopularityChangeReq(nSrcServer, nSrcService, nTarSession, nRoleID, nValue)
	local oRole = goGPlayerMgr:GetRoleByID(nRoleID)
	if not oRole or oRole:IsRobot() then 
		return 
	end
...
--总好友度变化
function Srv2Srv.FriendDegreeChangeReq(nSrcServer, nSrcService, nTarSession, nRoleID, nValue)
	local oRole = goGPlayerMgr:GetRoleByID(nRoleID)
	if not oRole or oRole:IsRobot() then 
		return 
	end
