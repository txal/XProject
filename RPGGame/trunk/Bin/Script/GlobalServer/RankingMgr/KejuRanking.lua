...
function CKejuRanking:Ctor(nID)
	...
	self.m_nRewardTick = goTimerMgr:Interval(nTimeStamp, function() self:RankReward() end)
	self.m_tTop100 = {}

	self.m_oTempleRanking = CSkipList:new(_fnDescSort2)
	local nTempleTimeStamp = os.WeekDayTime(os.time(),7,0)-os.time()						--周日0点结束，结算殿试
	self.m_nTempleRewardTick = goTimerMgr:Interval(nTempleTimeStamp,function () self:TempleRankReward() end)
end
...
function CKejuRanking:SaveData()
	for nRoleID, v in pairs(self.m_tDirtyMap) do
		...
		if tData then
			...
			goDBMgr:GetSSDB(gnServerID, "global",GF.GetServiceID()):HSet(gtDBDef.sKejuRankingDB, nRoleID, cjson.encode(tRealData))
		end
	end
	self.m_tDirtyMap = {}
end

function CKejuRanking:OnRelease()
	goTimerMgr:Clear(self.m_nRewardTick)
	goTimerMgr:Clear(self.m_nTempleRewardTick)
	self:SaveData()
end
...
function CKejuRanking:RankReward()
	...
	for nLogic,tJoinPlayer in pairs(tLogic) do
		goRemoteCall:Call("JoinKejuDianshi",gnServerID,nLogic,0,tJoinPlayer)
	end
end
...
function CKejuRanking:ResetRanking()
	goDBMgr:GetSSDB(gnServerID, "global", GF.GetServiceID()):HClear(gtDBDef.sKejuRankingDB)
