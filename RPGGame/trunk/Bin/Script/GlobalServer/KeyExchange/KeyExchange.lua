...
function CKeyExchange:LoadData()
	local sData = goDBMgr:GetSSDB(gnServerID, "global", GF.GetServiceID()):HGet(gtDBDef.sKeyExchangeDB, "data")
	if sData ~= "" then
...
function CKeyExchange:SaveData()
	...
	goDBMgr:GetSSDB(gnServerID, "global", GF.GetServiceID()):HSet(gtDBDef.sKeyExchangeDB, "data", cjson.encode(tData))
...
function CKeyExchange:OnRelease()
	goTimerMgr:Clear(self.m_nAutoSaveTimer)
	self.m_nAutoSaveTimer = nil
	self:SaveData()
end
...
goKeyExchange = goKeyExchange or CKeyExchange:new()