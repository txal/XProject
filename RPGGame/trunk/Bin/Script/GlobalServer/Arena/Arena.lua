...
function CArena:Init()
	self:LoadData()
	self.m_nTimer = goTimerMgr:Interval(1, function () self:Tick() end)
end

function CArena:OnRelease()
...
function CArena:AddArenaCoin(oRole, nNum, sReason, fnCallBack)
	...
	if fnCallBack then
		goRemoteCall:CallWait("AddArenaCoinReq", fnCallBack, nServer, nService, nSession, nRoleID, nNum, sReason)
	else
		goRemoteCall:Call("AddArenaCoinReq", nServer, nService, nSession, nRoleID, nNum, sReason)
	end
end
...
function CArena:TriggerBroadcastByDailyWin(oRoleArena)
	...
	if nWinKeep == gtArenaSysConf.nDailyWinKeepAnnounce then
		...
		GF.SendSystemTalk("系统", sAnnounceContent)
	end
end
...
function CArena:OnChallengeEnd(oRoleArena, bWin, nEnemyScore)
	...
	goRemoteCall:Call("ArenaAddBattleRewardReq", oRole:GetStayServer(), oRole:GetLogic(), 
...
function CArena:IsRobot(nID)
	if nID <= gnArenaConfRobotIDMax then
		return true
	end
	return false
end
...
function CArena:OnRoleBattleEnd(oRole, nEnemyID, nArenaSeason, bWin)
	...
	if bWin then --如果胜利，刷新匹配列表
		...
		goRemoteCall:Call("OnArenaWin", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetID(), tData)
	end
	--[[
	//竞技场战斗结果返回
...
function CArena:GetRoleArenaDataReq(oRole)
	...
	goRemoteCall:CallWait("ArenaSysOpenCheckReq", fnCheckSysOpenCallback, oRole:GetStayServer(), 
...
function CArena:UpdateAppellation(oRole)
	...
	local nSession = oRole:GetSession()
	local nRoleID = oRole:GetID()
	goRemoteCall:Call("UpdateArenaAppellation", nServer, nService, nSession, nRoleID, nAppeConfID, tAppeParam, nSubKey)
end
...
goArenaMgr = goArenaMgr or CArena:new()
