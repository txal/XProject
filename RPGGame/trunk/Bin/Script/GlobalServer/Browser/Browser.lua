...
function CBrowser:BrowserReq(nSession, tData)
	...
	if oFunc then
		xpcall(oFunc, function(sErr)
			...
			CmdNet.Srv2Bsr("BrowserRet", gnServerID, nSession, cjson_raw.encode({data=false, error=sErr}))
		end, self, nSession, tData)
	else
		local sErr = "方法不存在: "..sMethod
		CmdNet.Srv2Bsr("BrowserRet", gnServerID, nSession, cjson_raw.encode({error=sErr}))
		LuaTrace(sErr)
	end
end
...
function CBrowser:SendMsg(nSession, tData)
	...
	CmdNet.Srv2Bsr("BrowserRet", gnServerID, nSession, cjson_raw.encode(tData))
end
...
CBrowser["banuser"] = function (self, nSession, tData)
	...
	goRemoteCall:CallWait("UpdateAccountValueReq", function(bRes)
...
CBrowser["openact"] = function (self, nSession, tData)
	...
	if tActConf.bCrossServer then
		goRemoteCall:CallWait("GMOpenAct", function(bRes)
			if bRes then
...
CBrowser["backstageconflist"] = function (self, nSession, tData)
	-- self:SendMsg(nSession, {data=gtBackstageDef})
	self:SendMsg(nSession, {data=false, error="已废弃"})
end

--取游戏内配置表
CBrowser["getbackstageconf"] = function (self, nSession, tData)
	local nID = tonumber(tData.id) or 0
	local tAct = gtBackstageDef[nID]	
	if not tAct then	
		return self:SendMsg(nSession, {data=false, error="活动不存在:"..nID})
	end
	local tConf = goBackstage:GetConf(gnServerID, nID)
	if not tConf then 
		return self:SendMsg(nSession, {data=false, error="活动配置不存在:"..nID})
	end
	self:SendMsg(nSession, {data=tConf})
end
...
CBrowser["setbackstageconf"] = function (self, nSession, tData)
	...
	goBackstage:SetConf(gnServerID, nID, tData.conf)
	self:SendMsg(nSession, {data=true})
end
...
CBrowser["delbackstageconf"] = function (self, nSession, tData)
	...
	goBackstage:SetConf(gnServerID, nID, nil)
	self:SendMsg(nSession, {data=true})
end
...
CBrowser["getpetinfo"] = function (self, nSession, tData)
	...
	goRemoteCall:CallWait("GMGetPetInfoReq", function (tData)
		fnSendMsg(tData)
...
CBrowser["deletepet"] = function (self, nSession, tData)
	...
	goRemoteCall:CallWait("GMDeletePetReq",function (bFlag)
