	...
	if self:IsOpen() then
		if oRole then
			if not oRole:IsSysOpen(nSysID) then
				goRemoteCall:Call("OpenSystemReq", oRole:GetStayServer(), oRole:GetLogic(), 0, oRole:GetID(), nSysID)
			end
		else
			...
			for nSession, oTmpRole in pairs(tSessionMap) do
				if not oTmpRole:IsSysOpen(nSysID) then
					goRemoteCall:Call("OpenSystemReq", oTmpRole:GetStayServer(), oTmpRole:GetLogic(), 0, oTmpRole:GetID(), nSysID)
				end
			end
		end
	else
		if oRole then
			if oRole:IsSysOpen(nSysID) then
				goRemoteCall:Call("CloseSystemReq", oRole:GetStayServer(), oRole:GetLogic(), 0, oRole:GetID(), nSysID)
			end
		else
			local tSessionMap = goGPlayerMgr:GetRoleSSMap()
			for nSession, oTmpRole in pairs(tSessionMap) do
				if oTmpRole:IsSysOpen(nSysID) then
					goRemoteCall:Call("CloseSystemReq", oTmpRole:GetStayServer(), oTmpRole:GetLogic(), 0, oTmpRole:GetID(), nSysID)
				end
			end
		end
	end
...
function CFB:SyncState(oRole)
	...
	--全服广播
		...
		CmdNet.PBSrv2All("ActFBStateRet", tMsg)
	end
end
