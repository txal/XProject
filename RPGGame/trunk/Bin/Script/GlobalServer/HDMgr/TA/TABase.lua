...
function CTABase:UpdateVal(nRoleID, nVal)
	...
	self.m_tRoleMap[nRoleID] = math.min(gnMaxInteger, self.m_tRoleMap[nRoleID]+nVal)
	self:MarkDirty(true)
...
function CTABase:ProgressReq(oRole)
	...
	CmdNet.PBSrv2Clt(oRole:GetSession(), "TimeAwardProgressRet", tMsg)
end
			...
			for k = 1, nMaxAwardLv do
				...
				if nTarValue > 0 then
					...
					if nAwardState == 0 and nValue >= nTarValue then
						...
						GF.SendMail(oRole:GetServer(), "限时奖励活动奖励", sCont, tList, nRoleID)
					end
				end
			end
		end
