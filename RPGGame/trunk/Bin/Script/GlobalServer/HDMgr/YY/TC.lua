...
function CTC:UpdateRechargeAmount()
	...
	goRemoteCall:Call("OnTCRobotRecharge", gnWorldServerID, nService, 0, nAddNum, gnServerID)
end
...
function CTC:GetRoundConf(nRounds)
	...
	if not next(tRoundConf) then
		assert(false, string.format("%s活动轮次错误:%d", self:GetName(), nRounds))
	end
	return tRoundConf
end
			...
			if nCnt >= nNeedCnt and nChargeMoney>=nMoney then
				...
				if oRole then
					...
					if nState == 0 then
						...
						local sCont = string.format("您在%s活动中达到充值%d人民币，获得了以下奖励，请查收。", self:GetName(), tConf.nMoney) 
						GF.SendMail(oRole:GetServer(), string.format("%s活动奖励", self:GetName()), sCont, tList, nRoleID)
					end
				end 
			end
		end
