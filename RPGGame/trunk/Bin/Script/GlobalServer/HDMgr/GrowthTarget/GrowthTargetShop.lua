...
function CGrowthTargetShop:LoadData()
    print("加载开服目标活动商店数据")
	local sData = goDBMgr:GetSSDB(gnServerID, "global", GF.GetServiceID()):HGet(gtDBDef.sGrowthTargetActDB, "GrowthTargetShop") 
    if sData ~= "" then 
...
function CGrowthTargetShop:SaveData()
	...
	goDBMgr:GetSSDB(gnServerID, "global", GF.GetServiceID()):HSet(gtDBDef.sGrowthTargetActDB, "GrowthTargetShop", cjson.encode(tData))
