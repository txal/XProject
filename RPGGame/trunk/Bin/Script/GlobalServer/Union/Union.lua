...
function CUnion:SetDeclarationReq(oRole, sDesc)
	...
	GF.HasBadWord(sDesc, fnCallback)
end
...
function CUnion:SetPurposeReq(oRole, sCont)
	...
	GF.HasBadWord(sCont, fnCallback)
end
...
function CUnion:JoinUnion(oManager, nTarRoleID)
	...
	if oRole then
		...
		goRemoteCall:Call("OnJoinUnion", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetID(), {})
	end
...
function CUnion:AddExp(nVal, sReason, oRole)
	if nVal == 0 then return end
	self.m_nExp = math.max(0, math.min(gnMaxInteger, self.m_nExp + nVal))
	self:MarkDirty(true)
	...
	if nVal > 0 then --总经验
		...
		self.m_nTotalExp = math.max(0, math.min(gnMaxInteger, self.m_nTotalExp+nVal))
...
function CUnion:SignReq(oRole)
	...
	self:SyncDetailInfo(oRole)
	goRemoteCall:Call("OnUnionSignIn", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetID(), {})
	self:SyncRedPointData(oRole)
end
	...
	for nRoleID, v in pairs(self.m_tMemberMap) do
		...
		if oRole:IsOnline() then
			if #tRewardPoolList > 0 then 
				...
				GF.SendMail(gnServerID, sMailTitle, sMailContent, {}, nRoleID)
			end
		end
	end
end
...
function CUnion:CreateUnionScene()
 	...
 	goRemoteCall:CallWait("CreateDup", fnCallback, gnServerID, tDupConf.nLogic, 0, nDupID,tParam)
end
...
function CUnion:UnionEnterSceneReq(oRole)
	...
	local nRoleID = oRole:GetID()
 	...
 	goRemoteCall:Call("RoleEnterDup",oRole:GetServer(),oRole:GetLogic(),0,nRoleID,nDupMixID,{nPosX=0,nPosY=0})
end
...
function CUnion:RemoveDup()
 	...
 	goRemoteCall:Call("RemoveUnionDup", gnServerID,nServiceID,0, nDupMixID)
end