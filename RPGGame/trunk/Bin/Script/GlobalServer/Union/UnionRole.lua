...
function CUnionRole:OnExitUnion(nExitType)
	...
	elseif nExitType == CUnion.tExit.eKick then
		GF.SendMail(oRole:GetServer(), "帮派信息", string.format("您已被%s派请离了", oUnion:GetName()), {}, self.m_nRoleID)
	end
	...
	goRemoteCall:Call("RoleUpdateReq", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetServer(), self.m_nRoleID, tData)
	oRole:UpdateReq(tData)
	local nServiceID = goServerMgr:GetGlobalService(gnWorldServerID,110)
	goRemoteCall:Call("GRoleUpdateReq", gnWorldServerID, nServiceID, oRole:GetSession(),self.m_nRoleID,tData)

	--返回京城
	if oRole:GetDupMixID() == oUnion:GetDupMixID() then
		local nDupMixID = 1
		goRemoteCall:Call("RoleEnterDup", oRole:GetServer(),oRole:GetLogic(),0,oRole:GetID(),nDupMixID,{nPosX=0,nPosY=0})
	end
...
function CUnionRole:OnEnterUnion(oUnion)
	...
	local nServiceID = goServerMgr:GetGlobalService(gnWorldServerID,110)
	goRemoteCall:Call("GRoleUpdateReq", gnWorldServerID, nServiceID, oRole:GetSession(),self.m_nRoleID,{m_nUnionID = self.m_nUnionID})
...
function CUnionRole:AddUnionContri(nCount, sReason)
	...
	self:CheckReset()
	local oRole = goGPlayerMgr:GetRoleByID(self.m_nRoleID)
    self.m_nUnionContri = math.max(0, math.min(gnMaxInteger, self.m_nUnionContri+nCount))
    if nCount > 0 then
		self.m_nDayContri = self.m_nDayContri + nCount
    	self.m_nTotalContri = math.max(0, math.min(gnMaxInteger, self.m_nTotalContri+nCount))
    end
    self:MarkDirty(true)
    if nCount > 0 then
