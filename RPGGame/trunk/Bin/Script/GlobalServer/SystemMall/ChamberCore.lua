		...
		if tConf.nBuyCount ~= -1 then
			...
			local fnFlushCostCallBack = function (bRet)
				...
				if type(sFuncBack) == "string" then
					...
					goRemoteCall:Call(sFuncBack, oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetID(), nPropID)
				end
				self:MarkDirty(true)
			end
			...
			local fnFlushCostCallBack = function (bRet)
				...
				if nSellYinBi > 0 then
					...
					if nYuanBaoNum > 0 then 
						...
						oRole:AddItem(tItemList, "商会卖出获得")
						if nYuanBaoNum > 0 then 
							goRemoteCall:Call("KnapsackAddSaleYuanbaoRecordReq", oRole:GetStayServer(), 
				...
				goRemoteCall:Call("OnChamberCoreItemOnSale", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetID(), {})
				end
				print("商会出售消息返回", tMsg)
			end
			oRole:SubPropByGrid(tItemList1, "商会出售消耗",fnFlushCostCallBack, tConf.nBagType) 
		end
		goRemoteCall:CallWait("KnapsackGetSaleYuanbaoRemainNumReq", fnGetSaleYuanbaoCallback, oRole:GetStayServer(), 
...
--宠物技能书快速购买(学习)  
 function CChamberCore:FastBuyReq(oRole,nPropID, nPos)
	...
	goRemoteCall:Call("PetFastLearnSkillReq", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetID(), nPropID, nPos, nPrice)
 end
