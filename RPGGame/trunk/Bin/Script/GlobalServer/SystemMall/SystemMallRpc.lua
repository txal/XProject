...
--商品列表请求
function CltPBProc.SystemMallItemListReq(nCmd, Server, Srevice, nSession, tData)
...
--购买请求
function CltPBProc.SystemMallBuyReq( nCmd, Server, Srevice, nSession, tData )
	local oRole = goGPlayerMgr:GetRoleBySS(Server, nSession)
	if not oRole then return end
	local oSubShop = goMallMar:GetSubShop(tData.nShopType)
...
--商品出售
function CltPBProc.SystemMallSellReq( nCmd, Server, Srevice, nSession, tData)
	print("商品出售了,快点来买")
	local oRole = goGPlayerMgr:GetRoleBySS(Server, nSession)
	if not oRole then return end
	local oSubShop = goMallMar:GetSubShop(tData.nShopType)
...
--更新
function CltPBProc.SystemMallUpdateReq( nCmd, Server, Srevice, nSession, tData )
	local oRole = goGPlayerMgr:GetRoleBySS(Server, nSession)
	if not oRole then return end
	local oSubShop = goMallMar:GetSubShop(tData.nShopType)
...
--购买金币银币请求
function CltPBProc.SystemMallGoidBuyReq( nCmd, Server, Srevice, nSession, tData )
	local oRole = goGPlayerMgr:GetRoleBySS(Server, nSession)
	if not oRole then return end
...
--快速购买
function CltPBProc.SystemMallFastBuyListReq( nCmd, Server, Srevice, nSession, tData )
	local oRole = goGPlayerMgr:GetRoleBySS(Server, nSession)
	if not oRole then return end
	local oSubShop = goMallMar:GetSubShop(tData.nShopType)
...
function CltPBProc.SystemMallFastBuyReq( nCmd, Server, Srevice, nSession, tData )
	local oRole = goGPlayerMgr:GetRoleBySS(Server, nSession)
	if not oRole then return end
	local oSubShop = goMallMar:GetSubShop(tData.nShopType)
	print("tData----", tData)
...
function CltPBProc.SystemUnionContriAmountReq(nCmd,Server,Srevice,nSession,tData)
	local oRole = goGPlayerMgr:GetRoleBySS(Server,nSession)
	if not oRole then return end
	local oSubShop = goMallMar:GetSubShop(gtShopType.eCBuy)
...
function CltPBProc.SystemGetShopPriceReq(nCmd,Server,Srevice,nSession,tData)
	local oRole = goGPlayerMgr:GetRoleBySS(Server,nSession)
	if not oRole then return end
...
function CltPBProc.SystemGetPropPriceReq(nCmd,Server,Srevice,nSession,tData)
	local oRole = goGPlayerMgr:GetRoleBySS(Server,nSession)
	if not oRole then return end
	local oSubShop = goMallMar:GetSubShop(gtShopType.eChamberCore)
...
function CltPBProc.systemMallMoneyConvertReq(nCmd,Server,Srevice,nSession,tData)
	local oRole = goGPlayerMgr:GetRoleBySS(Server,nSession)
	if not oRole then return end
	local oSubShop = goMallMar:GetSubShop(gtShopType.eCBuy)
...
------------服务器内部
--获取道具价格
function Srv2Srv.GetShopPriceReq(nSrcServer, nSrcService, nTarSession, nRoleID,nPropID)
...
--远程购买-
function Srv2Srv.BuyShopPriceReq(nSrcServer, nSrcService, nTarSession, nRoleID, nShopType, nPropID, nBuyNum, sFuncBackReq)
	local oRole = goGPlayerMgr:GetRoleByID(nRoleID)
	if not oRole then return end
...
--查询商会购买价格
function Srv2Srv.QueryCommercePriceReq(nSrcServer, nSrcService, nTarSession, tItemList) 
	assert(tItemList)
...
--查询商会购买价格
function Srv2Srv.QueryCommercePriceTblReq(nSrcServer, nSrcService, nTarSession, tItemList) 
	assert(tItemList)
	local oShop = goMallMar:GetSubShop(gtShopType.eChamberCore)
	assert(oShop)
	local tData = {}
...
function Srv2Srv.QueryCommerceSalePriceTblReq(nSrcServer, nSrcService, nTarSession, tItemList) 
	assert(tItemList)
	local oShop = goMallMar:GetSubShop(gtShopType.eChamberCore)
	assert(oShop)
	local tData = {}
...
function Srv2Srv.GetChamberCoreSkillReq(nSrcServer, nSrcService, nTarSession)
	local oSubShop = goMallMar:GetSubShop(gtShopType.eChamberCore)
	if not oSubShop then return end
