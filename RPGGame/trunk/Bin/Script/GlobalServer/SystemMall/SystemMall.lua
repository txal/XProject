...
function CMallMgr:LoadData()
	local oDB = goDBMgr:GetSSDB(nServerID, "global", GF.GetServiceID())
	--全局数据
 ...
 function CMallMgr:init()
 	self:LoadData()
 	self.m_nRoleDataTimer = goTimerMgr:Interval(gnAutoSaveTime, function () self:TickRoleData() end)
...
function CMallMgr:RegHourTimer()
    ...
    self.m_nHourTimer = goTimerMgr:Interval(nNextDayTime, function() self:ZeroUpdate(true) end)
end
...
goMallMar = goMallMar or CMallMgr:new()
goMallMar:InitCfg()