...
function CGRole:AppellationUpdate(tData, fnCallback)
    ...
    goRemoteCall:CallWait("AppellationUpdateReq", fnUpdateCallback, self:GetStayServer(), 
...
function CGRole:GetDupConf() return ctDupConf[GF.GetDupID(self:GetDupMixID())] end
...
function CGRole:GetMixObjID() return (gtObjType.eRole<<32|self.m_nID) end
...
function CGRole:KnapsackRemainCapacity(nPropID, bBind, fnCallback, nBagType)
    ...
    if nBagType == gtItemType.eFaBao then
        goRemoteCall:CallWait("FaBaoKnapsackRemainCapacityReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), nPropID)
    else
         goRemoteCall:CallWait("KnapsackRemainCapacityReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), nPropID, bBind)
    end
end
...
function CGRole:ItemCount(nType, nID, fnCallback)
    ...
    goRemoteCall:CallWait("RoleItemCountReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), nType, nID)
end
...
function  CGRole:ItemCountList(tItemList, fnCallback)
    ...
    goRemoteCall:CallWait("RoleItemCountListReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), tItemList)
end
...
function CGRole:CheckItemCount(tItemList, fnCallback)
    ...
    goRemoteCall:CallWait("RoleCheckItemCountReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), tItemList)
end
...
function CGRole:AddItem(tItemList, sReason, fnCallback)
    ...
    goRemoteCall:CallWait("RoleAddItemReq", _fnProxyCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), tItemList, sReason)
end
...
function CGRole:AddUnionGiftBox(nType, nNum) 
    ...
    if gnServerID == nServerID then 
        ...
        goRemoteCall:Call("AddUnionGiftBoxCntByRole", nServerID, nServiceID,0, self:GetID(), nType, nNum) 
    end
...
function CGRole:SubItem(tItemList, sReason, fnCallback)
    ...
    goRemoteCall:CallWait("RoleSubItemReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), tItemList, sReason)
end
...
function CGRole:SubPropByGrid(tItemList, sReason, fnCallback, nBagType)
    ...
    if nBagType == gtItemType.eFaBao then
        goRemoteCall:CallWait("RoleSubFaBaoByGridReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), tItemList, sReason)
    else
         goRemoteCall:CallWait("RoleSubPropByGridReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), tItemList, sReason)
    end
...
function CGRole:SubItemShowNotEnoughTips(tItemList, sReason, bFirstBreak, bNum, fnCallback)
    ...
    goRemoteCall:CallWait("RoleSubItemShowNotEnoughTipsReq", fnCallback, self:GetStayServer(), self:GetLogic(), 
...
function CGRole:GetPropDataWithSub(tItemList, sReason, bUnbindLimit, fnCallback)
    ...
    goRemoteCall:CallWait("RoleGetPropDataWithSubReq", fnCallback, self:GetStayServer(), self:GetLogic(), 
...
function CGRole:GetPropData(nGrid, fnCallback, nBagType)
    ...
    if nBagType == gtItemType.eFaBao then
         goRemoteCall:CallWait("FaBaoItemDataReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), nGrid)
    elseif nBagType == gtItemType.eProp then
         goRemoteCall:CallWait("KnapsackItemDataReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), nGrid)
    end
end
...
function CGRole:GetPropDataList(tList, fnCallback)
    ...
    goRemoteCall:CallWait("KnapsackItemListDataReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), tList)
end
...
function CGRole:GetPropListDataReq(nPropID, fnCallback, nBagType)
    assert(nPropID and fnCallback and nBagType, "参数错误")
    if nBagType == gtItemType.eFaBao then
         goRemoteCall:CallWait("FaBaoItemListDataReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), nPropID)
    elseif nBagType == gtItemType.eProp then
         goRemoteCall:CallWait("KnapsackPropListDataReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), nPropID)
    end
end
...
function CGRole:TransferItemList(tPropDataList, sReason, fnCallback)
    ...
    goRemoteCall:CallWait("RoleTransferItemListReq", _fnProxyCallback, self:GetStayServer(), 
        self:GetLogic(), self:GetSession(), self:GetID(), tPropDataList, sReason)
end 

function CGRole:SendPropDetailInfo(tPropData)
    goRemoteCall:Call("RolePropDetailInfoReq", self:GetStayServer(), 
...
function CGRole:GetKnapsackFreeGridCount(fnCallback)
    goRemoteCall:CallWait("KnapsackFreeGridCountReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID())
end

--发放邮件奖励
function CGRole:SendMailAward(tItemList, fnCallback, bNotSync)
    goRemoteCall:CallWait("SendMailAwardReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), tItemList, bNotSync)
end
...
function CGRole:SendSysMail(sTitle, sContent, tItemList, fnCallback)
    ...
    if fnCallback then
        goRemoteCall:CallWait("SendMailReq", fnCallback, nServerID, nService, 0, sTitle, sContent, tItemList, self:GetID())
    else
        goRemoteCall:Call("SendMailReq", nServerID, nService, 0, sTitle, sContent, tItemList, self:GetID())
    end
end
...
function CGRole:RoleInfoReq(nTarRoleID)
    ...
    goRemoteCall:CallWait("RoleUnionInfoReq", function(tUnion)
...
function CGRole:PushAchieve(sEvent, tArgs, fnCallback)
    if fnCallback then
        goRemoteCall:CallWait("PushAchieve", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), sEvent, tArgs)
    else
        goRemoteCall:Call("PushAchieve", self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), sEvent, tArgs)
    end
end

--帮贡变化通知(目前生活技能用到)
function CGRole:ChangeUnionContri()
     goRemoteCall:Call("ChangeUnionContri", self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID(), sEvent, tArgs)
end
...
function CGRole:KickOffline()
    ...
    CmdNet.Srv2Srv("KickClientReq", nTarServer, nTarSession>>gnServiceShift, nTarSession)
end

--取账号状态gtAccountState 
function CGRole:GetAccountState(fnCallback)
    goRemoteCall:CallWait("AccountValueReq", fnCallback, self:GetServer(), goServerMgr:GetLoginService(self:GetServer()), self:GetSession(), self:GetAccountID(), "m_nAccountState")
end
...
function CGRole:KejuAnswerHelpQuestionReq(tData)
    goRemoteCall:Call("KejuAnswerHelpQuestionReq",self:GetServer(),self:GetLogic(),self:GetSession(),self:GetID(),tData)
end
...
function CGRole:KejuHelpQuestionDataReq(oRole,nQuestionID)
    ...
    goRemoteCall:CallWait("KejuHelpQuestionDataReq",fnCallback,self:GetServer(),self:GetLogic(),self:GetSession(),self:GetID(),nRoleID,nQuestionID)
end

--求助玩家八荒任务数据请求
function CGRole:BaHuangHuoZhenTaskInfoReq(oRole,nBoxID)
     goRemoteCall:Call("GetHelpRoleDataReq",self:GetServer(),self:GetLogic(),self:GetSession(),self:GetID(),oRole:GetID(),nBoxID)
end
...
function CGRole:BaHuangHuoZhenHelpPackingBoxReq(oRole, nBoxID)
    local fnCallback = function (sReason, tSubItem, tAddItem)
        ...
        local fnSubItemCallback = function (bRet, tData)
            ...
            goRemoteCall:CallWait("HelpPackingBoxCheckHandleReq",fnPackingBoxCallback, self:GetServer(),self:GetLogic(),self:GetSession(),self:GetID(),oRole:GetID(), nBoxID, tData)
        end
        goRemoteCall:CallWait("BaHuangHuoZhenSubItemReq",fnSubItemCallback, oRole:GetServer(),oRole:GetLogic(),oRole:GetSession(),oRole:GetID(),tSubItem)
    end
    goRemoteCall:CallWait("BaHuangHuoZhenHelpPackingBoxCheckReq",fnCallback, self:GetServer(),self:GetLogic(),self:GetSession(),self:GetID(),nBoxID)
end
...
function CGRole:QueryRelationshipInvitePreStr(fnCalback)
    ...
    goRemoteCall:CallWait("RelationInviteInfoQueryReq", fnQueryCallback, 
...
function CGRole:RemoteCall(sRpcName, fnCallback, ...)
    if fnCallback then
        goRemoteCall:CallWait(sRpcName, fnCallback, self:GetServer(), self:GetLogic(), self:GetSession(), ...)
    else
        goRemoteCall:Call(sRpcName, self:GetServer(), self:GetLogic(), self:GetSession(), ...)
    end
end
...
function CGRole:UpdateGrowthTargetActVal(eActType, nVal)
    ...
    if gnServerID == nServer then 
        ...
        goRemoteCall:Call("UpdateGrowthTargetActValReq", nServer, goServerMgr:GetGlobalService(nServer, 20), 
...
function CGRole:AddGrowthTargetActVal(eActType, nVal)
    ...
    if gnServerID == nServer then 
        ...
        goRemoteCall:Call("AddGrowthTargetActValReq", nServer, goServerMgr:GetGlobalService(nServer, 20), 
            0, self:GetID(), eActType, nVal)
    end
end
...
function CGRole:CheckTeamMemCanChuangGuan(fnCallback)
    assert(fnCallback, "参数错误")
    goRemoteCall:CallWait("GetChuangGuanTypeReq", fnCallback, self:GetStayServer(), self:GetLogic(), self:GetSession(), self:GetID())
end
