...
function CLogger:GetRoleInfo(oRole)
	...
	tInfo.nService = goServerMgr:GetLogService(tInfo.nServer)
...
function CLogger:_normal_log(nEventID, sReason, oRole, Field1, Field2, Field3, Field4, Field5, Field6)
	...
	local tInfo = self:GetRoleInfo(oRole)
	if oRole then
		goRemoteCall:Call("EventLogReq", tInfo.nServer, tInfo.nService, 0, nEventID, sReason, tInfo, Field1, Field2, Field3, Field4, Field5, Field6, os.time())

	else
		local tLogList = goServerMgr:GetLogServiceList()
		for _, tConf in pairs(tLogList) do
			goRemoteCall:Call("EventLogReq", tConf.nServer, tConf.nID, 0, nEventID, sReason, tInfo, Field1, Field2, Field3, Field4, Field5, Field6, os.time())
		end
	end
end
...
function CLogger:EventLog(nEventID, oRole, Field1, Field2, Field3, Field4, Field5, Field6)
	if nEventID == gtEvent.eOnline or nEventID == gtEvent.eOffline then --上线离线事件
		...
		goRemoteCall:Call("OnlineLogReq", tInfo.nServer, tInfo.nService, 0, tInfo, nType, Field1, os.time())
		return
	...
	elseif self.m_tTaskEventMap[nEventID] then --任务类
		...
		goRemoteCall:Call("TaskLogReq", tInfo.nServer, tInfo.nService, 0, tInfo, nType, Field1, os.time())
		return
...
function CLogger:AwardLog(nEventID, sReason, oRole, nItemType, nItemID, nItemNum, Field1, Field2, Field3)
	...
	--元宝有单独日志
		...
		goRemoteCall:Call("YuanBaoLogReq", tInfo.nServer, tInfo.nService, 0, tInfo, os.time())
		return
	end
...
function CLogger:CreateAccountLog(nSource, nAccountID, sAccountName, nVIP)
	...
	local nLogService = goServerMgr:GetLogService(gnServerID)
...
function CLogger:CreateRoleLog(nAccountID, nRoleID, sRoleName, nLevel, sImgHeader, nGender, nSchool)
	...
	local nLogService = goServerMgr:GetLogService(gnServerID)
	goRemoteCall:Call("CreateRoleLogReq", gnServerID, nLogService, 0, nAccountID, nRoleID, sRoleName, nLevel, sImgHeader, nGender, nSchool, os.time())
end
...
function CLogger:UpdateAccountLog(oRole, tParam) 
	...
	local nLogService = goServerMgr:GetLogService(nServerID)
...
function CLogger:InviteLog(oSrcRole, oTarRole)
	...
	local nLogService = goServerMgr:GetLogService(nSrcServer)
	goRemoteCall:Call("ShareLogReq", nSrcServer, nLogService, 0, nSrcServer, nSrcRoleID, nTarServer, nTarRoleID, os.time())
end

--聊天日志
function CLogger:TalkLog(oRole, tData)
	local nServer = oRole:GetServer()
	local nLogService = goServerMgr:GetLogService(nServer)
	goRemoteCall:Call("TalkLogReq", nServer, nLogService, 0, tData)
end
...
function CLogger:ActivityLog(oRole, nActID, sActName, tCost, tAward, nCharge, sExt1, sExt2, nSubActID, sSubActName)
	...
	local nServer = oRole:GetServer()
	local nLogService = goServerMgr:GetLogService(nServer)
	goRemoteCall:Call("ActivityLogReq", nServer, nLogService, 0, tLog)
end
...
function CLogger:CreateUnionLog(oRole, nUnionID, nDisplayID, sUnionName, nUnionLevel, nLeaderID, sLeaderName, nCreateTime)
	...
	local nServer = oRole:GetServer()
	local nLogService = goServerMgr:GetLogService(nServer)
	goRemoteCall:Call("CreateUnionLogReq", nServer, nLogService, 0, tLog)
end
--删除帮派
function CLogger:DelUnionLog(nServer, nUnionID)
	local nLogService = goServerMgr:GetLogService(nServer)
	goRemoteCall:Call("DelUnionLogReq", nServer, nLogService, 0, nUnionID)
end
--更新帮派日志
function CLogger:UpdateUnionLog(nServer, nUnionID, tParam)
	local nLogService = goServerMgr:GetLogService(nServer)
	goRemoteCall:Call("UpdateUnionLogReq", nServer, nLogService, 0, nUnionID, tParam)
end
--创建帮派成员日志
function CLogger:CreateUnionMemberLog(oRole, nUnionID, nPosition, nJoinTime, nLeaveTime, nCurrContri, nTotalContri, nDayContri)
	...
	local nServer = oRole:GetServer()
	local nLogService = goServerMgr:GetLogService(nServer)
	goRemoteCall:Call("CreateUnionMemberLogReq", nServer, nLogService, 0, tLog)
end
--更新帮派成员日志
function CLogger:UpdateUnionMemberLog(nServer, nRoleID, tParam)
	local nLogService = goServerMgr:GetLogService(nServer)
	goRemoteCall:Call("UpdateUnionMemberLogReq", nServer, nLogService, 0, nRoleID, tParam)
end
