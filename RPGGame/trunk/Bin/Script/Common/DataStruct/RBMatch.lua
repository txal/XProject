...
function CRBMatch:Match(min, max, nMatchNum, tExceptList, fnFilter) 
    ...
    if not fnFilter then
        local tRandResultList, tRandResultTbl = GF.RandDiffNum(nBeginIndex, nEndIndex, nMaxMatchNum) 
        --nMatchNum前面已判定大于0
        ...
        elseif 2 == nFindMode then 
            ...
            if #tTempList > nSelectNum then 
                ...
                if nSelectNum > nRemoveNum then 
                    local tRemoveList, tRemoveMap = GF.RandDiffNum(1, #tTempList, nRemoveNum)
                    ...
                    local tSelectList, tSelectMap = GF.RandDiffNum(1, #tTempList, nSelectNum)
                    for nIndex, key in ipairs(tTempList) do 
        ...
        if 1 == nFindMode then --尽量避免去扫描整个区间
            for nRandIndex in GF.RandDiffIterator(nBeginIndex, nEndIndex) do 
