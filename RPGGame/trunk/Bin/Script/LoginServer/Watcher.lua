...
function CWatcher:Init()
	...
	--屏蔽
	-- self:Sign()
	-- self.m_nTimer = goTimerMgr:Interval(180, function() self:OnTimer() end)
end
...
function CWatcher:TestLogin()
	...
	local function _TryCall(i)
		...
		goRemoteCall:CallWait("RemoteCallTestReq", function(nValue)
			if not nValue then
				return self:Report(2, tService[3])
			end
			LuaTrace("try call time:", tService[1], GF.GetUnixMSTime()-nValue)
			_TryCall(i+1)
		end, tService[2], tService[1], 0, GF.GetUnixMSTime())
	end
	_TryCall(1)
end
...
goWatcher = goWatcher or CWatcher:new()