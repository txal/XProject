...
--玩家点击翻牌
function CltPBProc.PVEClickRewardReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    local oBattleDup = goBattleDupMgr:GetBattleDup(oRole:GetBattleDupID())
...
--玩家点击创建怪物
function CltPBProc.PVECreateMonsterReq(nCmd, nServer, nService, nSession, tData)
    print("玩家点击创建怪物**************")
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    local oBattleDup = goBattleDupMgr:GetBattleDup(oRole:GetBattleDupID())
...
--玩家点击破解机关请求
function CltPBProc.PVEClickCrackOrganReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    local oBattleDup = goBattleDupMgr:GetBattleDup(oRole:GetBattleDupID())
...
--玩家点击破解机关结果返回请求
function CltPBProc.PVEPinTuResuitReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    local oBattleDup = goBattleDupMgr:GetBattleDup(oRole:GetBattleDupID())
...
--------服务器内部
--进入决战九霄副本
function Srv2Srv.EnterBattleDupReq(nSrcServer, nSrcService, nTarSession, sTitle, sContent, tItemList, nTarRoleID)
