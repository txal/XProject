...
function CGuaJiBattleDup:Ctor(nID, nType)
	self.m_nID = nID
    self.m_nType = nType
    self.m_tDupList = GF.WeakTable("v")
    self.m_tRoleMap = GF.WeakTable("v")
	self.m_tMonsterMap = GF.WeakTable("v")
...
function CGuaJiBattleDup:OnRelease()
	...
	goTimerMgr:Clear(self.m_nBattleSecTimer)
...
function CGuaJiBattleDup:RegAutoBattle(oRole)
    ...
    self.m_nBattleSecTimer = goTimerMgr:Interval(tGuanQiaConf.nPatrolSec, function() self:NoticeBattleStart() end)
end
...
function CGuaJiBattleDup:OnObjEnter(oLuaObj, bReconnect)
	...
	if nObjType == gtObjType.eRole then
		...
		--goRemoteCall:Call("StartGuaJiAutoReward", nServerID, goServerMgr:GetGlobalService(nServerID,20), 0, oLuaObj:GetID(), {nGuanQia=nGuanQia})
			...
			if nBattleTimes >= tGuanQiaConf.nChalBossLimit and not self.m_bIsInBattle and bAutoBattle then
				...
				if not oLuaObj:IsInBattle() then		--如果是重连在boss战先不启动
					...
					if nLeaveTime == 0 or (nLeaveTime > 0 and os.time() - nLeaveTime < 60) then		--离开时间等0表示首次进来，或小于60秒都马上战斗
						...
						goTimerMgr:Clear(self.m_nBattleSecTimer)
						self.m_nBattleSecTimer = goTimerMgr:Interval(tGuanQiaConf.nPatrolSec+3, function() self:NoticeBattleStart() end)
					end
				end
			end
			goRemoteCall:Call("StopGuaJiAutoReward", nServerID, goServerMgr:GetGlobalService(nServerID,20), 0, oLuaObj:GetID())
		end
		goRemoteCall:CallWait("GetIsAutoBattle", CallBack, nServerID, goServerMgr:GetGlobalService(nServerID,20), 0, oLuaObj:GetID())
	end
end
...
function CGuaJiBattleDup:OnObjLeave(oLuaObj, nBattleID)
	local nObjType = oLuaObj:GetObjType() --gtObjType
	    --人物
	if nObjType == gtObjType.eRole and (not oLuaObj:IsRobot()) then
        if nBattleID <= 0 then
...
function CGuaJiBattleDup:SendLeaveRewardInfo(oRole)
    ...
    goRemoteCall:CallWait("IsGuaJi", LeaveRewardInfo, nServerID, goServerMgr:GetGlobalService(nServerID,20), 0, oRole:GetID())
end
...
function CGuaJiBattleDup:SetAutoBattle(oRole, bAutoBattle)
	...
	local nServerID = oRole:GetServer()	
	goRemoteCall:Call("SetIsAutoBattle", nServerID, goServerMgr:GetGlobalService(nServerID,20), 0, oRole:GetID(), {bAutoBattle=bAutoBattle})
end
...
function CGuaJiBattleDup:OnBattleEnd(oLuaObj, tBTRes, tExtData)
	...
	local tGuanQiaConf = ctGuaJiConf:GetGuanQiaConf(nGuanQia)
	if nObjType == gtObjType.eRole and tBTRes.bWin then
		--战斗胜利奖励
		...
		--延时启动自动巡逻战斗
		goTimerMgr:Clear(self.m_nBattleSecTimer)
		self.m_nBattleSecTimer = goTimerMgr:Interval(7, function() self:NoticeBattleStart() end)		--前端显示界面5秒，关闭界面后2秒进入战斗
	elseif nObjType == gtObjType.eRole and not tBTRes.bWin then
...
function CGuaJiBattleDup:EnterBattleDupReq(oRole)
	...
	goBattleDupMgr:CreateBattleDup(gtBattleDupType.eGuaJi, function(nDupMixID)
		...
		--goRemoteCall:Call("GuaJiCreateRobotReq", gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), 0, tRobotList)
...
function CGuaJiBattleDup:SendGuaJiStatue(oRole)
	...
	local nServerID = oRole:GetServer()	
	goRemoteCall:CallWait("GetIsAutoBattle", SendMsg, nServerID, goServerMgr:GetGlobalService(nServerID,20), 0, oRole:GetID())
end
...
function CGuaJiBattleDup:GuaJiBattleEndNoticeReq(oRole)
	if self.m_bIsInBattle then
		...
		local nServerID = oRole:GetServer()
		goRemoteCall:CallWait("GetIsAutoBattle", CallBack, nServerID, goServerMgr:GetGlobalService(nServerID,20), 0, oRole:GetID())
	end
end
