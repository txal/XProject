...
--销毁副本
function CXinMoQinShi:OnRelease() 
							...
							goClientCall:CallWait("ConfirmRet", function(tData)
								if tData.nSelIdx == 2 then	
									...
									if oLuaObj:GetTeamID() <= 0 then	
											...
											if bCanJoinIn then
												...
												goClientCall:CallWait("ConfirmRet", function(tData)
													if tData.nSelIdx == 1 then		--继续匹配
														...
														goRemoteCall:CallWait("JoinMergeTeamReq", JoinMergeTeamCallBack, gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), oLuaObj:GetSession(), oLuaObj:GetTeamID(), gtBattleDupType.eXinMoQinShi)
													end
												end, oLuaObj, tMsg)
											else
												oLuaObj:MatchTeam(gtBattleDupType.eXinMoQinShi, tBattleDupConf.sName, true)
										...
										goRemoteCall:CallWait("CheckJoinMergeTeamReq", CheckCallBack, gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), oLuaObj:GetSession(), oLuaObj:GetTeamID(), gtBattleDupType.eXinMoQinShi)										
									end
								else
								...
								oLuaObj:GetTeam(function(nTeamID, tTeam)
									if nTeamID > 0 then
										if tTeam[1].nRoleID ~= oLuaObj:GetID() then
											goRemoteCall:Call("GotoLeader",gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), oLuaObj:GetSession(), oLuaObj:GetID(), {nBattleDupType=gtBattleDupType.eXinMoQinShi})
										end
									end
								end)
							end
...
function CXinMoQinShi:SendAllMonsterInfo(oRole)
	...
	if oRole then
		...
		local tSessionList = self:GetSessionList()
		CmdNet.PBBroadcastExter("XinMoQinShiMonListRet", tSessionList, tMsg)
	end
end
