...
--销毁副本
function CJueZhanJiuXiao:OnRelease()
	print("决战九霄副本被销毁", self.m_nID)
	for _, oDup in pairs(self.m_tDupList) do
		goDupMgr:RemoveDup(oDup:GetMixID())
	end
	 self.m_nMinTimer = goTimerMgr:Clear(self.m_nMinTimer)
	 self.m_nOverTime = goTimerMgr:Clear(self.m_nOverTime)
	 self.m_nMinTimerS = goTimerMgr:Clear(self.m_nMinTimerS)
	 self.m_nPinTuTimer = goTimerMgr:Clear( self.m_nPinTuTimer)
	 self.m_nSwitchDupTimer = goTimerMgr:Clear(self.m_nSwitchDupTimer)
...
function CJueZhanJiuXiao:CreateMonsterReq(oRole)
	...
	if tBattleDupConf.nTaskType == 0 then
		...
		goTimerMgr:Clear(self.m_nMinTimerS)
		self.m_nMinTimerS = nil
		...
		self.m_nPinTuTimer = goTimerMgr:Interval(30, function() self:AutoRobot() end)
		self.m_nMinTimerS = goTimerMgr:Interval(93, function() self:PinTuResuiTiming() end)
	else
		local nDupIndex =  self:GetDupIndex(oRole)
		local oDup = self.m_tDupList[nDupIndex]
		if oDup then
...
function CJueZhanJiuXiao:ClickCrackOrganReq(oRole)
	local fnCrackOrganCallBack = function(nTeamID, tTeam)
		...
		self.m_nMinTimerS = goTimerMgr:Interval(90 , function() self:PinTuResuiTiming() end)
	end
...
function CJueZhanJiuXiao:PinTuResuitReq(oRole, nResuit, bRobot)
	...
	local fnGetTeamCallBack = function (nTeamID, tTeam)
		...
		self.m_nMinTimer = goTimerMgr:Interval(6, function() self:AutoReceiveReward(true) end)
		self.m_nFanPaFlag = true
		self.m_nCount = 0
		goTimerMgr:Clear(self.m_nMinTimerS)
		self.m_nMinTimerS = nil
