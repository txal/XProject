...
--销毁副本
function CLuanShiYaoMo:OnRelease() 
							...
							goClientCall:CallWait("ConfirmRet", function(tData)
								if tData.nSelIdx == 2 then	
									...
									if oLuaObj:GetTeamID() <= 0 then	
											...
											if bCanJoinIn then
												...
												goClientCall:CallWait("ConfirmRet", function(tData)
													if tData.nSelIdx == 1 then		--继续匹配
														...
														goRemoteCall:CallWait("JoinMergeTeamReq", JoinMergeTeamCallBack, gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), oLuaObj:GetSession(), oLuaObj:GetTeamID(), gtBattleDupType.eLuanShiYaoMo)
													end
												end, oLuaObj, tMsg)
											else
												oLuaObj:MatchTeam(gtBattleDupType.eLuanShiYaoMo, tBattleDupConf.sName, true)
										...
										goRemoteCall:CallWait("CheckJoinMergeTeamReq", CheckCallBack, gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), oLuaObj:GetSession(), oLuaObj:GetTeamID(), gtBattleDupType.eLuanShiYaoMo)										
									end
								else
								...
								oLuaObj:GetTeam(function(nTeamID, tTeam)
									if nTeamID > 0 then
										if tTeam[1].nRoleID ~= oLuaObj:GetID() then
											goRemoteCall:Call("GotoLeader",gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), oLuaObj:GetSession(), oLuaObj:GetID(), {nBattleDupType=gtBattleDupType.eLuanShiYaoMo})
										end
									end
								end)
							end
	...
	elseif nObjType == gtObjType.eRole and tBTRes.bWin then
		...
		if nCompCount < nRewardTimes then
			...
			if oLuaObj:IsLeader() then
				...
				-- 	goRemoteCall:Call("LeaveTeamAndKickReq", gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID,110), oLuaObj:GetSession(), oLuaObj:GetID())
				-- end
			end
		else
			oLuaObj.m_oDailyActivity:OnCompleteDailyOnce(gtDailyID.eLuanShiYaoMo, "乱世妖魔奖励活跃")
