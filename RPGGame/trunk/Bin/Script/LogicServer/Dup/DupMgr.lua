...
function CDupMgr:Init()
    for nDupID, tConf in pairs(ctDupConf) do
        if tConf.nType == CDupBase.tType.eCity and tConf.nLogic == GF.GetServiceID() then
    ...
    self.m_nTimer = goTimerMgr:Interval(300, function() self:Tick() end)
end
...
function CDupMgr:AddDupCount(nDupMixID, nNum) 
    ...
    local nDupConfID = GF.GetDupID(nDupMixID)
...
function CDupMgr:EnterDup(nDupMixID, oNativeObj, nPosX, nPosY, nLine, nFace)
    ...
    -- if oLuaObj:GetObjType() == gtObjType.eRole then 
    --     local oRole = oLuaObj
    ...
    --         goRemoteCall:CallWait("AsyncEnterScene", fnCheck, nServer, nService, 0, oRole:GetID())
    --     else
    --         fnEnterCallback()
    --     end
    -- else
...
function CDupMgr:SwitchLogic(tSwitch)
    ...
    if tTarDupConf.nLogic == GF.GetServiceID() then
        return self:EnterDup(tSwitch.nTarDupMixID, oRole:GetNativeObj(), tSwitch.nPosX, tSwitch.nPosY, tSwitch.nLine, tSwitch.nFace)
    end
    ...
    if oRole:IsRobot() then --机器人
        ...
        goRemoteCall:Call("RobotSwitchLogicReq", nTargetServerID, tTarDupConf.nLogic, 
            tSwitch.nSession, tSwitch, tCreateData, tSaveData)
    else
        --远程调用
        goPlayerMgr:RoleOfflineReq(tSwitch.nRoleID, true) --把当前逻辑服的角色下了
        goRemoteCall:Call("SwitchLogicReq", nTargetServerID, tTarDupConf.nLogic, tSwitch.nSession, tSwitch)
    end
end
...
function CDupMgr:EnterDupReq(oRole, nDupMixID, nPosX, nPosY, nLine, nFace)
    ...
    if GF.GetServiceID() == tDupConf.nLogic then
        ...
        goRemoteCall:CallWait("EnterCheckReq", fnEnterCallback, oRole:GetServer(), 
...
function CDupMgr:OnDupCollected(nDupMixID)
    LuaTrace("副本地图被收集(CPP)***", nDupMixID, GF.GetDupID(nDupMixID))
    local oDup = self:GetDup(nDupMixID)
    if not oDup then
        return
    end
...
function CDupMgr:DupRoleViewListReq(oRole)
    ...
    local tRoleList = {}
    local tRoleNativeList = oDup:GetAreaObservers(oRole:GetAOIID(), gtObjType.eRole)
