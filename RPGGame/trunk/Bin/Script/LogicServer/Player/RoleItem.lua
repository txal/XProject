        ...
        if tConf.nType == gtPropType.eCurr then
            ...
            if nItemNum > 0 then
                if tConf.nType == gtPropType.eFaBao then
                    ...
                    GF.SendItemTalk(self, "getitem", {nItemID, nItemNum}, bNotSync)
    ...
    elseif nItemType == gtItemType.eCurr then
        if nItemID ~= gtCurrType.ePetExp and nItemNum > 0 then
            ...
            if nPropID then --发送个人频道 
                GF.SendItemTalk(self, "getitem", {nPropID, nItemNum}, bNotSync)
            end
        end
        ...
        elseif nItemID == gtCurrType.eUnionContri then
            --日志交由真正加货币的地方写
            goRemoteCall:Call("AddUnionContriReq", self:GetServer(), 20, self:GetSession(), self:GetID(), nItemNum, sReason)

        elseif nItemID == gtCurrType.eUnionExp then
            --日志交由真正加货币的地方写
            goRemoteCall:Call("AddUnionExpReq", self:GetServer(), 20, self:GetSession(), self:GetID(), nItemNum, sReason)
    ...
    for _, tItem in ipairs(tItemList) do 
        ...
        if not self.m_oKnapsack:IsOccupyBagGrid(nItemType, nItemID) then
            ...
            if nMailNum > 0 then
                ...
                if #tMailList >= gnMaxMailItemLength then 
                    GF.SendMail(self:GetServer(), "背包已满", "背包已满，请及时领取邮件", tMailList, self:GetID())
                    tMailList = {}
                    bMailTips = true
                end
            end
...
function CRole:TransferItemList(tPropDataList, sReason, bNotSync)
	...
	if #tTempList > 0 then  --打包整合一起发送邮件
		GF.SendMail(self:GetServer(), "背包已满", "背包已满，请及时领取邮件", tTempList, self:GetID())
		self:Tips("背包空间不足，请及时清理背包")
    end
...
function CRole:QueryCommercePrice(tItemList, fnCallback)
    ...
    local nServerID = self:GetServer()
    local nService = goServerMgr:GetGlobalService(nServerID, 20)
    goRemoteCall:CallWait("QueryCommercePriceReq", fnCallback, 
...
function CRole:QueryItemPrice(tItemList, bCeil, fnCallback)
    return GF.QueryItemPrice(self:GetServer(), tItemList, bCeil, fnCallback)
end
