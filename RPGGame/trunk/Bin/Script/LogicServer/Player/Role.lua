...
function CRole:RegAutoSave()
    goTimerMgr:Clear(self.m_nSaveTimer)
    self.m_nSaveTimer = goTimerMgr:Interval(gnAutoSaveTime, 
    function(nTimerID)
...
function CRole:OnLoaded()
    ...
    local nLastDupConfID = GF.GetDupID(nLastDupMixID)
    ...
    local nDupConfID = GF.GetDupID(nCurDupMixID)
...
function CRole:GetMixObjID() return gtObjType.eRole<<32|self.m_nID end
...
function CRole:GetDupID() return GF.GetDupID(self:GetDupMixID()) end --副本配置ID
    ...
    if nOldActState ~= self.m_nActState then
        ...
        for _, tConf in pairs(tGlobalServiceList) do
            if tConf.nServer == self:GetServer() or tConf.nServer == gnWorldServerID then
                goRemoteCall:Call("GRoleActStateUpdateReq", tConf.nServer, tConf.nID, self:GetSession(), self:GetID(), self.m_nActState)
            end
        end
        if bBroadcast then
...
function CRole:BindSession(nSession)
    ...
    if self.m_nSession > 0 then
        self.m_nGateway = GF.GetGateServiceBySession(self.m_nSession)
    end
end
...
function CRole:UpdatePower()
    ...
    nPower = math.min(gnMaxInteger, math.floor(nPower))
...
function CRole:UpdateAttr()
    ...
    local tAttrList = {}
    for k, v in pairs(self.m_tBattleAttr) do
        v = math.min(gnMaxInteger, math.floor(v))
...
function CRole:GlobalRoleOnline(bOnline, bRelease)
    ...
    for _, tConf in pairs(tGlobalServiceList) do
        if tConf.nServer == self:GetServer() or tConf.nServer == gnWorldServerID then
            goRemoteCall:Call(sFunc, tConf.nServer, tConf.nID, self:GetSession(), self:GetID(), tRole)
        end
    end
end
...
function CRole:GlobalRoleUpdate(tParam)
    ...
    for _, tConf in pairs(tGlobalServiceList) do
        if tConf.nServer == self:GetServer() or tConf.nServer == gnWorldServerID then
            goRemoteCall:Call("GRoleUpdateReq", tConf.nServer, tConf.nID, self:GetSession(), self:GetID(), tParam)
        end
    end
end
...
function CRole:GlobalBattleUpdate(tData)
    ...
    for _, tConf in pairs(tGlobalServiceList) do
        if tConf.nServer == self:GetServer() or tConf.nServer == gnWorldServerID then
            goRemoteCall:Call("OnBattleEndReq", tConf.nServer, tConf.nID, self:GetSession(), self:GetID(), tData)
        end
    end
end
...
function CRole:AfterOnline()
    ...
    if self.m_nVIP == 0 then
        ...
        goTimerMgr:Interval(1, function(nTimerID)
...
function CRole:OnDisconnect()
    ...
    if oDup then
        ...
        LuaTrace("CurrDup:", tCurDup, GF.GetDupID(tCurDup[1]), "OnDisconnect: 找不到场景->玩家还没进入场景")
    end
...
function CRole:AddVitality(nVal)
    ...
    --消耗活力限时奖励
    if nVal < 0 then
        goRemoteCall:Call("OnTAHLReq", self:GetServer(), goServerMgr:GetGlobalService(self:GetServer(),20), 0, self:GetID(), self.m_nVitality-nOldVitality)
    end
...
function CRole:AddStoreExp(nVal)
    ...
    self.m_nStoreExp = math.max(0, math.min(gnMaxInteger, self.m_nStoreExp+nVal))
    self:MarkDirty(true)
...
function CRole:AddYuanBao(nVal, bNotSync)
    ...
    self.m_nYuanBao = math.max(0, math.min(gnMaxInteger, self.m_nYuanBao+nVal))
    ...
    for _, tService in pairs(tGlobalServiceList) do
        goRemoteCall:Call("OnRoleYuanBaoChange", tService.nServer, tService.nID, 0, self:GetID(), nVal, false)
    end
...
function CRole:AddBYuanBao(nVal, bNotSync)
    ...
    self.m_nBYuanBao = math.max(0, math.min(gnMaxInteger, self.m_nBYuanBao+nVal))
    ...
    local tGlobalServiceList = goServerMgr:GetGlobalServiceList(self:GetServer())
    for _, tService in pairs(tGlobalServiceList) do
        goRemoteCall:Call("OnRoleYuanBaoChange", tService.nServer, tService.nID, 0, self:GetID(), nVal, true)
    end
...
function CRole:AddJinBi(nVal, bNotSync)
    ...
    self.m_nJinBi= math.max(0, math.min(gnMaxInteger, self.m_nJinBi+nVal))
    self:MarkDirty(true)
    ...
    --累计消耗金币
    if nVal < 0 then
        goRemoteCall:Call("OnTAJBReq", self:GetServer(), goServerMgr:GetGlobalService(self:GetServer(),20), 0, self:GetID(), self.m_nJinBi-nOldJinBi)
    end
...
function CRole:AddYinBi(nVal, bNotSync)
    ...
    self.m_nYinBi = math.max(0, math.min(gnMaxInteger, self.m_nYinBi+nVal))
    self:MarkDirty(true)
    ...
    elseif nVal < 0 then
    --累计消耗银币
        goRemoteCall:Call("OnTAYBReq", self:GetServer(), goServerMgr:GetGlobalService(self:GetServer(),20), 0, self:GetID(), self.m_nYinBi-nOldYinBi)
    end
...
function CRole:AddChivalry(nVal)
    if nVal < 0 then
        ...
        self.m_nChivalry = math.max(0, math.min(gnMaxInteger, self.m_nChivalry+nVal))
...
function CRole:AddArenaCoin(nVal)
    ...
    self.m_nArenaCoin = math.max(0, math.min(gnMaxInteger, self.m_nArenaCoin+nVal))
    self:MarkDirty(true)
...
function CRole:AddActValue(nVal)
    ...
    self.m_oDailyActivity.m_nTotalActValue = math.max(0, math.min(gnMaxInteger, nActVal+nVal))
    self:MarkDirty(true)
    self.m_oDailyActivity:CheckActRewardState()
    goRemoteCall:Call("GRoleActiveNumChangeReq", gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), 
...
function CRole:AddJinDing(nVal)
    ...
    self.m_nJinDing= math.max(0, math.min(gnMaxInteger, self.m_nJinDing+nVal))
    self:MarkDirty(true)
...
function CRole:AddFuYuan(nVal)
    if nVal == 0 then return end
    self.m_nFuYuan= math.max(0, math.min(gnMaxInteger, self.m_nFuYuan+nVal))
    self:MarkDirty(true)
...
function CRole:AddLanZuan(nVal)
    if nVal == 0 then return end
    self.m_nLanZuan= math.max(0, math.min(gnMaxInteger, self.m_nLanZuan+nVal))
    self:MarkDirty(true)
...
function CRole:SyncRoleLogic(bRelease)
    ...
    CmdNet.Srv2Srv("SyncRoleLogic", nServer, nGateway, nSession, self:GetID(), (bRelease and 1 or 0))
end
...
function CRole:ReturnScene(bReconnect)
    ...
    if oDup then 
        ...
        if GF.IsBlockUnit(tCurrDup[1], tCurrDup[2], tCurrDup[3]) then
...
function CRole:OnEnterScene(nDupMixID, bReconnect) --这里还没进行观察者和被观察者相关同步
    ...
    --是否在障碍点
    if GF.IsBlockUnit(nDupMixID, tCurrDup[2], tCurrDup[3]) then
        ...
        if tDupConf.nType == CDupBase.tType.eCity then 
            ...
            if nTargetActFlag == gtRoleTarActFlag.eArena then 
                if tActParam and tActParam.nEnemyID then 
                    ...
                    goRemoteCall:Call("JoinArenaBattleReq", nServer, nService, self:GetSession(), self:GetID(), nEnemyID)
                end
            end
        end
    end
...
function CRole:FlushRoleView()
    ...
    goRemoteCall:Call("GRoleUpdateReq", gnWorldServerID,nServiceID,self:GetSession(),self:GetID(),{m_tShapeData=self:GetShapeData()})
end
...
function CRole:UpdateRoleSummary()
    ...
    goRemoteCall:Call("RoleUpdateSummaryReq", self:GetServer(), goServerMgr:GetLoginService(self:GetServer()), self:GetSession()
...
function CRole:RoleModNameReq(sName, bProp)
    ...
    if string.len(sName) > gnMaxRoleNameLen then
...
function CRole:UpdateTeamDataReq(bAll)
    goRemoteCall:Call("WUpdateTeamDataReq", gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), 
...
function CRole:CheckDirtySave(sModule, bDirty, tData)
    ...
    local sData = cjson_raw.encode(tData)
    local sLastSortedData = GF.SortString(sLastData)
    local sSortedData = GF.SortString(sData)
