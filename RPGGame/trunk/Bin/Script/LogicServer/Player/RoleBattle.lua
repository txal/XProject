...
function CRole:OnBattleEnd(tBTRes, tExtData)
    ...
    -- goRemoteCall:Call("OnBattleEndReq", gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), self:GetSession(), self:GetID(), tBTRes)
	...
	if tExtData and tExtData.bArenaBattle then
		...
		goRemoteCall:Call("ArenaBattleEndReq", nServerID, goServerMgr:GetGlobalService(nServerID, 20), 
	...
	if tExtData and tExtData.bMentorshipTaskBattle then
		...
		goRemoteCall:Call("MentorshipTaskBattleEndReq", gnWorldServerID, nSrcService, 
...
function CRole:CreateTeam(fnCallback)
	goRemoteCall:CallWait("WCreateTeamReq", function(nTeamID, tTeam)
		fnCallback(nTeamID, tTeam)
	end, gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), self:GetSession(), self:GetID())
end
...
function CRole:QuitTeam(fnCallback)
	if fnCallback then 
		goRemoteCall:CallWait("WQuitTeamReq", fnCallback, gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), self:GetSession(), self:GetID())
	else
		goRemoteCall:Call("WQuitTeamReq", gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), self:GetSession(), self:GetID())
	end
end
...
function CRole:MatchTeam(nGameType, sGameName, bSys)
	goRemoteCall:Call("WMatchTeamReq", gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), self:GetSession(), self:GetID(), nGameType, sGameName, bSys)
end

--取消组队匹配
function CRole:CancelMatchTeam(nGameType, sGameName)
	goRemoteCall:Call("WCancelMatchTeamReq", gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), self:GetSession(), self:GetID(), nGameType, sGameName)
end
