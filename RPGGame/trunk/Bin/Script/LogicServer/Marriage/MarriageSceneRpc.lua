------- 客户端服务器 --------
--结婚请求
function CltPBProc.RoleMarryReq(nCmd, nSrcServer, nSrcService, nTarSession, tData)
...
--选择婚礼级别反馈请求
function CltPBProc.MarriageChoosWeddingLevelReactReq(nCmd, nSrcServer, nSrcService, nTarSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nSrcServer, nTarSession)
    if not oRole then return end
    if not goMarriageSceneMgr then
        return
...
--拾取喜糖请求
function CltPBProc.MarriagePickWeddingCandyReq(nCmd, nSrcServer, nSrcService, nTarSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nSrcServer, nTarSession)
    if not oRole then return end
    if not goMarriageSceneMgr then
        return
...
--花轿租赁请求
function CltPBProc.MarriagePalanquinRentReq(nCmd, nSrcServer, nSrcService, nTarSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nSrcServer, nTarSession)
    if not oRole then return end
    if not goMarriageSceneMgr then
        return
...
function CltPBProc.MarriagePickItemStateReq(nCmd, nSrcServer, nSrcService, nTarSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nSrcServer, nTarSession)
    if not oRole then return end
    if not goMarriageSceneMgr then
        return
...
--GM指令刷新月老道具
function Srv2Srv.UpdateOldMan(nSrcServer, nSrcService, nTarSession, nRoleID, nNumber)
    local oOldMan = goMarriageSceneMgr:GetOldManItemInst()
    if oOldMan then
        --TODD先释放之前已经刷新了的
