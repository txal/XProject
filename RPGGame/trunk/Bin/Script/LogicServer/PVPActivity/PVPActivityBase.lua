...
function CPVPActivityBase:UpdateRankData(oRoleData)
	...
	-- if GF.IsRobot(nRoleID) then --机器人不参与排名
	-- 	return 
	-- end
...
function CPVPActivityBase:OnStart()
	...
	if self:IsAutoMatch() then
		self.m_nMatchTimer = goTimerMgr:Interval(gnPVPActivityAutoMatchInterval, function () self:AutoMatch() end)
	end
...
function CPVPActivityBase:KickRole(nRoleID)
	...
	goRemoteCall:CallWait("KickFromTeamReq", fnCallBack, gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), 0, nRoleID)
end
...
function CPVPActivityBase:AddRankReward()
	...
	local fnTraverseCallback = function(nDataIndex, nRank, nRoleID, tData) 
		local oRoleData = self:GetRoleData(nRoleID)
		if oRoleData and oRoleData.m_nBattleCount >= 3 and not GF.IsRobot(nRoleID) then --大于等于3次才发送奖励
...
function CPVPActivityBase:OnEnd()
	...
	if self.m_nMatchTimer then --这里可以直接停止匹配计时器了
		goTimerMgr:Clear(self.m_nMatchTimer)
		self.m_nMatchTimer = nil
	end
	...
	self:AddRankReward()
	self.m_nKickAllTimer = goTimerMgr:Interval(60, function () self:KickAllRole() end)
end

function CPVPActivityBase:KickAllRole()
	if self.m_nKickAllTimer then
		goTimerMgr:Clear(self.m_nKickAllTimer)
		self.m_nKickAllTimer = nil
	end
	--踢出所有未离开场景玩家
...
--实例被销毁，清理资源
function CPVPActivityBase:OnRelease()
...
function CPVPActivityBase:AfterRoleEnter(oRole)
	...
	if oRole:IsRobot() then 
		...
		goRemoteCall:Call("RobotJoinTeamMatchReq", gnWorldServerID, 
	...
	if nMaxJoinNum < nJoinNum and nRobotNum > 0 then 
		...
		local tKickList, tKickIndex = GF.RandDiffNum(1, nRobotNum, nKickNum)
		local nCount = 1
		tKickList = {}
...
function CPVPActivityBase:OnRoleLeave(oRole)
	...
	if oRoleData then
		...
		if oRole:IsRobot() then 
			...
			goRemoteCall:Call("RobotCancelTeamMatchReq", gnWorldServerID, 
				goServerMgr:GetGlobalService(gnWorldServerID, 110), 0, nRoleID, self:GetMixDupType())
		end
	else		
...
function CPVPActivityBase:OnRoleBattleEnd(oRole, tBTRes, tExtData)
	...
	if oRoleData:GetScore() <= 0 then
		...
		local nKickTimerID = goTimerMgr:Interval(3, function (nTimerID) 
			goTimerMgr:Clear(nTimerID)
...
function CPVPActivityBase:TeamListBattleInfoReq(tRoleList, fnCallBack)
	...
	goRemoteCall:CallWait("TeamListBattleInfoReq", fnCallBack, gnWorldServerID, goServerMgr:GetGlobalService(gnWorldServerID, 110), 0, tRoleList)
end
...
function CPVPActivityBase:AddRobot(nNum) 
	...
	if gnServerID == gnWorldServerID then 
		...
		for nServerID, v in pairs(tServerMap) do 
			if GF.GetMaxRoleLevelByServer(nServerID) >= nMinLevel then 
	...
	for k = 1, nNum do 
		...
		local nMaxLevel = math.min(math.max(nMinLevel, GF.GetMaxRoleLevelByServer(nTarServer)),
			#ctRoleLevelConf)
	...
	goRemoteCall:Call("PVPActCreateRobotReq", gnWorldServerID, 
...
-- 	local nMoveSpeed = gnRobotMoveSpeed
-- 	assert(nMoveSpeed > 0)

-- 	local nMoveCount = math.random(1, math.ceil(nSetNum/3))
-- 	local tRandList = GF.RandDiffNum(1, nSetNum, nMoveCount)
...
function CPVPActivityBase:RobotBattle()
	...
	for nRobotID, oRobot in pairs(tRobotAttackMap) do 
		if not tRecordList[nRobotID] then 
			local tObserved = oDup:GetAreaObserveds(oRobot:GetAOIID(), gtObjType.eRole)
			local tMatchSet = {}
