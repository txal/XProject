...
function CPVPActivityMgr:Init()
	for k, tConf in pairs(ctPVPActivityConf) do --为每一个活动在对应的服上创建一个管理器
		...
		if GF.GetServiceID() == self:GetActivityServiceID(tConf.nPVPActivityID) then
...
function CPVPActivityMgr:OnRelease()
...
goPVPActivityMgr = goPVPActivityMgr or CPVPActivityMgr:new()
