...
function CPVPActivityMgrBase:Init()
	self.m_nTimer = goTimerMgr:Interval(1, function () self:Tick() end) --为了实时性，1秒一次检查，没其他复杂逻辑，不影响性能
...
function CPVPActivityMgrBase:OnRelease()
	--[[
	...
	if self.m_nReleaseTimer then
		goTimerMgr:Clear(self.m_nReleaseTimer)
		self.m_nReleaseTimer = nil
	end
	]]
...
function CPVPActivityMgrBase:NotifyActivityOpen() 
	goRemoteCall:Call("GPVPActivityOpenNotify", gnWorldServerID, 
...
function CPVPActivityMgrBase:NotifyActivityClose() 
	...
	goRemoteCall:Call("GPVPActivityCloseNotify", gnWorldServerID, 
		goServerMgr:GetGlobalService(gnWorldServerID, 110), 0, self:GetActivityID(), gnServerID)
end

function CPVPActivityMgrBase:ReleaseActivityInst() 
	--这里不调用self:OnRelease()，其内部会回收self.m_nTimer，会导致此管理器后续工作不正常
	print("活动管理器开始回收活动实例")
	for k, oInst in pairs(self.m_tInstMap) do
		oInst:OnRelease()
	end
...
function CPVPActivityMgrBase:OnAcitivityEnd()
	...
	self.m_nReleaseTimer = goTimerMgr:Interval(150, function () self:ReleaseActivityInst() end) --活动实例保留300秒
