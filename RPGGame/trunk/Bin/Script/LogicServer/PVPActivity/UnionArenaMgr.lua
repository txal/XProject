...
function CUnionArenaMgr:OnActivityStart()
    ...
    for nServerID, tConf in pairs(tServerList) do
    	goRemoteCall:CallWait("PackUnionArenaData",fCallback,tConf.nServer,tConf.nID,0)
	end
...
function CUnionArenaMgr:OnActivityStart2(tAllUnionData, tServerUnion)
	...
	for nServerID,tServerUnionData in pairs(tServerUnion) do
		...
		goRemoteCall:Call("MatchUnionArena",nServerID,goServerMgr:GetGlobalService(nServerID, 20),0,tServerMatchUnionData)
	end
	--匹配对应关系,A->B,B->A
