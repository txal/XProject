...
function CTeamMgr:CreateTeam(oPlayer, nGameType, nMapType)
	...
	local nRoomID = goBattleCnt:MakeRoomID()
...
--新手限制
function CTeamMgr:CheckNewbieLimit(oPlayer, nGameType, nMapType)
	if nGameType == self.tGameType.eGVG and nMapType == self.tMapType.eBugHole then
		if oPlayer:GetLevel() <= ctBugHoleEtc[1].nNewbieLevel then
			oPlayer:ScrollMsg(string.format(ctLang[40], ctBugHoleEtc[1].nNewbieLevel+1))	
			return 
		end
	end
	return true
...
function CTeamMgr:CreateTeamReq(oPlayer, nGameType, nMapType)
	if not self:CheckNewbieLimit(oPlayer, nGameType, nMapType) then
		return
	end
	return self:CreateTeam(oPlayer, nGameType, nMapType)
end

function CTeamMgr:_get_player_count_(tRoom)
	local nCount = 0
	for _, v in pairs(tRoom.tPlayerMap) do
		nCount = nCount + 1
	end
	return nCount
end

function CTeamMgr:_get_battle_mgr_(tRoom)
	if tRoom.nGameType == self.tGameType.eGVE then
		if tRoom.nMapType == self.tMapType.eBugStorm then
			return goBattleCnt:GetBattleMgr(gtBattleType.eBugStorm)	
		end
	elseif tRoom.nGameType == self.tGameType.eGVG then
		if tRoom.nMapType == self.tMapType.eBugHole then
			return goBattleCnt:GetBattleMgr(gtBattleType.eBugHole)	
		end
	end
end
...
function CTeamMgr:EnterTeamReq(oPlayer, nRoomID)
	...
	if self:_get_player_count_(tRoom) >= 5 then
		return oPlayer:ScrollMsg(ctLang[3])
	end
	if not self:CheckNewbieLimit(oPlayer, tRoom.nGameType, tRoom.nMapType) then
		return
	end
	self:JoinTeam(oPlayer, nRoomID)
end
...
function CTeamMgr:StartMatchReq(oPlayer)
	...
	local oBattleMgr = self:_get_battle_mgr_(tRoom)
	if oBattleMgr then
