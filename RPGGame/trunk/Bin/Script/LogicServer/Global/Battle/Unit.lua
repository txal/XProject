...
function CUnit:SelectAI()
	...
	if self.m_oBattle:IsPVP() or self.m_oBattle:IsArena() then
		...
		elseif self.m_nObjType == gtObjType.eRole then
...
function CUnit:OnRelease()
	...
	goTimerMgr:Clear(self.m_nLeaveTimer)
...
function CUnit:SetLeave(nExtTime) 
	...
	if nLeaveTime <= 0 then
		...
		self.m_nLeaveTimer = goTimerMgr:Interval(nLeaveTime, function() self:OnLeave() end)
	end
end
...
function CUnit:AddAttr(nType, nVal, tParentAct, sFrom, oSrcUnit, bCrit, bDefense)
	...
	local nMinVal = gtRatioAttr[nType] and -gnMaxInteger or 0
...
function CUnit:OnRoundPrepare(nRound, nMainTime, bReconnect)
    ...
    if self:IsAuto() then
    	if self:IsRole() then
	    	...
	    	self.m_nAutoTimer = goTimerMgr:Interval(nAutoTime, function() self:OnAutoTimer() end)
...
function CUnit:OnDead(tParentAct, oSrcUnit)
	...
	--清空怒气值
	self:AddAttr(gtBAT.eNQ, -gnMaxInteger, nil, "死亡清空SP")
