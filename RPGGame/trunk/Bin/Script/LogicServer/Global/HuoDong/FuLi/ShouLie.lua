...
local function PreProcessConf()
	for nID, tConf in pairs(ctSLPropConf) do
...
local nMaxSLRecordNum = 3 			--记录狩猎奖励数量

function CShouLie:Ctor(nID)
	CHDBase.Ctor(self, nID)     	--继承基类
	self:Init()
end

function CShouLie:Init()
	self.m_nServerScore = 0 					--今日全服羽翎
	self.m_nSubState = CHDBase.tState.eClose 	--活动子状态
	self.m_tPlayerScoreMap = {}					--个人羽翎映射:{[charid]=score, ...}
	self.m_tPlayerUseScoreMap = {}				--个人已使用羽翎映射:{[charid]=score, ...}
	self.m_tUnionScoreMap = {} 					--联盟羽翎映射:{[unionid]=score, ...}
	self.m_tExchangeMap = {} 					--兑换道具映射:{[charid]={[propid]=num, ...}, ...}

	self.m_tBuyPropMap = {} 					--购买道具映射:{[charid]={[propid]=num, ...}
	self.m_tScoreAwardMap = {} 					--全服羽翎奖励映射:{[charid]=1, ...}
	...
	self.m_tTmpUnionScoreMap = {} 	--联盟排行榜临时羽翎映射
	self.m_tTmpPlayerScoreMap = {} 	--玩家排行榜临时羽翎映射
end

function CShouLie:LoadData()
	...
	self.m_tPlayerScoreMap = tData.m_tPlayerScoreMap or self.m_tPlayerScoreMap
	self.m_tPlayerUseScoreMap = tData.m_tPlayerUseScoreMap or self.m_tPlayerUseScoreMap
	self.m_tUnionScoreMap = tData.m_tUnionScoreMap or self.m_tUnionScoreMap 
	self.m_tExchangeMap = tData.m_tExchangeMap or self.m_tExchangeMap
	self.m_tBuyPropMap = tData.m_tBuyPropMap or self.m_tBuyPropMap
	self.m_tScoreAwardMap = tData.m_tScoreAwardMap or self.m_tScoreAwardMap
	self.m_tPlayerRankAwardMap = tData.m_tPlayerRankAwardMap or self.m_tPlayerRankAwardMap 
	self.m_tUnionRankAwardMap = tData.m_tUnionRankAwardMap or self.m_tUnionRankAwardMap 

	--奖励记录
	self.m_tRecordList = tData.m_tRecordList
end

function CShouLie:SaveData()
...
--玩家上线
function CShouLie:Online(oPlayer)
	self:SyncState(oPlayer)
end

--更新状态
function CShouLie:UpdateState()
	CHDBase.UpdateState(self)
	self:UpdateSubState()
end

--进入初始状态
function CShouLie:OnStateInit()
	print("狩猎进入初始状态")
	self:SyncState()
end

--进入活动状态
function CShouLie:OnStateStart()
	print("狩猎进入开始状态")
	--初始化
	self:Init()
	self:SyncState()
...
--进入领奖状态
function CShouLie:OnStateAward()
	print("狩猎进入领奖状态")
...
--进入关闭状态
function CShouLie:OnStateClose()
	print("狩猎进入关闭状态")
	self:SyncState()
end

--子状态开始，结束时间
function CShouLie:GetSubTime()
	local nNow = os.time()
	local tTime = ctSLEtcConf[1].tDayTime[1]
...
--更新子状态
function CShouLie:UpdateSubState()
...
--活动子状态即将开始
function CShouLie:OnSubStateInit()
	print("狩猎进入初始子状态")
	self:UpdateRanking(true)
	self:SyncState()
end
...
--活动子状态开始
function CShouLie:OnSubStateStart()
	print("狩猎进入开始子状态")
	self.m_tBuyPropMap = {}	--限购次数重置
	self.m_nServerScore = 0 --全服羽翎重置
	self:MarkDirty(true)
	self:SyncState()
end

--活动子状态结束
function CShouLie:OnSubStateClose()
	print("狩猎进入关闭子状态")
	self:UpdateRanking(true)
	self:SyncState()
end
...
--同步活动状态
function CShouLie:SyncState(oPlayer)
	...
	if oPlayer then
		...
		tMsg.bPlayerAward = true
		tMsg.bUnionAward = true
		tMsg.bActAward = true
		CmdNet.PBSrv2Clt(oPlayer:GetSession(), "ShouLieStateRet", tMsg)
	--全服广播
	else
		...
		for nSession, oTmpPlayer in pairs(tSessionMap) do
			...
			CmdNet.PBSrv2Clt(oTmpPlayer:GetSession(), "ShouLieStateRet", tMsg)
		end
	end
end

--取已购买道具次数
function CShouLie:GetPropBuyTimes(nCharID, nPropID)
	local tConf = assert(ctSLPropConf[nPropID])
...
--增加道具购买次数
function CShouLie:AddPropBuyTimes(nCharID, nPropID)
	local tBuyMap = self.m_tBuyPropMap[nCharID] or {}
...
--道具列表请求
function CShouLie:PropListReq(oPlayer)
	...
	for nPropID, tConf in pairs(ctSLPropConf) do
		local tItem = {nPropID=nPropID, nBuyTimes=self:GetPropBuyTimes(nCharID, nPropID)}
		table.insert(tList, tItem)
	end
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "ShouLiePropListRet", {tList=tList})
end

--购买道具请求
function CShouLie:BuyPropReq(oPlayer, nPropID)
	local nStartState = CHDBase.tState.eStart 
	if self.m_nState ~= nStartState or self.m_nSubState ~= nStartState then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	...
	oPlayer:SubItem(tPrice[1], tPrice[2], tPrice[3], "狩猎活动购买道具")
	oPlayer:AddItem(gtItemType.eProp, tConf.nPropID, 1, "狩猎活动购买道具")
	self:AddPropBuyTimes(nCharID, nPropID)

	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "ShouLieBuyPropRet", {nPropID=nPropID})
	self:PropListReq(oPlayer)
end

--添加奖励记录
function CShouLie:AddRecord(sRecord)
	table.insert(self.m_tRecordList, 1, sRecord)
	if #self.m_tRecordList > nMaxSLRecordNum then 
...
--使用道具请求
function CShouLie:UsePropReq(oPlayer, nPropID)
	local nStartState = CHDBase.tState.eStart 
	if self.m_nState ~= nStartState or self.m_nSubState ~= nStartState then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nCharID = oPlayer:GetCharID()
	local tConf = assert(ctSLPropConf[nPropID], "道具不存在")
	local nCurrNum = oPlayer:GetItemCount(gtItemType.eProp, nPropID)
	if nCurrNum <= 0 then
		return oPlayer:Tips("道具不足")
	end
	oPlayer:SubItem(gtItemType.eProp, nPropID, 1, "狩猎活动使用道具")
	...
	--伪概率判定
	local tAward = oPlayer.m_oWGL:CheckAward(gtWGLDef.eSL)
	if tAward[1] and tAward[1][1] > 0 then
		for _, tItem in ipairs(tAward) do
			oPlayer:AddItem(tItem[1], tItem[2], tItem[3], "狩猎活动使用道具伪概率")
		...
		for _, tAward in pairs(tConf.tAward) do
			if nRnd >= tAward.nMinW and nRnd <= tAward.nMaxW then
				oPlayer:AddItem(tAward[2], tAward[3], tAward[4], "狩猎活动使用道具")
	...
	local sRecord = string.format(ctLang[26], oPlayer:GetName(), sCont)
	self:AddRecord(sRecord)

	--电视
	local tPropConf = ctPropConf[tItem.nID]
	if tPropConf.nColor >= 5 then
		local sNotice = string.format(ctLang[29], oPlayer:GetName(), tPropConf.nColor, CGuoKu:PropName(tItem.nID))
		goTV:_TVSend(sNotice)
	end

	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "ShouLieUsePropRet", tMsg)
	--小红点
	self:CheckRedPoint(nCharID)
end

--活动奖励信息请求
function CShouLie:AwardInfoReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local tConf = ctSLEtcConf[1]
	local nCharID = oPlayer:GetCharID()
	...
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "ShouLieAwardInfoRet", tMsg)
end

--领取活动奖励请求
function CShouLie:AwardReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local tConf = ctSLEtcConf[1]
	...
	oPlayer:AddItem(tAward[1], tAward[2], tAward[3], "狩猎活动奖励")
	self.m_tScoreAwardMap[nCharID] = 2
	self:MarkDirty(true)
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "ShouLieAwardRet", {nID=tAward[2], nNum=tAward[3]})
...
--兑换列表请求
function CShouLie:ExchangeListReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nCharID = oPlayer:GetCharID()
	...
	for nPropID, tConf in pairs(ctSLExchangeConf) do
		local tItem = {nPropID=nPropID, nExcNum=tExchange[nPropID] or 0}
		table.insert(tMsg.tList, tItem)
	end
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "ShouLieExchangeListRet", tMsg)
end

--兑换物品请求
function CShouLie:ExchangeReq(oPlayer, nPropID)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nCharID = oPlayer:GetCharID()
	local nCurrScore = self.m_tPlayerScoreMap[nCharID] or 0
	local nUseScore = self.m_tPlayerUseScoreMap[nCharID] or 0
	local tConf = assert(ctSLExchangeConf[nPropID])
	if nUseScore+tConf.nScore > nCurrScore then
		return oPlayer:Tips("羽翎不足")
	end
	local tExchange = self.m_tExchangeMap[nCharID] or {}
	...
	oPlayer:AddItem(gtItemType.eProp, nPropID, 1, "狩猎兑换物品")
	self:MarkDirty(true)

	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "ShouLieExchangeRet", {nID=nPropID, nNum=1})
	self:ExchangeListReq(oPlayer)
end

--排行榜更新
function CShouLie:UpdateRanking(bEnd)
...
--取我的排名
function CShouLie:GetPlayerRank(nCharID)
...
--取我的联盟排名
function CShouLie:GetUnionRank(nCharID)
...
--个人排行榜请求
function CShouLie:PlayerRankingReq(oPlayer, nRankNum)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	self:UpdateRanking()
	...
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "ShouLieRankingRet", tMsg)
end

--联盟排行榜请求
function CShouLie:UnionRankingReq(oPlayer, nRankNum)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	self:UpdateRanking()
...
--取玩家领奖状态
function CShouLie:GetPlayerAwardState(nCharID)
	...
	local nMyRank, nMyValue = self:GetPlayerRank(nCharID)
	if nMyRank == 0 or nMyRank > ctSLPLRankingConf[#ctSLPLRankingConf].tRank[1][2] then
		return 0 --未上榜不能领取
	end
	return 1 --可领取
end
...
--取联盟领奖状态
function CShouLie:GetUnionAwardState(nCharID)
	if self.m_nState ~= CHDBase.tState.eAward then
		return 0 --状态不对不可领奖
	end
	local oUnion = goUnionMgr:GetUnionByCharID(nCharID)
	...
	local nMyRank, sMyName, nMyValue = self:GetUnionRank(nCharID)
	if nMyRank == 0 or nMyRank > ctSLUNRankingConf[#ctSLUNRankingConf].tRank[1][2] then
		return 0 --未上榜不能领取
	end
	return 1 --可领取
end
...
--取个人排行奖励信息
function CShouLie:PlayerRankAwardInfoReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nCharID = oPlayer:GetCharID()
	...
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "ShouLieRankAwardInfoRet", tMsg)
end

--取联盟排行奖励信息
function CShouLie:UnionRankAwardInfoReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nCharID = oPlayer:GetCharID()
...
--领取个人排行奖励
function CShouLie:PlayerRankAwardReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eAward then
	...
	self:MarkDirty(true)
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "ShouLieRankAwardRet", {nType=1, tList=tList})
...
--领取公会排行奖励
function CShouLie:UnionRankAwardReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("未到领奖时间:"..self.m_nState)
	end
	local nCharID = oPlayer:GetCharID()
	...
	for k = #ctSLUNRankingConf, 1, -1 do
		local tConf = ctSLUNRankingConf[k]
		local tRank = tConf.tRank[1]
		if nMyRank >= tRank[1] then
			if self:GetOpenTimes() > 1 then  
				local tAward = tConf["tAward1"..nPos]
				for _, tItem in ipairs(tAward) do
					oPlayer:AddItem(tItem[1], tItem[2], tItem[3], "狩猎联盟排行奖励:"..nPos)
	...
	self:MarkDirty(true)
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "ShouLieRankAwardRet", {nType=2, tList=tList})
...
--是否可以领取奖励
function CShouLie:CanGetAward(oPlayer)
	local nCharID = oPlayer:GetCharID()
	...
	if not nAwardState then
		nAwardState = self.m_nServerScore >= ctSLEtcConf[1].nScore and 1 or 0
	end
	return (nPlayerAward==1), (nUnionAward==1), (nAwardState==1)
end

--小红点处理
function CShouLie:CheckRedPoint(nCharID)
