...
local function PreProcessConf()
	for nID, tConf in pairs(ctDDPropConf) do
...
local nMaxDDRecordNum = 3 			--记录点灯奖励数量

function CDianDeng:Ctor(nID)
	CHDBase.Ctor(self, nID)     	--继承基类
	self:Init()
end

function CDianDeng:Init()
	self.m_nServerScore = 0 					--今日全服心愿
	self.m_nSubState = CHDBase.tState.eClose 	--活动子状态
	self.m_tPlayerScoreMap = {}					--个人心愿映射:{[charid]=score, ...}
	self.m_tPlayerUseScoreMap = {}				--个人已使用心愿映射:{[charid]=score, ...}
	self.m_tUnionScoreMap = {} 					--联盟心愿映射:{[unionid]=score, ...}
	self.m_tExchangeMap = {} 					--兑换道具映射:{[charid]={[propid]=num, ...}, ...}

	self.m_tBuyPropMap = {} 					--购买道具映射:{[charid]={[propid]=num, ...}
	self.m_tScoreAwardMap = {} 					--全服心愿奖励映射:{[charid]=1, ...}
	...
	self.m_tTmpUnionScoreMap = {} 	--联盟排行榜临时心愿映射
	self.m_tTmpPlayerScoreMap = {} 	--玩家排行榜临时心愿映射
end

function CDianDeng:LoadData()
...
--玩家上线
function CDianDeng:Online(oPlayer)
	self:SyncState(oPlayer)
end

--更新状态
function CDianDeng:UpdateState()
	CHDBase.UpdateState(self)
	self:UpdateSubState()
end

--进入初始状态
function CDianDeng:OnStateInit()
	print("点灯进入初始状态")
	self:SyncState()
end

--进入活动状态
function CDianDeng:OnStateStart()
	print("点灯进入开始状态")
	--初始化
	self:Init()
	self:SyncState()
...
--进入领奖状态
function CDianDeng:OnStateAward()
	print("点灯进入领奖状态")
...
--进入关闭状态
function CDianDeng:OnStateClose()
	print("点灯进入关闭状态")
	self:SyncState()
end

--子状态开始，结束时间
function CDianDeng:GetSubTime()
	local nNow = os.time()
	local tTime = ctDDEtcConf[1].tDayTime[1]
...
--更新子状态
function CDianDeng:UpdateSubState()
...
--活动子状态即将开始
function CDianDeng:OnSubStateInit()
	print("点灯进入初始子状态")
	self:UpdateRanking(true)
	self:SyncState()
end
...
--活动子状态开始
function CDianDeng:OnSubStateStart()
	print("点灯进入开始子状态")
	self.m_tBuyPropMap = {}	--限购次数重置
	self.m_nServerScore = 0 --全服心愿重置
	self:MarkDirty(true)
	self:SyncState()
end

--活动子状态结束
function CDianDeng:OnSubStateClose()
	print("点灯进入关闭子状态")
	self:UpdateRanking(true)
	self:SyncState()
end
...
--取活动子状态(日开放时间状态)
function CDianDeng:GetSubState()
...
--同步活动状态
function CDianDeng:SyncState(oPlayer)
	...
	if oPlayer then
		...
		CmdNet.PBSrv2Clt(oPlayer:GetSession(), "DianDengStateRet", tMsg)
	--全服广播
	else
		...
		for nSession, oTmpPlayer in pairs(tSessionMap) do
			...
			tMsg.bUnionAward = bUnionAward
			tMsg.bActAward = bActAward
			CmdNet.PBSrv2Clt(oTmpPlayer:GetSession(), "DianDengStateRet", tMsg)
		end
	end
end

--取已购买道具次数
function CDianDeng:GetPropBuyTimes(nCharID, nPropID)
	local tConf = assert(ctDDPropConf[nPropID])
...
--增加道具购买次数
function CDianDeng:AddPropBuyTimes(nCharID, nPropID)
	local tBuyMap = self.m_tBuyPropMap[nCharID] or {}
...
--道具列表请求
function CDianDeng:PropListReq(oPlayer)
	...
	for nPropID, tConf in pairs(ctDDPropConf) do
		local tItem = {nPropID=nPropID, nBuyTimes=self:GetPropBuyTimes(nCharID, nPropID)}
		table.insert(tList, tItem)
	end
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "DianDengPropListRet", {tList=tList})
end

--购买道具请求
function CDianDeng:BuyPropReq(oPlayer, nPropID)
	local nStartState = CHDBase.tState.eStart 
	if self.m_nState ~= nStartState or self.m_nSubState ~= nStartState then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	...
	oPlayer:SubItem(tPrice[1], tPrice[2], tPrice[3], "点灯活动购买道具")
	oPlayer:AddItem(gtItemType.eProp, tConf.nPropID, 1, "点灯活动购买道具")
	self:AddPropBuyTimes(nCharID, nPropID)

	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "DianDengBuyPropRet", {nPropID=nPropID})
	self:PropListReq(oPlayer)
end

--添加奖励记录
function CDianDeng:AddRecord(sRecord)
	table.insert(self.m_tRecordList, 1, sRecord)
	if #self.m_tRecordList > nMaxDDRecordNum then 
...
--使用道具请求
function CDianDeng:UsePropReq(oPlayer, nPropID)
	local nStartState = CHDBase.tState.eStart 
	if self.m_nState ~= nStartState or self.m_nSubState ~= nStartState then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nCharID = oPlayer:GetCharID()
	local tConf = assert(ctDDPropConf[nPropID], "道具不存在")
	local nCurrNum = oPlayer:GetItemCount(gtItemType.eProp, nPropID)
	if nCurrNum <= 0 then
		return oPlayer:Tips("道具不足")
	end
	oPlayer:SubItem(gtItemType.eProp, nPropID, 1, "点灯活动使用道具")
	...
	--伪概率判定
	local tAward = oPlayer.m_oWGL:CheckAward(gtWGLDef.eDD)
	if tAward[1] and tAward[1][1] > 0 then
		for _, tItem in ipairs(tAward) do
			oPlayer:AddItem(tItem[1], tItem[2], tItem[3], "点灯活动使用道具伪概率")
		...
		for _, tAward in pairs(tConf.tAward) do
			if nRnd >= tAward.nMinW and nRnd <= tAward.nMaxW then
				oPlayer:AddItem(tAward[2], tAward[3], tAward[4], "点灯活动使用道具")
	...
	local sRecord = string.format(ctLang[28], oPlayer:GetName(), sCont)
	...
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "DianDengUsePropRet", tMsg)
	--小红点
	self:CheckRedPoint(nCharID)
end

--活动奖励信息请求
function CDianDeng:AwardInfoReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local tConf = ctDDEtcConf[1]
	local nCharID = oPlayer:GetCharID()
	...
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "DianDengAwardInfoRet", tMsg)
end

--领取活动奖励请求
function CDianDeng:AwardReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local tConf = ctDDEtcConf[1]
	...
	oPlayer:AddItem(tAward[1], tAward[2], tAward[3], "点灯活动奖励")
	self.m_tScoreAwardMap[nCharID] = 2
	self:MarkDirty(true)
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "DianDengAwardRet", {nID=tAward[2], nNum=tAward[3]})
...
--兑换列表请求
function CDianDeng:ExchangeListReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nCharID = oPlayer:GetCharID()
	...
	for nPropID, tConf in pairs(ctDDExchangeConf) do
		local tItem = {nPropID=nPropID, nExcNum=tExchange[nPropID] or 0}
		table.insert(tMsg.tList, tItem)
	end
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "DianDengExchangeListRet", tMsg)
end

--兑换物品请求
function CDianDeng:ExchangeReq(oPlayer, nPropID)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nCharID = oPlayer:GetCharID()
	local nCurrScore = self.m_tPlayerScoreMap[nCharID] or 0
	local nUseScore = self.m_tPlayerUseScoreMap[nCharID] or 0
	local tConf = assert(ctDDExchangeConf[nPropID])
	if nUseScore+tConf.nScore > nCurrScore then
		return oPlayer:Tips("心愿不足")
	end
	local tExchange = self.m_tExchangeMap[nCharID] or {}
	...
	oPlayer:AddItem(gtItemType.eProp, nPropID, 1, "点灯兑换物品")
	self:MarkDirty(true)

	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "DianDengExchangeRet", {nID=nPropID, nNum=1})
	self:ExchangeListReq(oPlayer)
end

--排行榜更新
function CDianDeng:UpdateRanking(bEnd)
...
--取我的排名
function CDianDeng:GetPlayerRank(nCharID)
...
--取我的联盟排名
function CDianDeng:GetUnionRank(nCharID)
...
--个人排行榜请求
function CDianDeng:PlayerRankingReq(oPlayer, nRankNum)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	self:UpdateRanking()
	...
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "DianDengRankingRet", tMsg)
end

--联盟排行榜请求
function CDianDeng:UnionRankingReq(oPlayer, nRankNum)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	self:UpdateRanking()
...
--取玩家领奖状态
function CDianDeng:GetPlayerAwardState(nCharID)
	...
	local nMyRank, nMyValue = self:GetPlayerRank(nCharID)
	if nMyRank == 0 or nMyRank > ctDDPLRankingConf[#ctDDPLRankingConf].tRank[1][2] then
		return 0 --未上榜不能领取
	end
	return 1 --可领取
end
...
--取联盟领奖状态
function CDianDeng:GetUnionAwardState(nCharID)
	if self.m_nState ~= CHDBase.tState.eAward then
		return 0 --状态不对不可领奖
	end
	local oUnion = goUnionMgr:GetUnionByCharID(nCharID)
	...
	local nMyRank, sMyName, nMyValue = self:GetUnionRank(nCharID)
	if nMyRank == 0 or nMyRank > ctDDUNRankingConf[#ctDDUNRankingConf].tRank[1][2] then
		return 0 --未上榜不能领取
	end
	return 1 --可领取
end
...
--取个人排行奖励信息
function CDianDeng:PlayerRankAwardInfoReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nCharID = oPlayer:GetCharID()
	...
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "DianDengRankAwardInfoRet", tMsg)
end

--取联盟排行奖励信息
function CDianDeng:UnionRankAwardInfoReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nCharID = oPlayer:GetCharID()
...
--领取个人排行奖励
function CDianDeng:PlayerRankAwardReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eAward then
	...
	self:MarkDirty(true)
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "DianDengRankAwardRet", {nType=1, tList=tList})
...
--领取公会排行奖励
function CDianDeng:UnionRankAwardReq(oPlayer)
	if self.m_nState ~= CHDBase.tState.eAward then
		return oPlayer:Tips("未到领奖时间:"..self.m_nState)
	end
	local nCharID = oPlayer:GetCharID()
	...
	for k = #ctDDUNRankingConf, 1, -1 do
		local tConf = ctDDUNRankingConf[k]
		local tRank = tConf.tRank[1]
		if nMyRank >= tRank[1] then
			if self:GetOpenTimes() > 1 then  
				local tAward = tConf["tAward1"..nPos]
				for _, tItem in ipairs(tAward) do
					oPlayer:AddItem(tItem[1], tItem[2], tItem[3], "点灯联盟排行奖励:"..nPos)
	...
	self:MarkDirty(true)
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "DianDengRankAwardRet", {nType=2, tList=tList})
...
--是否可以领取奖励
function CDianDeng:CanGetAward(oPlayer)
	local nCharID = oPlayer:GetCharID()
	...
	if not nAwardState then
		nAwardState = self.m_nServerScore >= ctDDEtcConf[1].nScore and 1 or 0
	end
	return (nPlayerAward==1), (nUnionAward==1), (nAwardState==1)
end

--小红点处理
function CDianDeng:CheckRedPoint(nCharID)
