...
function CSLRanking:Ctor()
	self.m_tDirtyMap = {} 
	self.m_oRanking = CSkipList:new(_fnDescSort) --{nCharID={sName,nValue}, ...}
end

function CSLRanking:LoadData()
	local tKeys = goDBMgr:GetSSDB("Player"):HKeys(gtDBDef.sSLRankingDB)
	print("加载势力排行榜:", #tKeys)
	for _, sCharID in ipairs(tKeys) do
		local nCharID = tonumber(sCharID)
		local sData = goDBMgr:GetSSDB("Player"):HGet(gtDBDef.sSLRankingDB, nCharID)
...
function CSLRanking:SaveData()
	for nCharID, v in pairs(self.m_tDirtyMap) do
		...
		if tData then
			...
			goDBMgr:GetSSDB("Player"):HSet(gtDBDef.sSLRankingDB, nCharID, cjson.encode(tRealData))
		end
	end
	self.m_tDirtyMap = {}
end

function CSLRanking:OnRelease()
	self:SaveData()
end

--重置清理数据库
function CSLRanking:ResetRanking()
	goDBMgr:GetSSDB("Player"):HClear(gtDBDef.sSLRankingDB)
	self.m_oRanking = CSkipList:new(_fnDescSort)
	self.m_tDirtyMap = {}
end

--设置脏数据
function CSLRanking:MarkDirty(nCharID, bDirty)
	bDirty = bDirty or nil
	self.m_tDirtyMap[nCharID] = bDirty
end

--更新数据
function CSLRanking:Update(oPlayer, nValue)
	...
	if self:GetPlayerRank(nCharID) == 1 then 
	 	goBroadcast:SetBroadcast(47, 1, nCharID)
	end
end

--取某个玩家排名
function CSLRanking:GetPlayerRank(nCharID)
	local nRank = self.m_oRanking:GetRankByKey(nCharID)
	return nRank
end

--排行榜请求
function CSLRanking:SLRankingReq(oPlayer, nRankNum)
	local nCharID = oPlayer:GetCharID()
	...
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "SLRankingRet", tMsg)
	print("CSLRanking:SLRankingReq***", tMsg)
end
