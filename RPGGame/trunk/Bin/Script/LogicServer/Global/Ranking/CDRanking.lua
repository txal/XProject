...
function CCDRanking:Ctor()
	self.m_tDirtyMap = {} 
	self.m_oRanking = CSkipList:new(_fnDescSort) --{nCharID={sName,nValue}, ...}
end

function CCDRanking:LoadData()
	local tKeys = goDBMgr:GetSSDB("Player"):HKeys(gtDBDef.sCDRankingDB)
	print("加载妃子才德排行榜:", #tKeys)
	for _, sCharID in ipairs(tKeys) do
		local nCharID = tonumber(sCharID)
		local sData = goDBMgr:GetSSDB("Player"):HGet(gtDBDef.sCDRankingDB, nCharID)
...
function CCDRanking:SaveData()
	for nCharID, v in pairs(self.m_tDirtyMap) do
		...
		if tData then
			...
			goDBMgr:GetSSDB("Player"):HSet(gtDBDef.sCDRankingDB, nCharID, cjson.encode(tRealData))
		end
	end
	self.m_tDirtyMap = {}
end

function CCDRanking:OnRelease()
	self:SaveData()
end

--重置清理数据库
function CCDRanking:ResetRanking()
	goDBMgr:GetSSDB("Player"):HClear(gtDBDef.sCDRankingDB)
	self.m_oRanking = CSkipList:new(_fnDescSort)
	self.m_tDirtyMap = {}
end

--设置脏数据
function CCDRanking:MarkDirty(nCharID, bDirty)
	bDirty = bDirty or nil
	self.m_tDirtyMap[nCharID] = bDirty
end

--更新数据
function CCDRanking:Update(oPlayer, nValue)
...
--取某个玩家排名
function CCDRanking:GetPlayerRank(nCharID)
	local nRank = self.m_oRanking:GetRankByKey(nCharID)
	return nRank
end

--排行榜请求
function CCDRanking:CDRankingReq(oPlayer, nRankNum)
	local nCharID = oPlayer:GetCharID()
	...
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "CDRankingRet", tMsg)
	print("CCDRanking:CDRankingReq***", tMsg)
end
