...
function CNLRanking:Ctor()
	self.m_tDirtyMap = {} 
	self.m_oRanking = CSkipList:new(_fnDescSort) --{nCharID={sName,nValue}, ...}
end

function CNLRanking:LoadData()
	local tKeys = goDBMgr:GetSSDB("Player"):HKeys(gtDBDef.sNLRankingDB)
	print("加载妃子总能力排行榜:", #tKeys)
	for _, sCharID in ipairs(tKeys) do
		local nCharID = tonumber(sCharID)
		local sData = goDBMgr:GetSSDB("Player"):HGet(gtDBDef.sNLRankingDB, nCharID)
...
function CNLRanking:SaveData()
	for nCharID, v in pairs(self.m_tDirtyMap) do
		...
		if tData then
			...
			goDBMgr:GetSSDB("Player"):HSet(gtDBDef.sNLRankingDB, nCharID, cjson.encode(tRealData))
		end
	end
	self.m_tDirtyMap = {}
end

function CNLRanking:OnRelease()
	self:SaveData()
end

--重置清理数据库
function CNLRanking:ResetRanking()
	goDBMgr:GetSSDB("Player"):HClear(gtDBDef.sNLRankingDB)
	self.m_oRanking = CSkipList:new(_fnDescSort)
	self.m_tDirtyMap = {}
end

--设置脏数据
function CNLRanking:MarkDirty(nCharID, bDirty)
	bDirty = bDirty or nil
	self.m_tDirtyMap[nCharID] = bDirty
end

--更新数据
function CNLRanking:Update(oPlayer, nValue)
	...
	if self:GetPlayerRank(nCharID) == 1 then 
	 	goBroadcast:SetBroadcast(38, 1, nCharID)
	end

	--活动
    goHDMgr:GetHuoDong(gtHDDef.eTimeAward):UpdateVal(nCharID, gtTAType.eNL, nDiffVal)
end

--取某个玩家排名
function CNLRanking:GetPlayerRank(nCharID)
	local nRank = self.m_oRanking:GetRankByKey(nCharID)
	return nRank
end

--排行榜请求
function CNLRanking:NLRankingReq(oPlayer, nRankNum)
	local nCharID = oPlayer:GetCharID()
	...
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "NLRankingRet", tMsg)
	print("CNLRanking:NLRankingReq***", tMsg.nMyRank, tMsg.sMyName, tMsg.nMyValue)
end
