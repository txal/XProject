...
function CHZRanking:Ctor()
	self.m_tDirtyMap = {} 
	self.m_oRanking = CSkipList:new(_fnDescSort) --{nCharID={sName,nValue}, ...}
end

function CHZRanking:LoadData()
	local tKeys = goDBMgr:GetSSDB("Player"):HKeys(gtDBDef.sHZRankingDB)
	print("加载皇子属性加成排行榜:", #tKeys)
	for _, sCharID in ipairs(tKeys) do
		local nCharID = tonumber(sCharID)
		local sData = goDBMgr:GetSSDB("Player"):HGet(gtDBDef.sHZRankingDB, nCharID)
...
function CHZRanking:SaveData()
	for nCharID, v in pairs(self.m_tDirtyMap) do
		...
		if tData then
			...
			goDBMgr:GetSSDB("Player"):HSet(gtDBDef.sHZRankingDB, nCharID, cjson.encode(tRealData))
		end
	end
	self.m_tDirtyMap = {}
end

function CHZRanking:OnRelease()
	self:SaveData()
end

--重置清理数据库
function CHZRanking:ResetRanking()
	goDBMgr:GetSSDB("Player"):HClear(gtDBDef.sHZRankingDB)
	self.m_oRanking = CSkipList:new(_fnDescSort)
	self.m_tDirtyMap = {}
end

--设置脏数据
function CHZRanking:MarkDirty(nCharID, bDirty)
	bDirty = bDirty or nil
	self.m_tDirtyMap[nCharID] = bDirty
end

--更新数据
function CHZRanking:Update(oPlayer, nValue)
	...
	if self:GetPlayerRank(nCharID) == 1 then 
	 	goBroadcast:SetBroadcast(48, 1, nCharID)
	end
end

--取某个玩家排名
function CHZRanking:GetPlayerRank(nCharID)
	local nRank = self.m_oRanking:GetRankByKey(nCharID)
	return nRank
end

--排行榜请求
function CHZRanking:HZRankingReq(oPlayer, nRankNum)
	local nCharID = oPlayer:GetCharID()
	...
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "HZRankingRet", tMsg)
	print("CHZRanking:HZRankingReq***", tMsg)
end
