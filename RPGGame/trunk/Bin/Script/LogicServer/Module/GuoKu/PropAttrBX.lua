...
function CPropAttrBx:Use(nNum)
	...
	for k = 1, nNum do
		...
		if tConf.nSubType == gtCurrType.eRandGrow then
			local tMCList = oPlayer.m_oMingChen:RandObj(1)
			if #tMCList <= 0 then
				return oPlayer:Tips("没有可随机的知己")
			end
	...
	for _, tItem in pairs(tAwardMap) do
		...
		if tItem.nObjType == gtItemType.eMingChen then
			local oMC = oPlayer.m_oMingChen:GetObj(tItem.nObjID)
	...
	CmdNet.PBSrv2Clt(oPlayer:GetSession(), "GuoKuUseAttrBoxRet", {nPropID=tConf.nID, nPropNum=nNum, tAwardList=tAwardList})
	return true
end