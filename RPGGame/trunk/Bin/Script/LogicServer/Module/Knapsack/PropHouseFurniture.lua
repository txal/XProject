...
function CPropHouseFurniture:Use(nParam1)
	...
	goRemoteCall:CallWait("HouseFurnitureIsLock", fnCheckCallback, gnWorldServerID, nServiceID, 0, nRoleID,nFurnitureID)
end
...
function CPropHouseFurniture:TrueUse()
	...
	local nRoleID = oRole:GetID()
	local nFurnitureID = self:GetPropConf().eParam()
	goRemoteCall:CallWait("HouseUnLockFurniture", fnCallback, gnWorldServerID, nServiceID, 0, nRoleID,nFurnitureID)
end