...
function CPropArenaTicket:Use(nParam1)
	...
	local fnAddPreCheckCallback = function (bRet)
		...
		local fnAddArenaChallCallback = function (bRet)
			...
			--do something
			--限时奖励
	        goRemoteCall:Call("OnTATZSReq", nServerID, goServerMgr:GetGlobalService(nServerID,20), 0, oRole:GetID(), nUseNum)
		end	

		goRemoteCall:CallWait("ArenaAddChallReq", fnAddArenaChallCallback, nServerID, 
			goServerMgr:GetGlobalService(nServerID, 20), oRole:GetSession(), oRole:GetID(), nUseNum)
	end

	goRemoteCall:CallWait("ArenaAddChallPreCheckReq", fnAddPreCheckCallback, nServerID, 
