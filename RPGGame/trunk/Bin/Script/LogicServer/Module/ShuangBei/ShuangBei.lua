...
function CShuangBei:AddShuangBei(nShuangBei)
    if 0 <= nShuangBei then  --添加时要判断是不是达到上限,大于0说明是存储的，消耗的nShuangBei只有小于0
        if self.m_nUnuseShuangbei < ctShuangBeiConf[1].nMaxSave then
            self.m_nUnuseShuangbei = math.max(0, math.min(gnMaxInteger, self.m_nUnuseShuangbei+nShuangBei))
        end
        self:MarkDirty(true)
        return self.m_nUnuseShuangbei
    else
        self.m_nUseShuangbei = math.max(0, math.min(gnMaxInteger, self.m_nUseShuangbei+nShuangBei))
        self:MarkDirty(true)
