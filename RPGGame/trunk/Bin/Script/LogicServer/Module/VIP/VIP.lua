...
function CVIP:OnRechargeSuccess(nID, nMoney, nYuanBao, nBYuanBao, nTime)
	self.m_nTotalMoney = math.min(gnMaxInteger, math.max(0, self.m_nTotalMoney+nMoney))
	...
	if tConf.nType == gtRechargeType.eCommon then		--普通充值
		...
		for _, tService in ipairs(tGlobalServiceList) do 
			goRemoteCall:Call("OnRoleRechargeSuccess", tService.nServer, tService.nID, 0, self.m_oRole:GetID(), nID, nMoney, nYuanBao, nBYuanBao, nTime)
		end
...
function CVIP:SyncFirstRecharge()
	...
	local tConf = goBackstage:GetConf(self.m_oRole:GetServer(), gtBackstageType.eFristRecharge) --后台配置
	...
	local function _fnCheckSysTalk(tItem)
		if tItem[2] == gtItemType.ePet then
			...
			if tPetConf and tTalkConf then
				GF.SendSystemTalk("系统", string.format(tTalkConf.sContent, self.m_oRole:GetName(), tPetConf.sName, tPetConf.sName))
			end
		end
	end
...
function CVIP:AddVIPExp(nVal)
	self.m_nTotalMoney = math.min(gnMaxInteger, math.max(0, self.m_nTotalMoney+nVal))
	self:MarkDirty(true)
	self:CheckVIPUpgrade()
	return self.m_nTotalMoney
end
