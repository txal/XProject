function CltPBProc.DailyActivityReq(nCmd, nServer, nService, nSession, tData)
...
function Srv2Srv.CheckJoinDailyActReq(nSrcServer, nSrcService, nTarSession, nRoleID, nActID)
...
function Srv2Srv.ReturnTeamJoinPVEActCheckReq(nSrcServer, nSrcService, nTarSession, nRoleID)
	local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
function CltPBProc.ShareGameStatusReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
...
function CltPBProc.ShareGameSuccessReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
...
function CltPBProc.ShareGameRewardReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
...
function CltPBProc.ClickCanJoinActButtonReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
