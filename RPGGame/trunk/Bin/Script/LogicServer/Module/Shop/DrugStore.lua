...
function CDrugStore:Ctor(oModule)
...
function CDrugStore:LoadData(tData)
	print("CDrugStore:LoadData( tData )",tData)
	self.m_tAllreadyBuy = tData.m_tAllreadyBuy 
	self.m_tFirstBuyTime = tData.m_tFirstBuyTime 
end

function CDrugStore:SaveData()
	local tData = {}
...
function CDrugStore:MarkDirty(bMark)
	self.m_oModule:MarkDirty(bMark)
end

--记录限购物品 已购买个数
function CDrugStore:RecordLimItemBuyNum(tItem, nNum)
...
--重置限购表
function CDrugStore:ResetRecordLimBuyNum()
...
--物品列表请求
function CDrugStore:ItemListReq(nShopType)
