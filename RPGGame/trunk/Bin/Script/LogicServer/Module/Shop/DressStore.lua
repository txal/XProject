...
function CDressStore:Ctor(oModule)
	--print("CDressStore:Ctor(oModule)")
...
function CDressStore:LoadData(tData)
	print("CDressStore:LoadData( tData )",tData)
	self.m_tAllreadyBuy = tData.m_tAllreadyBuy 
	self.m_tFirstBuyTime = tData.m_tFirstBuyTime 
end

function CDressStore:SaveData()
	local tData = {}
...
function CDressStore:MarkDirty(bMark)
	self.m_oModule:MarkDirty(bMark)
end

--记录限购物品 已购买个数
function CDressStore:RecordLimItemBuyNum(tItem, nNum)
	...
	self:MarkDirty(true)
	--print("self.m_tAllreadyBuy",self.m_tAllreadyBuy)
end

--重置限购表
function CDressStore:ResetRecordLimBuyNum()
...
--获取限购物品 已购买个数
function CDressStore:GetLimItemBuyNum(tItem)
	self:ResetRecordLimBuyNum()  --获取购买个数之前 要判断是否重置 
	return self.m_tAllreadyBuy[tItem.nLimBuyType][tItem.nID] or 0
end

--购买请求
function CDressStore:BuyReq(nShopType, nID, nNum)
	if nNum >200 then
	...
	--print("ShopBuyRet",tMsg)
end

--物品列表请求
function CDressStore:ItemListReq(nShopType)
	local _ctDrugConf = self.m_oModule:GetItemByShopTypeAndId(nShopType)
	local tMsg = {}
	tMsg.nShopType = nShopType
	tMsg.tList = {}
	for nID,tConf in pairs(_ctDrugConf) do
		local nRemainNum = -1
