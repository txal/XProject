...
function CLianYin:DoMarry(oSrcPlayer, oSrcHZ, nTarHZID, nTarCharID)
	if (nTarCharID or 0) > 0 then --不在线
		...
		CmdNet.PBSrv2Clt(oSrcPlayer:GetSession(), "LYSuccessRet", {tList={tInfo}})
		--小红点
		...
		CmdNet.PBSrv2Clt(self.m_oPlayer:GetSession(), "LYSuccessRet", {tList={tTarInfo}})
		...
		CmdNet.PBSrv2Clt(oSrcPlayer:GetSession(), "LYSuccessRet", {tList={tSrcInfo}})
...
function CLianYin:CheckOfflineMarry()
	...
	if #tList > 0 then
		CmdNet.PBSrv2Clt(self.m_oPlayer:GetSession(), "LYSuccessRet", {tList=tList})
	end
end
...
function CLianYin:SyncLYList()
	...
	CmdNet.PBSrv2Clt(self.m_oPlayer:GetSession(), "LYListRet", {tList=tList})
end
...
--符合条件萌宠列表
 function CLianYin:MatchHZReq(nTarCharID, nTarHZID)
	...
	CmdNet.PBSrv2Clt(self.m_oPlayer:GetSession(), "LYHZMatchListRet", {tList=tList})
end
