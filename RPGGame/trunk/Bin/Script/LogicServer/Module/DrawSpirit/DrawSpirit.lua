...
function CDrawSpirit:AddSpirit(nNum, sReason, bNotSync)
    self.m_nSpirit = math.min(math.max(self.m_nSpirit + nNum, 0), gnMaxInteger)
    self:MarkDirty(true)
...
function CDrawSpirit:AddMagicPill(nNum, sReason, bNotSync)
    self.m_nMagicPill = math.min(math.max(self.m_nMagicPill + nNum, 0), gnMaxInteger)
    self:MarkDirty(true)
    if not bNotSync then
        if sReason and sReason == "摄魂" then 
...
function CDrawSpirit:AddCrystal(nNum, sReason, bNotSync)
    self.m_nCrystal = math.min(math.max(self.m_nCrystal + nNum, 0), gnMaxInteger)
    self:MarkDirty(true)
    if not bNotSync then
        if sReason and sReason == "摄魂" then 
    ...
    if tRewardConf and tRewardConf.nRatio > 0 then 
        ...
        if tRewardConf.nRatio >= 20 and nSpiritNum > 0 and not oRole:IsRobot() then 
            ...
            if tTalkConf then 
                ...
                GF.SendHearsayMsg(sBroadcastContent)
            end
        end
    end
