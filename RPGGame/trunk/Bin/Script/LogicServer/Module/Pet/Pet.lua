...
function CPet:SendPetMail(nPetID)
	...
	GF.SendMail(self.m_oRole:GetServer(), "宠物栏已满", "宠物栏已满，请及时领取邮件", tItemList, self.m_oRole:GetID())
...
function CPet:RenamedReq(nId, nPos, sNewName)
	...
	local sTmpName = GF.FilterSpecChars(sNewName)
	...
	if ctPetInfoConf[nId].sName == sNewName then
		...
		GF.HasBadWord(sNewName, fnCallback)
	end
end
...
function CPet:SynthesisReq(nZID, nFID, nZPos, nFPos, nBDType, bFlag, bYuanBaoBuy)
	...
	if bYuanBaoBuy then
		...
		goRemoteCall:CallWait("GetChamberCoreSkillReq",fnGetPetSkillCallback, nServerID, nTarService, 0)
	else
...
function CPet:FastLearnSkillReq(nPropID, nPos, nPrice)
	...
	goRemoteCall:Call("BuyShopPriceReq", nServerID, nGlobalLogic, 0, self.m_oRole:GetID(),101,nPropID, 1, "PetSomeMethodSkillReq")
end
...
function CPet:LianGuReq(nId, nPos, nProId, nType,nNum)
    ...
    goRemoteCall:Call("OnTAZZDReq", nServer, goServerMgr:GetGlobalService(nServer,20), 0, self.m_oRole:GetID(), nNum)
end
...
function CPet:AddExp(nExp, sReason, bNotSync)
    ...
    GF.SendItemTalk(self.m_oRole, "getpetexp", {tPetObj.nId, ctPropConf:GetCurrProp(gtCurrType.ePetExp), nIntExp}, bNotSync)
    --日志
...
function CPet:PetScoreChange()
	...
	if tMaxPet then
		...
		goRemoteCall:Call("PetScoreChangeReq", nServerID, nServiceID, 0, self.m_oRole:GetID(), tMaxPet.nPos, tMaxPet.sName, tMaxPet.jnpf)
	end
