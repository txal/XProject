...
function CPartnerObj:CalcFightAbility() 
	...
	for nAttrID, nAttrVal in pairs(self.m_tAttr) do 
		nAttrScore = nAttrScore + GF.CalcAttrScore(nAttrID, nAttrVal)
	end
...
function CPartnerObj:OnIntimacyChanged(nOldVal, nChangeVal) 
	...
	if nChangeVal ~= 0 then
	    ...
	    goRemoteCall:Call("OnCBQMDReq", oRole:GetServer(), goServerMgr:GetGlobalService(oRole:GetServer(), 20), 0, oRole:GetID(), nChangeVal) 
	end
end
