...
function CPartner:AddPartnerStoneNum(nStoneID, nNum)
    ...
    self.m_tMaterialStone[nStoneID] = math.max(0, math.min(self.m_tMaterialStone[nStoneID] + nNum, gnMaxInteger))
...
function CPartner:AddMaterialCollectCount(nCount)
	self.m_nMaterialCollectCount = math.max(0, math.min(self.m_nMaterialCollectCount + nCount, gnMaxInteger))
...
function CPartner:StarLevelUpReq(nPartnerID)
	...
	if nNewStar and sPasSkillName and ctHearsayConf["partnerstarup"] then
		GF.SendHearsayMsg(string.format(ctHearsayConf["partnerstarup"].sHearsay, self.m_oRole:GetName(), oPartner:GetName(), nNewStar, sPasSkillName))
	end
	self.m_oRole:UpdateActGTPartnerPowerSum()
end
