...
function CMonthCard:LoadData(tData)
	if tData then
		local tCardConf = goBackstage:GetConf(self.m_oRole:GetServer(), gtBackstageType.eCards)
		self.m_tCardMap = {}
...
function CMonthCard:OnRechargeSuccess(nID)
	...
	local function _fnCheckSysTalk(nID)
		...
		GF.SendSystemTalk("系统", string.format(tTalkConf.sContent, self.m_oRole:GetName(), tRechConf.sName))
	end
	_fnCheckSysTalk(nID)
end
