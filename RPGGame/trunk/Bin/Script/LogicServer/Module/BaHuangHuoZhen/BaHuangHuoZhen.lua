...
function CBaHuangHuoZhen:OnRelease()
...
function CBaHuangHuoZhen:CheckRewardState()
	if self:GetCompleteRewardState() == gtReeardState.eComplete then
		...
		GF.SendMail(self.m_oRole:GetServer(), "八荒火阵任务获得", "八荒火阵任务获得,请查收", tItemList, self.m_oRole:GetID())
	end
end
...
function CBaHuangHuoZhen:SendTaskInfoHandle(tMsg, nType, fnCallBack)
	...
	-- end
	-- goRemoteCall:CallWait("GetMultipleItemBasePriceReq", fnGetItemBasePriceCallBack, nServerID, nGlobalLogic, 0, self.m_oRole:GetID(), tItemList)
...
function CBaHuangHuoZhen:UnionHelp(oBox)
	...
	local fnGetUnionCallBack = function (nUnionID)
		...
		goRemoteCall:Call("SendUnionTalkReq", gnWorldServerID, nGlobalService, 0, self.m_oRole:GetID(), sContent)		
	end
	goRemoteCall:CallWait("GetPlayerUnionReq", fnGetUnionCallBack, nServerID, nGlobalLogic, 0, self.m_oRole:GetID())
...
function CBaHuangHuoZhen:HelpPlayerBoxListReq(nHelpRoleID,  nBoxID)
	...
	local fnGetUnionCallBack = function (nRoleUnionID, nTarRoleUnioiD)
		...
		goRemoteCall:Call("GetHelpRoleDataReq", nServerID, nServiceID, 0,self.m_oRole:GetID(), nHelpRoleID,nBoxID)
	end
	goRemoteCall:CallWait("GetPlayerUnionReq", fnGetUnionCallBack, nServerID, nGlobalLogic, 0, self.m_oRole:GetID(), nHelpRoleID)
end
...
function CBaHuangHuoZhen:HelpPackingBoxReq(nHelpRoleID, nBoxID, tItemList)
	...
	local fnGetUnionCallBack = function (nRoleUnionID, nTarRoleUnioiD)
		...
		goRemoteCall:Call("HelpPackingBoxReq", nServerID, nGlobalLogic, 0, self.m_oRole:GetID(), nHelpRoleID, nBoxID)
	end
	goRemoteCall:CallWait("GetPlayerUnionReq", fnGetUnionCallBack, nServerID, nGlobalLogic, 0, self.m_oRole:GetID(), nHelpRoleID)
end
...
function CBaHuangHuoZhen:GetHelpRoleData(nRoleID, nHeleRoleID, nBoxID)
	...
	local fnCallBack = function ()
		...
		goRemoteCall:Call("PushHelpRoleBoxReq", nServerID, nGlobalLogic, 0, self.m_oRole:GetID(), nHeleRoleID,tMsg, sTips)
	end
	 self:PracticeExpHandle(fnCallBack)
end
...
function CBaHuangHuoZhen:HelpPackingBoxCheckHandle(nHeleRoleID,nBoxID, tData)
	...
	local nGlobalService = goServerMgr:GetGlobalService(gnWorldServerID, 110)
	goRemoteCall:Call("SendUnionTalkReq", gnWorldServerID, nGlobalService, 0, tData.nRoleID, sContent)
	...
	--GF.SendMail(tData.nServerID, sTitle, sContent, {}, tData.nRoleID)
	GF.SendMail(self.m_oRole:GetServer(), sTitle, sContent, {}, self.m_oRole:GetID())
