...
function CTeamMgr:OnRelease()
	self:GetMatchMgr():OnRelease()
...
function CTeamMgr:SyncTeamCache(nRoleID, bAll)
	...
	if oTeam then
		...
		goRemoteCall:Call("RoleUpdateReq", oRole:GetStayServer(), oRole:GetLogic(), 
...
function CTeamMgr:UpdateTeamFollow(oTeam, bTeamDismiss)
	...
	if bTeamDismiss then
		for nIndex, tRole in ipairs(tRoleList) do
			...
			goRemoteCall:Call("SetFollowReq", oTmpRole:GetStayServer(), oTmpRole:GetLogic(), oTmpRole:GetSession(), oTmpRole:GetMixObjID(), {})
		end
		return
	end
	...
	goRemoteCall:Call("SetFollowReq", oLeaderRole:GetStayServer(), oLeaderRole:GetLogic(), oLeaderRole:GetSession(), oLeaderRole:GetMixObjID(), tFollowList)
end
...
function CTeamMgr:OnRoleQuit(nRoleID)
	...
	--解除自己跟随
	local oRole = goGPlayerMgr:GetRoleByID(nRoleID)
	goRemoteCall:Call("SetFollowReq", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetMixObjID(), {})
	--更新队伍跟随
...
function CTeamMgr:LeaderActivityCheck(nRoleID, nLastPacketTime)
	...
	--通知逻辑服
	goRemoteCall:Call("TeamLeaderActivityRet", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), nRoleID, nInactivityTime)
end
