...
function CTeam:OnRelease()
	...
	goTimerMgr:Clear(self.m_nApplyLeaderTimer)
end
...
function CTeam:CheckTeamActive() 
	for k, tRoleData in pairs(self.m_tRoleList) do 
		if not GF.IsRobot(tRoleData.nRoleID) then 
...
-- 		if GF.IsRobot(tRoleData.nRoleID) then
-- 			local oRole = goGPlayerMgr:GetRoleByID(tRoleData.nRoleID)
-- 			if oRole then 
-- 				self:QuitReq(oRole) 
	...
	if nRoleID then 
		for nIndex, tRole in ipairs(self.m_tRoleList) do
			if tRole.nRoleID == nRoleID then 
				...
				goRemoteCall:Call("RoleUpdateReq", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), 
...
function CTeam:OnLeaderChange(nOldLeaderRoleID)
	...
	if tTeamLeader then 
		...
		if tSessionList and #tSessionList > 0 then 
			CmdNet.PBBroadcastExter("TipsMsgRet", tSessionList, {sCont = sTeamContent})
		end
...
function CTeam:OnReturnTeam(tRole, tLeaderTarget)
	...
	if tLeaderTarget then
		...
		goRemoteCall:Call("WSwitchLogicReq", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), tLeaderTarget)
		...
		local tSessionList = self:GetSessionList()
		if tSessionList and #tSessionList > 0 then 
			CmdNet.PBBroadcastExter("TipsMsgRet", tSessionList, {sCont = sContent})
		end

		goRemoteCall:CallWait("QueryRoleDupInfoReq", function(nDupMixID, nLine, nPosX, nPosY)
			...
			goRemoteCall:Call("WSwitchLogicReq", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), tTarget)
			self.m_oTeamMgr:UpdateTeamFollow(self)
...
function CTeam:RemoveTeam()
	...
	if tSessionList and #tSessionList > 0 then 
		CmdNet.PBBroadcastExter("TipsMsgRet", tSessionList, {sCont = sCont})
	end
end
	...
	if nIndex == 1 then
		...
		for nIndex, tTmpRole in ipairs(self.m_tRoleList) do
			if (not tTmpRole.bLeave) and tTmpRole.nRoleID ~= nRoleID 
				and not GF.IsRobot(tTmpRole.nRoleID) then
...
function CTeam:OnEnterScene(oRole)
	...
	if not self:IsLeader(oRole:GetID()) then
		...
		-- local tQuitList = {}
		-- local tLeaderDupConf = oLeader:GetDupConf()
		-- local bLeaderPVEActDup = GF.IsPVEActDup(tLeaderDupConf.nID)
		-- for k = 2, self:GetMembers() do 
		-- 	local tRole = self.m_tRoleList[k]
		-- 	if GF.IsRobot(tRole.nRoleID) then 
		...
		-- 			local bRobotPVEActDup = GF.IsPVEActDup(tRobotLeaderDupConf.nID)
	...
	goRemoteCall:CallWait("QueryRoleDupInfoReq", _fnCallback, oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetID())
end
...
function CTeam:ReturnTeamReq(oRole, tLeaderTarget)
	...
	if (tLeaderDupConf.nBattleType == 200) or (tLeaderDupConf.nBattleType == 201) 
		...
		goRemoteCall:CallWait("ReturnTeamJoinPVEActCheckReq",fnReturnTeamCallback, 
	...
	if bLeaderNormalAct and nLeaderDailyActID > 0 then 
		goRemoteCall:CallWait("CheckJoinDailyActReq",fnReturnTeamCallback, oRole:GetStayServer(), 
...
function CTeam:UnionMemberListReq(oRole)
	...
	goRemoteCall:CallWait("UnionMemberListReq", function(tMemberList)
...
function CTeam:OnBattleEnd(oRole, tBTRes)
	...
	for k = 2, #self.m_tRoleList do 
		local tRole = self.m_tRoleList[k]
		if GF.IsRobot(tRole.nRoleID) and tRole.bLeave then 
			local oRobot = goGPlayerMgr:GetRoleByID(tRole.nRoleID)
			assert(oRobot)
...
function CTeam:ApplyLeaderReq(oRole)
	...
	goClientCall:CallWait("ConfirmRet", function(tData)
		...
		if tData.nSelIdx == 2 then
			...
			goTimerMgr:Clear(self.m_nApplyLeaderTimer)
			self.m_nApplyLeaderTimer = goTimerMgr:Interval(30, function() self:OnApplyLeaderTimer(oRole:GetID()) end)
...
function CTeam:SyncTeam(oRole)
	...
	if not oLeader:IsReleased() then
		goRemoteCall:CallWait("WGlobalTeamPartnerReq", function(tPartnerInfo)
