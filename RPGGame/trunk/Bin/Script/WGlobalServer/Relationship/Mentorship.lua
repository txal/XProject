...
function CMentorshipMgr:Init()
    self:LoadData()
    self.m_nTimer =  goTimerMgr:Interval(20, function () self:OnTimer() end)
end

function CMentorshipMgr:OnRelease()
...
function CMentorshipMgr:MentorshipCheck(nMasterID, nApprenticeID)
    ...
    if GF.IsRobot(nMasterID) or GF.IsRobot(nApprenticeID) then 
...
function CMentorshipMgr:TaskBattleReq(oRole)
    ...
    goRemoteCall:Call("MentorshipTaskBattleReq", oRole:GetStayServer(), oRole:GetLogic(), 
        oRole:GetSession(), nRoleID, {nTaskID = nTaskID, nTimeStamp = os.time(), nSrcService = GF.GetServiceID()})
end
...
function CMentorshipMgr:SyncLogicCache(nRoleID, nSrcServer, nSrcService, nTarSession) 
    ...
    goRemoteCall:Call("RoleMentorshipUpdateReq", nSrcServer, nSrcService, nTarSession, nRoleID, tData)
end
...
goMentorshipMgr = goMentorshipMgr or CMentorshipMgr:new()
