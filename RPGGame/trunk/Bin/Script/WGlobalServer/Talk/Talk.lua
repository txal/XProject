...
function CTalk:_SendTalkMsg(tTalkIdent, nChannel, sCont, tSessionList)
	...
	if tSessionList then
		CmdNet.PBBroadcastExter("TalkRet", tSessionList, {tList={tTalk}})
	else
		CmdNet.PBSrv2All("TalkRet", {tList={tTalk}}) 
	end
...
function CTalk:SendUnionMsg(oRole, sCont)
	...
	goRemoteCall:Call("UnionTalkReq", nServer, nService, 0, oRole:GetID(), tTalk)
...
function CTalk:SendCurrMsg(oRole, sCont)
	goRemoteCall:CallWait("DupRoleViewListReq", function(tRoleList)
