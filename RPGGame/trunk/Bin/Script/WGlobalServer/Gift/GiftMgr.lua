...
function CGiftMgr:Init()
	self:RegAutoSave()
	self.m_nMinTimer = goTimerMgr:Interval(60, function() self:OnMinTimer() end)
	self:LoadData()
end
...
function CGiftMgr:CheckSendType(oRole, nTarRoleID, tItemList, nType)
	...
	if nType == gtSendPropType.eTreasure then
		...
		goRemoteCall:CallWait("GetTotalMoneyReq", fnGetTotalMoneyReqCallBack,oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetID())
...
function CGiftMgr:SendPropHandles(oRole, nTarRoleID, tList, nType)
	...
	local fnGetAccountStateCallBack = function(nAccountState)
		...
		local fnPropDataCallback = function(bRet, tPropDataList)
			...
			if nType == gtSendPropType.eFlower then 
				...
				GF.SendMail(oTarRole:GetServer(), sMailTitle, sMailContent, tPropDataList, oTarRole:GetID())
	...
	goRemoteCall:CallWait("AccountValueReq", fnGetAccountStateCallBack, oRole:GetServer(), goServerMgr:GetLoginService(oRole:GetServer()), oRole:GetSession(), oRole:GetAccountID(), "m_nAccountState")
end
...
goCGiftMgr = goCGiftMgr or CGiftMgr:new()