...
function Srv2Srv.PVPActCreateRobotReq(nSrcServer, nSrcService, nTarSession, tParamList)
...
function Srv2Srv.AddRole2RobotPoolReq(nSrcServer, nSrcService, nTarSession, nRoleID)
...
function Srv2Srv.RemoveRoleFromRobotPoolReq(nSrcServer, nSrcService, nTarSession, nRoleID)
    assert(nRoleID and nRoleID > 0)
    -- print(string.format(">>>>> 通知删除机器人玩家池(%d) <<<<<<", nRoleID))
    goGRobotMgr:RemoveFromMatchPool(nRoleID)
end

function Srv2Srv.GuaJiCreateRobotReq(nSrcServer, nSrcService, nTarSession, tParamList)
    if not tParamList or #tParamList <= 0 then 
        return 
    end
