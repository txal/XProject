...
function CCBBase:LoadData()
	...
	-- self.m_tAwardState = tData.m_tAwardState or self.m_tAwardState
...
function CCBBase:RankingReq(oRole, nRankNum)
	...
	for k=1, nRankNum do 
		...
		if tRank then
			table.insert(tRanking, {nRank=k, sName=goGPlayerMgr:GetRoleByID(tRank[1]):GetName(), nValue=tRank[2], sExtName=""})
		else
			break
		end
	end
