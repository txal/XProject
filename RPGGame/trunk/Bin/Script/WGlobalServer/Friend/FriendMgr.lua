...
function CFriendMgr:FriendApplyReq(oRole, nTarRoleID, sMessage)
	...
	local function fnCallBack(nTotalMoney)
		...
		if GF.HasBadWord(sMessage) then
		...
		oRole:SendMsg("FriendApplySuccessRet", {nTarRoleID=nTarRoleID})
		goRemoteCall:Call("OnAddFriend", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetID(), {})
	end
    goRemoteCall:CallWait("GetTotalMoneyReq", fnCallBack, oRole:GetStayServer(), oRole:GetLogic(), 0, oRole:GetID())
end
...
function CFriendMgr:AddFriendReq(oRole, nTarRoleID)
	...
	goRemoteCall:Call("OnBecomeFriend", oRole:GetStayServer(), oRole:GetLogic(), oRole:GetSession(), oRole:GetID(), {nFriendNum=nFriendNum})

	local oTarRole = goGPlayerMgr:GetRoleByID(nTarRoleID)
    if oTarRole then 
		local nTarRoleFriendNum = self:GetFriendCount(nTarRoleID)
		goRemoteCall:Call("OnBecomeFriend", oTarRole:GetStayServer(), oTarRole:GetLogic(), oTarRole:GetSession(), oTarRole:GetID(), {nFriendNum=nTarRoleFriendNum})
    end
end
...
function CFriendMgr:OnBattleEnd(nRoleID, tBTRes)
	...
	for _, nTeamRoleID in ipairs(tBTRes.tTeamRoleList) do
		if not GF.IsRobot(nTeamRoleID) then 
...
function CFriendMgr:BroadcastFriendTalk(oRole, bOnlyOnline, sCont, tFilter)
	...
	if #tSessionList > 0 then
		CmdNet.PBBroadcastExter("FriendTalkRet", tSessionList, {tTalk = tTalkMsg})
	end
end
...
function CFriendMgr:TalkReq(oRole, nTarRoleID, sCont, bXMLMsg, bSys)
    ...
    goRemoteCall:CallWait("QueryRoleTotalRechargeReq", fnQueryCallback, 
...
function CFriendMgr:OnDegreesChange(nRoleID)
	...
	goRemoteCall:Call("FriendDegreeChangeReq", oRole:GetServer(), goServerMgr:GetGlobalService(oRole:GetServer(), 20), 0, nRoleID, nTotalDegrees)
end
...
function CFriendMgr:SyncHouseFriendData(nRoleID,nTarRoleID)
	...
	goRemoteCall:Call("FriendChange",gnWorldServerID,nServiceID,0,nRoleID,nTarRoleID)
end

goFriendMgr = goFriendMgr or CFriendMgr:new()