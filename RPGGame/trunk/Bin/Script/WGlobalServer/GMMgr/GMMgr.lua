...
CGMMgr["reloadall"] = function(self, nServer, nService, nSession, tArgs)
	...
	for _, tConf in pairs(tList) do
		if tConf.nServer ~= gnWorldServerID then
			goRemoteCall:Call("GMCommandReq", tConf.nServer, tConf.nID, nSession, "lgm reload local")
			goRemoteCall:Call("GMCommandReq", tConf.nServer, tConf.nID, nSession, "rgm reload")
			goRemoteCall:Call("GMCommandReq", tConf.nServer, tConf.nID, nSession, "agm reload")
			goRemoteCall:Call("GMCommandReq", tConf.nServer, tConf.nID, nSession, "reload")
		elseif tConf.nID ~= GF.GetServiceID() then
			goRemoteCall:Call("GMCommandReq", tConf.nServer, tConf.nID, nSession, "reload", nServer)
		end
	end
	--自己
...
--关闭整个跨服组服务器,不允许单独关闭某个本地服,更新也是整个跨服组一起更新
CGMMgr["closeserver"] = function(self, nServer, nService, nSession, tArgs)
	local tRouter = goServerMgr:GetRouterService()
	CmdNet.Srv2Srv("CloseServerReq", gnWorldServerID, tRouter.nID, 0, gnWorldServerID)
end
