...
void Actor::StartRun(int nSpeedX, int nSpeedY, int8_t nDir)
{
...
void Actor::BroadcastStartRun()
{
	...
	gpoPacketCache->Reset();
	goPKWriter << m_nAOIID << (uint16_t)m_oPos.x << (uint16_t)m_oPos.y << (int16_t)m_nRunSpeedX << (int16_t)m_nRunSpeedY << (uint8_t)m_nDir;
	Packet* poPacket = gpoPacketCache->DeepCopy();
