...
$json = rawurldecode(strval($_GET['data']));
$data = json_decode($json, true);
$pmtName = empty($_GET['pmtname']) ? "" : rawurldecode(strval($_GET['pmtname']));
$sign = empty($_GET['sign']) ? "" : strval($_GET['sign']);

if (empty($data['money']) || empty($data['charid']) || empty($data['charname'])
	|| empty($data['rechargeid']) || empty($data['rechargename']) || empty($sign)) {
	exit("参数非法2 data:$json sign:$sign");
}

$signStr = $MD5_KEY.$json.$pmtName;
$tarSign = md5($signStr);
if ($sign != $tarSign) {
	exit("签名错误:$sign=>$tarSign $signStr");
}
...
$orderInfo = array("订单号:"=>$orderID, "商品ID:"=>$rechargeID, "商品类型:"=>$rechargeType, "商品名称:"=>$rechargeName,
	"角色ID:"=>$charID, "角色昵称:"=>"$charName", "订单金额:"=>$money, "订单状态:"=>$order['result_code']);
