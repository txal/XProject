...
#include "Include/DBDriver/ssdb_win/buffer.h"
#include "Include/DBDriver/ssdb_win/socketlibtypes.h"
...
ox_socket_connect(const char* server_ip, int port)
{
    ...
    if(clientfd != SOCKET_ERROR)
    {
		...
		int ret = connect(clientfd, (struct sockaddr*)&server_addr, sizeof(struct sockaddr));
		if (ret == SOCKET_ERROR)
		{
			const char* psErr = Platform::LastErrorStr(GetLastError());
			XLog(LEVEL_ERROR, "%s", psErr);
...
Status SSDBClient::setnx(const std::string& key, const std::string& val, std::string *ret)
{
...
Status SSDBClient::hsize(const std::string &name, int64_t *ret)
{
    m_request->appendStr("hsize");
    m_request->appendStr(name);
    m_request->endl();
    request(m_request->getResult(), m_request->getResultLen());
...
Status SSDBClient::hkeys(const std::string &name, const std::string &key_start, const std::string &key_end, uint64_t limit, std::vector<std::string> *ret)
{
	std::string s_limit = tostr(limit);
	m_request->appendStr("hkeys");
	m_request->appendStr(name);
	m_request->appendStr(key_start);
	m_request->appendStr(key_end);
	m_request->appendStr(s_limit);
	m_request->endl();
    request(m_request->getResult(), m_request->getResultLen());
	return read_list(m_reponse, ret);
}

Status SSDBClient::hscan(const std::string &name, const std::string &key_start, const std::string &key_end, uint64_t limit, std::vector<std::string> *ret)
{
	std::string s_limit = tostr(limit);
	m_request->appendStr("hscan");
	m_request->appendStr(name);
	m_request->appendStr(key_start);
	m_request->appendStr(key_end);
	m_request->appendStr(s_limit);
...
Status SSDBClient::hincr(const std::string &name, const std::string &key, int64_t incrby, int64_t *ret)
{
	std::string s_incrby = tostr(incrby);
	m_request->appendStr("hincr");
    m_request->appendStr(name);
    m_request->appendStr(key);
	m_request->appendStr(s_incrby);
    m_request->endl();
    request(m_request->getResult(), m_request->getResultLen());
	return read_int64(m_reponse, ret);
}

Status SSDBClient::hclear(const std::string &name, int64_t *ret /*= NULL*/)
{
