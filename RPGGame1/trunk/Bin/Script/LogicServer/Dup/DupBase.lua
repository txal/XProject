...
function CDupBase:KickAllRole()
    ...
    local tRoleNativeList = self:GetObjList(-1, gtGDef.tObjType.eRole)
...
function CDupBase:BroadcastScene(nLine, sCmd, tMsg)
    local tObjList = self:GetObjList(nLine, gtGDef.tObjType.eRole)
    if #tObjList <= 0 then
        return
    end
    local tSessionList = {}
...
function CDupBase:Enter(oNativeObj, nPosX, nPosY, nLine, nFace)
    ...
    local bRobot = false 
    if oLuaObj and oLuaObj:GetObjType() == gtGDef.tObjType.eRole then 
