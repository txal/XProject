...
function Network.CltPBProc.RoleEnterSceneReq(nCmd, nSrcServer, nSrcService, nTarSession, tData)
    print("Network.CltPBProc.RoleEnterSceneReq***", nCmd, nSrcServer, nSrcService, nTarSession, tData)
    ...
    local tTarDupConf = ctDupConf[CUtil:GetDupID(tData.nDupMixID)]
...
function Network.CltPBProc.RoleLeaveSceneReq(nCmd, nSrcServer, nSrcService, nTarSession, tData)
...
------服务器内部
function Network.RpcSrv2Srv.DupRoleViewListReq(nSrcServer, nSrcService, nTarSession, nRoleID)
...
function Network.RpcSrv2Srv.CreateDup(nSrcServer,nSrcService,nTarSession,nDupID,tParam)
...
function Network.RpcSrv2Srv.RemoveDup(nSrcServer,nSrcService,nTarSession,nDupMixID)
...
function Network.RpcSrv2Srv.RemoveUnionDup(nSrcServer,nSrcService,nTarSession,nDupMixID)
    local oDupObj = goDupMgr:GetDup(nDupMixID)
    if not oDupObj then return end

    local tObjList = oDupObj:GetObjList(-1, gtGDef.tObjType.eRole)
...
function Network.RpcSrv2Srv.RoleEnterDup(nSrcServer,nSrcService,nTarSession,nRoleID,nDupMixID,tPos)
    tPos = tPos or {}
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
    local oCurrDupObj = oRole:GetCurrDupObj()
...
function Network.RpcSrv2Srv.EnterCheckReq(nSrcServer, nSrcService, nTarSession, nRoleID, nDupMixID, tRoleParam)
