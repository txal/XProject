...
function CBattleMgr:Ctor()
	self.m_nAutoID = 0
	self.m_tBattleMap = {}
	self.m_nClearTimer = GetGModule("TimerMgr"):Interval(nClearInterval, function() self:ClearBattle() end)
...
function CBattleMgr:GenID()
	...
	local nUnixMSTime = CUtil:GetUnixMSTime()
...
function CBattleMgr:GetPVETeamLv(nType, tRoleBTData)
	...
	if nType == 1 then
		...
		for nUnit, tBTData in pairs(tRoleBTData.tUnitMap) do
			if tBTData.nObjType == gtGDef.tObjType.eRole then
