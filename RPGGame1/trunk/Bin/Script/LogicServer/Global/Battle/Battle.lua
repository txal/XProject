...
--释放战斗对象
function CBattle:Release(bClear)
	GetGModule("TimerMgr"):Clear(self.m_nReadyTimer)
	self.m_nReadyTimer = nil
	GetGModule("TimerMgr"):Clear(self.m_nRoundTimer)
	self.m_nRoundTimer = nil
	GetGModule("TimerMgr"):Clear(self.m_nDelayTimer)
	self.m_nDelayTimer = nil
	GetGModule("TimerMgr"):Clear(self.m_nStartTimer)
	self.m_nStartTimer = nil
	GetGModule("TimerMgr"):Clear(self.m_nSecondTimer)
...
function CBattle:BattleBegin()
	...
	self:CheckSkillTips()
	--每秒检测播放完成
	self.m_nSecondTimer = GetGModule("TimerMgr"):Interval(1, function() self:CheckPlayFinish() end)
end
...
function CBattle:CheckSkillTips()
	if #self.m_tRoundData.tAction > 0 then
		...
		self.m_nStartTimer = GetGModule("TimerMgr"):Interval(nRoundTime, function() self:RoundPrepare() end)
	else
		self:RoundPrepare()
	end
end
...
function CBattle:RoundPrepare()
	...
	--倒计时
	GetGModule("TimerMgr"):Clear(self.m_nReadyTimer)
	self.m_nReadyTimer = GetGModule("TimerMgr"):Interval(nReadyTime, function() self:OnReadyTimer() end)
...
function CBattle:OnRoundStart()
	...
	self:Broadcast("RoundDataRet", self.m_tRoundData)
	local nRoundTime = math.max(1, self.m_tRoundData.nRoundTime)
	self.m_nRoundTimer = GetGModule("TimerMgr"):Interval(nRoundTime, function() self:OnRoundTimer() end)
end
...
function CBattle:OnRoundLimitEnd()
	if self:IsArena() or self:IsPVP() then 
		...
		--防止延迟结算期间，活动通知战斗主动结束
		self.m_nDelayTimer = GetGModule("TimerMgr"):Interval(2, function(nTimerID)
...
function CBattle:ChangeBattleCommandReq(oRole, nCmdID, sCmdName)
	...
	CUtil:HasBadWord(sCmdName, function(bRes)
