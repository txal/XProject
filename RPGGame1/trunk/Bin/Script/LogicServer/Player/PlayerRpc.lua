...
--@nSession 目标网络句柄
--@tData PB数据
function Network.CltPBProc.RoleAttrReq(nCmd, nServer, nService, nSession, tData)
...
--参数同上
function Network.CltPBProc.RoleModNameReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
...
function Network.CltPBProc.RoleStuckLevelReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    oRole:StuckLevelReq(tData.bStuck)
end

function Network.CltPBProc.RoleServerLvReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
...
---------------服务器内部----------------
--角色上线通知(LOGIN)
function Network.RpcSrv2Srv.RoleOnlineReq(nSrcServer, nSrcService, nTarSession, nRoleID)
	return goPlayerMgr:RoleOnlineReq(nSrcServer, nTarSession, nRoleID)
end

--角色下线通知(LOGIN)
function Network.RpcSrv2Srv.RoleOfflineReq(nSrcServer, nSrcService, nTarSession, nRoleID)
	return goPlayerMgr:RoleOfflineReq(nRoleID)
end

--角色断线通知(LOGIN)
function Network.RpcSrv2Srv.RoleDisconnectReq(nSrcServer, nSrcService, nTarSession, nRoleID)
    return goPlayerMgr:RoleDisconnectReq(nRoleID)
end

--角色删除检查(LOGIN)
function Network.RpcSrv2Srv.DeleteRoleCheckReq(nSrcServer, nSrcService, nTarSession, nRoleID)
    return goPlayerMgr:DeleteRoleCheckReq(nRoleID)
end

--物品数量请求([W]GLOBAL)
function Network.RpcSrv2Srv.RoleItemCountReq(nSrcServer, nSrcService, nTarSession, nRoleID, nType, nID)
...
function Network.RpcSrv2Srv.KnapsackRemainCapacityReq(nSrcServer, nSrcService, nTarSession, nRoleID, nPropID, bBind)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
--物品列表数量请求(GLOBAL)
function Network.RpcSrv2Srv.RoleItemCountListReq(nSrcServer, nSrcService, nTarSession, nRoleID, tItemList)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
    local tRet = {}
...
--检查物品数量请求(GLOBAL)
function Network.RpcSrv2Srv.RoleCheckItemCountReq(nSrcServer, nSrcService, nTarSession, nRoleID, tItemList)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
    local bRetFlag = true
    local tTrueList = {}
...
--物品数量增加([W]GLOBAL)
function Network.RpcSrv2Srv.RoleAddItemReq(nSrcServer, nSrcService, nTarSession, nRoleID, tItemList, sReason)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
--物品数量扣除([W]GLOBAL)
function Network.RpcSrv2Srv.RoleSubItemReq(nSrcServer, nSrcService, nTarSession, nRoleID, tItemList, sReason)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
    local tTransItem = {}
    for _, tItem in ipairs(tItemList) do 
...
--扣除指定格子的物品
function Network.RpcSrv2Srv.RoleSubPropByGridReq(nSrcServer, nSrcService, nTarSession, nRoleID, tItemList, sReason)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
--获取并扣除背包指定格子道具
-- {{nID, nGrid, nNum}, ...}
function Network.RpcSrv2Srv.RoleGetPropDataWithSubReq(nSrcServer, nSrcService, nTarSession, nRoleID, tItemList, sReason, bUnbindLimit)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
    --check
...
--物品数量扣除提示
function Network.RpcSrv2Srv.RoleSubItemShowNotEnoughTipsReq(nSrcServer, nSrcService, nTarSession, nRoleID, tItemList, sReason, bFirstBreak, bNum)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
    local tList = {}
    for k, v in ipairs(tItemList) do
...
--传送物品列表请求([W]GLOBAL)
function Network.RpcSrv2Srv.RoleTransferItemListReq(nSrcServer, nSrcService, nTarSession, nRoleID, tPropDataList, sReason)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
--根据道具数据发送物品详细信息给客户端
function Network.RpcSrv2Srv.RolePropDetailInfoReq(nSrcServer, nSrcService, nTarSession, nRoleID, tPropData)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
--获取物品的详细信息
function Network.RpcSrv2Srv.RoleItemDetailDataReq(nSrcServer, nSrcService, nTarSession, nRoleID, nBelongServer, nItemType, nKey)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then 
        -- return false, "角色不在线"
...
--查询摆摊刷新道具([W]GLOBAL)
function Network.RpcSrv2Srv.QueryMarketFlushItemReq(nSrcServer, nSrcService, nTarSession, nRoleID)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
--查询角色的简要信息
function Network.RpcSrv2Srv.RoleInfoDataReq(nSrcServer, nSrcService, nTarSession, nRoleID, nBelongServer)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then 
        -- return false, "角色不在线"
        --可能异步查询期间，机器人销毁，从而导致触发异常
...
function Network.RpcSrv2Srv.SwitchLogicReq(nSrcServer, nSrcService, nTarSession, tSwitch) 
    goPlayerMgr:OnSwitchLogicReq(tSwitch)
end

function Network.RpcSrv2Srv.RobotSwitchLogicReq(nSrcServer, nSrcService, nTarSession, tSwitch, tCreateData, tSaveData)
...
--@nSrcServer: 来源服务器ID(可能是世界服过来,所以要带上角色自己服务器ID)
--@nTarSession: 目标角色会话ID
function Network.RpcSrv2Srv.WSwitchLogicReq(nSrcServer, nSrcService, nTarSession, tTarget) 
...
--查询角色的副本信息[WGLOBAL]
function Network.RpcSrv2Srv.QueryRoleDupInfoReq(nSrcServer, nSrcService, nTarSession, nRoleID)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then
...
--更新角色属性([W]GLOBAL)
function Network.RpcSrv2Srv.RoleUpdateReq(nSrcServer, nSrcService, nTarSession, nServerID, nRoleID, tData)
    CRole:RoleUpdateReq(nServerID, nRoleID, tData)
    return true
end

--取角色属性([W]GLOBAL)
function Network.RpcSrv2Srv.RoleValueReq(nSrcServer, nSrcService, nTarSession, nRoleID, sKey)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
--发送邮件奖励
function Network.RpcSrv2Srv.SendMailAwardReq(nSrcServer, nSrcService, nTarSession, nRoleID, tItemList, bNotSync)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
function Network.RpcSrv2Srv.SetFollowReq(nSrcServer, nSrcService, nTarSession, nMixObjID, tFollowList)
...
--师徒任务战斗
function Network.RpcSrv2Srv.MentorshipTaskBattleReq(nSrcServer, nSrcService, nTarSession, nRoleID, tData)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
function Network.RpcSrv2Srv.AppellationUpdateReq(nSrcServer, nSrcService, nTarSession, nRoleID, nServerID, tData)
    return CRole:AppellationUpdateReq(nServerID, nRoleID, tData)
end

--家园属性同步
function Network.RpcSrv2Srv.SyncHouseBattleAttr(nSrcServer, nSrcService, nTarSession,nRoleID,tHouseBattleAttr)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
--家园购买礼物
function Network.RpcSrv2Srv.HouseGiveGiftReq(nSrcServer, nSrcService, nTarSession,nRoleID,nPropID,nAmount,bMoneyAdd)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
--查询角色的副本信息[WGLOBAL]
function Network.RpcSrv2Srv.GetRoleCurrDupInfoReq(nSrcServer, nSrcService, nTarSession, nRoleID)
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>获取角色所在副本信息")
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then
...
function Network.RpcSrv2Srv.UpdateUnionAppellation(nSrcServer, nSrcService, nTarSession, nRoleID, nAppeConfID, tParam, nSubKey)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
...
function Network.RpcSrv2Srv.UpdateArenaAppellation(nSrcServer, nSrcService, nTarSession, nRoleID, nAppeConfID, tParam, nSubKey)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then
...
--创建机器人
function Network.RpcSrv2Srv.CreateRobotReq(nSrcServer, nSrcService, nSession, nServer, nRobotID, nRoleID, nRobotType, nDupMixID, tParam)
	return goPlayerMgr:CreateRobotReq(nServer, nRobotID, nRoleID, nRobotType, nDupMixID, tParam)
end


--创建机器人
function Network.RpcSrv2Srv.CreateSysEquCacheReq(nSrcServer, nSrcService, nSession, tPropIDList, nMirrorID)
    local tEquCache = {}
    -- --创建一个临时机器人，利用机器人生成装备
    -- local nRobotDupID = 0
...
--社会关系喊话属性查询
function Network.RpcSrv2Srv.RelationInviteInfoQueryReq(nSrcServer, nSrcService, nTarSession, nRoleID)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
    local tData = {}
...
--查询玩家总充值金额
function Network.RpcSrv2Srv.QueryRoleTotalRechargeReq(nSrcServer, nSrcService, nTarSession, nRoleID)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then return end
...
--心跳
function Network.CltCmdProc.KeepAlive(nCmd, nSrcServer, nSrcService, nTarSession, nRoleID, nLastKeepAliveTime)
    local oRole = goPlayerMgr:GetRoleByID(nRoleID)
    if not oRole then
        return
    end
