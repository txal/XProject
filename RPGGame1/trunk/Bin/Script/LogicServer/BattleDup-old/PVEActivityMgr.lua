...
function CPVEActivityMgr:Ctor(nID, nType)
	...
	self.m_nHourTimer= 0
	self.m_tDupList = CUtil:WeakTable("v") 	--地图列表{地图对象,...  --ID
...
function CPVEActivityMgr:RegTick()
	if self:IsSceneLogic() then
		self.m_nTimer = GetGModule("TimerMgr"):Interval(1, function() self:CheckActOpen() end)
...
function CPVEActivityMgr:InitStamp()
	if not self.m_nRankingTimer then
		...
		self.m_nRankingTimer = GetGModule("TimerMgr"):Interval(nOverMis, function() self:PVEActSettle() end)
...
function CPVEActivityMgr:EnterReadyDupReq(oRole)
	...
	if CUtil:GetServiceID() == nDupnLogic then
...
function CPVEActivityMgr:Release()
	 GetGModule("TimerMgr"):Clear(self.m_nRankingTimer)
	 self.m_nRankingTimer = nil
	GetGModule("TimerMgr"):Clear(self.m_nTimer)
	self.m_nTimer = nil
	GetGModule("TimerMgr"):Clear(self.m_nGMOpenActEndTimer)
...
function CPVEActivityMgr:OpenAct(nActID, nReadyTime, nEndTime)
	...
	self:NotifyActivityOpen()
	self.m_nGMOpenActEndTimer = GetGModule("TimerMgr"):Interval(self.m_nActivityEndStamp - os.time(), function() self:GMOpenActEnd() end)
...
function CPVEActivityMgr:GMOpen(nActID, nReadyTime, nEndTime, oRole)
	...
	if nTarService == CUtil:GetServiceID() then
