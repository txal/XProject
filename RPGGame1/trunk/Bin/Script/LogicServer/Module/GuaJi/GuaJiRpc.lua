--客户端->服务器
function Network.CltPBProc.GuaJiReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    goBattleDupMgr:EnterBattleDupReq(oRole, gtBattleDupType.eGuaJi)
end

function Network.CltPBProc.GuaJiAutoBattleOperaReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
...
function Network.CltPBProc.GuaJiChalBossReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    local oBattleDup = goBattleDupMgr:GetBattleDup(oRole:GetBattleDupID())
...
function Network.CltPBProc.GuaJiBattleEndNoticeReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    local oBattleDup = goBattleDupMgr:GetBattleDup(oRole:GetBattleDupID())
...
function Network.CltPBProc.StartNoticReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    local oBattleDup = goBattleDupMgr:GetBattleDup(oRole:GetBattleDupID())
...
----------------------------------------------
function Network.RpcSrv2Srv.GuaJiReward(nSrcServer,nSrcService,nTarSession,nRoleID, tData)
