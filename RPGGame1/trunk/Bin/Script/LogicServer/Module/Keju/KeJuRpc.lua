...
function Network.CltPBProc.OpenKejuDataReq(nCmd,nServer,nService,nSession,tData)
	local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    oRole.m_oKeju:OpenKeJu(tData.nKejuType)
end

function Network.CltPBProc.AnswerKejuQuestionReq(nCmd, nServer, nService, nSession, tData)
	local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    oRole.m_oKeju:AnswerQuestion(tData.nQuestionID,tData.nAnswerNo)
end

function Network.CltPBProc.CloseKejuQuestion(nCmd, nServer, nService, nSession, tData)
	local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    oRole.m_oKeju:CloseKeJu(tData.nKejuType)
end

function Network.CltPBProc.KejuAskHelpReq(nCmd,nServer,nService,nSession,tData)
	local oRole = goPlayerMgr:GetRoleBySS(nServer,nSession)
	if not oRole then return end
	oRole.m_oKeju:KejuAskHelp(tData.nQuestionID)
end

function Network.CltPBProc.KejuAnswerHelpQuestionReq(nCmd,nServer,nService,nSession,tData)
	local oRole = goPlayerMgr:GetRoleBySS(nServer,nSession)
	if not oRole then return end
...
function Network.CltPBProc.KejuAskHelpDataReq(nCmd,nServer,nService,nSession,tData)
	local oRole = goPlayerMgr:GetRoleBySS(nServer,nSession)
	if not oRole then return end
	oRole.m_oKeju:KejuAskHelpData(tData.nQuestionID)
end

function Network.CltPBProc.KejuHelpQuestionDataReq(nCmd,nServer,nService,nSession,tData)
	local oRole = goPlayerMgr:GetRoleBySS(nServer,nSession)
	if not oRole then return end
...
-----------------------服务器内部--------------------------
function Network.RpcSrv2Srv.JoinKejuDianshi(nSrcServer,nSrcService,nTarSession,tJoinPlayer)
...
function Network.RpcSrv2Srv.KejuHelpQuestionDataReq(nSrcServer,nSrcService,nTarSession,nRoleID,nHelpRoleID,nQuestionID)
	local oRole = goPlayerMgr:GetRoleByID(nRoleID)
	if not oRole then return end
