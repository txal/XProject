...
function CAppellationPVPAct:Ctor(oModule, nID, nConfID, tParam, nSubKey)
    CAppellationBase.Ctor(self, oModule, nID, nConfID, tParam, nSubKey)
    self.m_nExpiryTime = tParam.nExpiryTime or 0
end

function CAppellationPVPAct:LoadData(tData)
    if not tData then 
        return 
    end
...
function CAppellationPVPAct:SaveData()
    local tData = CAppellationBase.SaveData(self)
    tData.nExpiryTime = self.m_nExpiryTime
    return tData
end

function CAppellationPVPAct:IsExpired(nTimeStamp)
