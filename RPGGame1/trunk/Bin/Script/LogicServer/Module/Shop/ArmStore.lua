...
function CArmStore:Ctor(oModule)
	--print("CArmStore:Ctor(oModule)")
...
function CArmStore:LoadData(tData)
	print("CArmStore:LoadData( tData )",tData)
	self.m_tAllreadyBuy = tData.m_tAllreadyBuy 
	self.m_tFirstBuyTime = tData.m_tFirstBuyTime 
end

function CArmStore:SaveData()
	local tData = {}
...
function CArmStore:MarkDirty(bMark)
	self.m_oModule:MarkDirty(bMark)
end

--记录限购物品 已购买个数
function CArmStore:RecordLimItemBuyNum(tItem, nNum)
...
--重置限购表
function CArmStore:ResetRecordLimBuyNum()
...
--物品列表请求
function CArmStore:ItemListReq(nShopType)
