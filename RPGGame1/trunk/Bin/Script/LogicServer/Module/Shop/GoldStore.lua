...
function CGoldStore:Ctor(oModule)
...
function CGoldStore:LoadData(tData)
	print("CGoldStore:LoadData( tData )",tData)
	self.m_tAllreadyBuy = tData.m_tAllreadyBuy 
	self.m_tFirstBuyTime = tData.m_tFirstBuyTime 
end

function CGoldStore:SaveData()
	local tData = {}
...
function CGoldStore:MarkDirty(bMark)
	self.m_oModule:MarkDirty(bMark)
end

--记录限购物品 已购买个数
function CGoldStore:RecordLimItemBuyNum(tItem, nNum)
...
--重置限购表
function CGoldStore:ResetRecordLimBuyNum()
...
--物品列表请求
function CGoldStore:ItemListReq(nShopType)
