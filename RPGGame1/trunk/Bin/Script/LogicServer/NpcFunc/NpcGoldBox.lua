...
function CNpcGoldBox:FuYuanExchangeReq(oRole, nExchangeID)
	...
	if sBroad then
		CUtil:SendNotice(oRole:GetServer(), sBroad)
	end
end
...
function CNpcGoldBox:RegHourTimer()
	...
	self.m_nHourTimer = GetGModule("TimerMgr"):Interval(nRefreshInterval, function() self:OnHourTimer() end)
end