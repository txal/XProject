...
function CBattleDupBase:Ctor(nID, nGameType)
    ...
    self.m_tRoleMap = CUtil:WeakTable("kv")    --{nRoleID:oRole, ...}进入场景加入，离开场景删除
    self.m_tDupRoleMap = CUtil:WeakTable("kv") --{nRoleID:oRole, ...}发起战斗的玩家，必须属于m_tDupRoleMap
    ...
    self.m_tMonsterMap = CUtil:WeakTable("kv")  --{nMonsterID:oMonster, ...}
...
function CBattleDupBase:Release()
...
function CBattleDupBase:OnObjEnter(oObj, bReconnect)
    ...
    if oObj:GetObjType() == gtGDef.tObjType.eRole then 
...
function CBattleDupBase:OnRoleTeamChange(oRole)
    ...
    if oRole:GetTeamID() <= 0 then --只关心离队或者队伍ID小于0的队伍变更
        ...
        if bExistOther then 
            ...
            --延迟3秒踢出玩家
            GetGModule("TimerMgr"):Interval(3, function (nTimerID) 
