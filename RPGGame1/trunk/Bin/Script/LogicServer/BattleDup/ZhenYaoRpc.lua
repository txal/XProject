--客户端->服务器
function Network.CltPBProc.ZhenYaoCreateMonsterReq(nCmd, nServer, nService, nSession, tData)
...
function Network.CltPBProc.CreateMonsterReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
...
function Network.CltPBProc.DupBuffOperaReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
    ...
    --print("Network.CltPBProc.DupBuffOperaReq***", oRole:GetID(), oRole:GetName())
end

function Network.CltPBProc.ZhenYaoMatchTeamReq(nCmd, nServer, nService, nSession, tData)
    local oRole = goPlayerMgr:GetRoleBySS(nServer, nSession)
    if not oRole then return end
