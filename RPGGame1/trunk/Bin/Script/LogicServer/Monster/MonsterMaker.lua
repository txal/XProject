...
function CMonsterMaker:CreateMonster(nMonsterID)
	...
	--1波怪刷完,检测下1波怪
		if self.m_nCurrGroup < #self.m_tGroupConfList then
			...
			if tNextGroupConf.nPrepType == 2 and tNextGroupConf.tPrepID[1][1] == self.m_nCurrGroup then
				GetGModule("TimerMgr"):Clear(self.m_nMakerTick)
				self.m_nMakerTick = GetGModule("TimerMgr"):Interval(tNextGroupConf.nDelayTime*0.001, function() self:MakeMonster(self.m_nCurrGroup+1) end)
			end
		end
	...
	elseif tGroupConf.nIntervalTime > 0 then
		...
		self.m_nMakerTick = GetGModule("TimerMgr"):Interval(tGroupConf.nIntervalTime*0.001, function() self:CreateMonster(nNextMonsterID) end)
