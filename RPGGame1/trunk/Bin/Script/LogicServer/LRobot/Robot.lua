...
--通过配置生成的机器人，nSrcID必须小于gtGDef.tConst.nRobotIDMax
...
function CRobot:Ctor(nServer, nRobotID, nSrcID, nRobotType, nDupMixID, tParam, tSaveData)
    ...
    if CUtil:IsRobot(nSrcID) and not tSaveData then 
...
--     if not CUtil:IsRobot(self.m_nSrcID) then 
--         return 
--     end
--     --TODO 使用配置数据初始化相关数据
--     assert(false, "当前未实现")
...
function CRobot:LoadSelfData(tSaveData)
    if CUtil:IsRobot(self.m_nID) and not tSaveData then 
