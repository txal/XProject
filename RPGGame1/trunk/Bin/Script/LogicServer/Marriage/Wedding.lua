...
function CWedding:CleanWeddingTimer()
	if self.m_nWeddingTimer then
		GetGModule("TimerMgr"):Clear(self.m_nWeddingTimer)
		self.m_nWeddingTimer = nil
	end
end
...
function CWedding:CleanCandyTimer() 
	if self.m_nCandyTimer then 
		GetGModule("TimerMgr"):Clear(self.m_nCandyTimer)
		self.m_nCandyTimer = nil
	end
end
...
function CWedding:NotifyChooseWeddingLevel()
	...
	self.m_nWeddingTimer = GetGModule("TimerMgr"):Interval(30, function () self:WeddingLevelChooseTimeOut() end)
	--谁申请谁负责 通知客户端选择
...
function CWedding:StartWedding()
	...
	Network.PBSrv2All("MarriageWeddingStartBroadcastRet", tBroadcastMsg)
...
function CWedding:DoWeddingStep(nStep)
	...
	if tStepConf.nTimeCost > 0 then
		self.m_nWeddingTimer = GetGModule("TimerMgr"):Interval(tStepConf.nTimeCost, function () self:OnWeddingStepEnd() end)
	else
...
function CWedding:OnWeddingEnd()
	...
	CUtil:SendMail(oHusband:GetServer(), sMailTitle, sMailContent, {}, oHusband:GetID())
	CUtil:SendMail(oWife:GetServer(), sMailTitle, sMailContent, {}, oWife:GetID())
...
function CWedding:SetCandy(nCandyNum)
	...
	self.m_nCandyTimer = GetGModule("TimerMgr"):Interval(30, function () self:OnWeddingCandyStepEnd() end)
	...
	CUtil:SendNotice(0, sCandyContent)
end
