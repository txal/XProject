...
function CHDCircle:RegAutoSave()
	self.m_nAutoSave = GetGModule("TimerMgr"):Interval(gtGDef.tConst.nAutoSaveTime, function() self:SaveData() end)
end
...
function CHDCircle:Release()
	GetGModule("TimerMgr"):Clear(self.m_nMinTimer)
	self.m_nMinTimer = nil
	GetGModule("TimerMgr"):Clear(self.m_nAutoSave)
	self.m_nAutoSave = nil
	self:SaveData()
end
...
function CHDCircle:RegMinTimer()
	GetGModule("TimerMgr"):Clear(self.m_nMinTimer)
	self.m_nMinTimer = GetGModule("TimerMgr"):Interval(os.NextMinTime(os.time()), function() self:OnMinTimer() end)	
end
