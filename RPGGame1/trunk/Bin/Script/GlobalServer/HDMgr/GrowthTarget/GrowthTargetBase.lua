...
function CGrowthTargetBase:LoadData() 
    ...
    for nRoleID, nVal in pairs(self.m_tRoleMap) do 
        if not CUtil:IsRobot(nRoleID) then --过滤错误的机器人数据
            ...
            if #tTargetAwardList > 0 then 
                ...
                if #tAwardList > 0 then 
                    ...
                    for _, tItem in ipairs(tAwardList) do 
                        ...
                        if #tMailItemList >= gtGDef.tConst.nMaxMailItemLength then 
                            CUtil:SendMail(gnServerID, sTitle, sContent, tMailItemList, nRoleID)
                            tMailItemList = {}
                        end
                    end
...
function CGrowthTargetBase:Release() 
    self:SaveData()
end
