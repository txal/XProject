...
--玩家上线
function CHDQF:Online(oRole)
	self:SyncState(oRole)
end

--更新状态
function CHDQF:UpdateState()
	CHDBase.UpdateState(self)
	self:UpdateSubState()
end

--进入初始状态
function CHDQF:OnStateInit()
	print("花灯祈福进入初始状态")
	self:SyncState()
end

--进入活动状态
function CHDQF:OnStateStart()
	print("花灯祈福进入开始状态")
	--初始化
	self:Init()
	self:SyncState()
...
--进入关闭状态
function CHDQF:OnStateClose()
	print("花灯祈福进入关闭状态")
	self:SyncState()
end

--子状态开始，结束时间
function CHDQF:GetSubTime()
	local nNow = os.time()
	local tTime = ctHDQFEtcConf[1].tDayTime[1]
...
--活动子状态开始
function CHDQF:OnSubStateStart()
	print("花灯祈福进入开始子状态")
...
--活动子状态结束
function CHDQF:OnSubStateClose()
	print("花灯祈福进入关闭子状态")
	self:UpdateRanking(true)
	self:MarkDirty(true)
	self:SyncState()
...
--同步活动状态
function CHDQF:SyncState(oRole)
...
--取已购买道具次数
function CHDQF:GetPropBuyTimes(nRoleID, nPropID)
	local tConf = assert(ctHDQFPropConf[nPropID])
...
--增加道具购买次数
function CHDQF:AddPropBuyTimes(nRoleID, nPropID)
	local tBuyMap = self.m_tBuyPropMap[nRoleID] or {}
...
--道具列表请求
function CHDQF:PropListReq(oRole)
	...
	for nPropID, tConf in pairs(ctHDQFPropConf) do
...
--购买道具请求
function CHDQF:BuyPropReq(oRole, nPropID)
	if self.m_nState ~= CHDBase.tState.eStart or self.m_nSubState ~= CHDBase.tState.eStart then
		return oRole:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	...
	if tPrice[1] == 0 then --免费
		...
		oRole:AddItem(tItemList, "花灯祈福活动购买道具", function(bRet)
			if bRet then
	...
	oRole:SubItem(tItemList, "花灯祈福活动购买道具", function(bRet)
		if not bRet then
...
--花灯祈福奖励记录
function CHDQF:AddAwardRecord(oRole, tAward)
	if not tAward then return end
	local sName = oRole:GetName()
	local sRecord = string.format(ctLang[1], sName, "花灯祈福", ctPropConf:PropName(tAward[2]).."*"..tAward[3])
...
--取花灯祈福奖励列表
function CHDQF:GetAwardRecord()
	return self.m_tAwardRecordList
end

--使用道具请求
function CHDQF:UsePropReq(oRole, nPropID)
	if self.m_nState ~= CHDBase.tState.eStart  or self.m_nSubState ~= CHDBase.tState.eStart  then
		return oRole:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nRoleID = oRole:GetID()
	local tConf = assert(ctHDQFPropConf[nPropID], "道具不存在")
	oRole:SubItem({{nType=gtItemType.eProp,nID=nPropID,nNum=1}}, "花灯祈福活动使用道具", function(bRet)
		if not bRet then
		...
		oRole:AddItem(tItemList, "花灯祈福活动奖励", function(bRet)
			if bRet then
...
--兑换列表请求
function CHDQF:ExchangeListReq(oRole)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oRole:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nRoleID = oRole:GetID()
	...
	for nPropID, tConf in pairs(ctHDQFExchangeConf) do
...
--兑换物品请求
function CHDQF:ExchangeReq(oRole, nPropID)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oRole:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nRoleID = oRole:GetID()
	local nCurrScore = self.m_tRoleScoreMap[nRoleID] or 0
	local nUseScore = self.m_tRoleUseScoreMap[nRoleID] or 0
	local tConf = assert(ctHDQFExchangeConf[nPropID])
	...
	oRole:AddItem({{nType=gtItemType.eProp, nID=nPropID, nNum=1}}, "花灯祈福兑换物品", function(bRet)
		if bRet then
...
--排行榜更新
function CHDQF:UpdateRanking(bEnd)
...
--取我的排名
function CHDQF:GetRoleRank(nRoleID)
...
--取我的联盟排名
function CHDQF:GetUnionRank(nRoleID)
...
--个人排行榜请求
function CHDQF:RoleRankingReq(oRole, nRankNum)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oRole:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	self:UpdateRanking()
...
--联盟排行榜请求
function CHDQF:UnionRankingReq(oRole, nRankNum)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oRole:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	self:UpdateRanking()
...
--取玩家领奖状态
function CHDQF:GetRoleAwardState(nRoleID)
	...
	local nMyRank, nMyValue = self:GetRoleRank(nRoleID)
	if nMyRank == 0 or nMyRank > ctHDQFPLRankingConf[#ctHDQFPLRankingConf].tRank[1][2] then
		return 0 --未上榜不能领取
	end
	return 1 --可领取
end
...
--取联盟领奖状态
function CHDQF:GetUnionAwardState(nRoleID)
	if self.m_nState ~= CHDBase.tState.eAward then
		return 0 --状态不对不可领奖
	end
	local oUnion = goUnionMgr:GetUnionByRoleID(nRoleID)
	...
	local nMyRank, sMyName, nMyValue = self:GetUnionRank(nRoleID)
	if nMyRank == 0 or nMyRank > ctHDQFUNRankingConf[#ctHDQFUNRankingConf].tRank[1][2] then
		return 0 --未上榜不能领取
	end
	return 1 --可领取
end
...
--取个人排行奖励信息
function CHDQF:RoleRankAwardInfoReq(oRole)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oRole:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nRoleID = oRole:GetID()
...
--取联盟排行奖励信息
function CHDQF:UnionRankAwardInfoReq(oRole)
	if self.m_nState ~= CHDBase.tState.eStart and self.m_nState ~= CHDBase.tState.eAward then
		return oRole:Tips("活动已结束:"..self.m_nState..":"..self.m_nSubState)
	end
	local nRoleID = oRole:GetID()
...
--领取个人排行奖励
function CHDQF:RoleRankAwardReq(oRole)
	if self.m_nState ~= CHDBase.tState.eAward then
	...
	for k = #ctHDQFPLRankingConf, 1, -1 do
		local tConf = ctHDQFPLRankingConf[k]
	...
	oRole:AddItem(tItemList, "花灯祈福个人排行奖励", function(bRet)
		if bRet then
...
--领取公会排行奖励
function CHDQF:UnionRankAwardReq(oRole)
	if self.m_nState ~= CHDBase.tState.eAward then
		return oRole:Tips("未到领奖时间:"..self.m_nState)
	end
	local nRoleID = oRole:GetID()
	...
	for k = #ctHDQFUNRankingConf, 1, -1 do
		local tConf = ctHDQFUNRankingConf[k]
		local tRank = tConf.tRank[1]
		if nMyRank >= tRank[1] then
	...
	oRole:AddItem(tItemList, "花灯祈福联盟排行奖励", function(bRet)
		if bRet then
...
--是否可以领取奖励
function CHDQF:CanGetAward(oRole)
	local nRoleID = oRole:GetID()
	...
	if not nAwardState then
		nAwardState = self.m_nServerScore >= ctHDQFEtcConf[1].nScore and 1 or 0
	end
	return (nPlayerAward==1), (nUnionAward==1), (nAwardState==1)
end

--每日奖励是否可领
function CHDQF:IsDayAward(oRole)
...
function CHDQF:IsSubOpen()
	return self.m_nSubState == CHDBase.tState.eStart
end

--领取每日奖励请求
function CHDQF:DayAwardReq(oRole)
	if not self:IsOpen() or not self:IsSubOpen() then
	...
	oRole:AddItem({{nType=gtItemType.eCurr,nID=gtCurrType.eJinBi,nNum=nGoldAward}}, "花灯祈福每日奖励", function(bRet)
		if bRet then
			self:SyncState(oRole)
		end
	end)
