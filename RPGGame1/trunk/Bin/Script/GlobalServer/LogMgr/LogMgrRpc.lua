...
--事件日志
function Network.RpcSrv2Srv.EventLogReq(nSrcServer, nSrcService, nTarSession, nEventID, sReason, tInfo, ...)
...
--账号日志
function Network.RpcSrv2Srv.CreateAccountLogReq(nSrcServer, nSrcService, nTarSession, nSource, nAccountID, sAccountName, nVIP, nTime)
...
--角色日志
function Network.RpcSrv2Srv.CreateRoleLogReq(nSrcServer, nSrcService, nTarSession, nAccountID, nRoleID, sRoleName, nLevel, sImgHeader, nGender, nSchool, nTime)
...
--更新账号信息
function Network.RpcSrv2Srv.UpdateAccountLogReq(nSrcServer, nSrcService, nTarSession, nAccountID, tParam)
	local sSetSql = ""
...
--更新账号信息
function Network.RpcSrv2Srv.UpdateRoleLogReq(nSrcServer, nSrcService, nTarSession, nRoleID, tParam)
	local sSetSql = ""
	for k, v in pairs(tParam) do
		sSetSql = sSetSql .. string.format("%s='%s',", k, _StrField(v))
	end
...
--上线下线日志
function Network.RpcSrv2Srv.OnlineLogReq(nSrcServer, nSrcService, nTarSession, tInfo, nType, nKeepTime, nTime)
...
--分享日志
function Network.RpcSrv2Srv.ShareLogReq(nSrcServer, nSrcService, nTarSession, nSrcServer, nSrcRole, nTarServer, nTarRole, nTime)
...
--元宝日志
function Network.RpcSrv2Srv.YuanBaoLogReq(nSrcServer, nSrcService, nTarSession, tInfo, nTime)
...
--聊天日志
function Network.RpcSrv2Srv.TalkLogReq(nSrcServer, nSrcService, nTarSession, tData)
	goChatReport:AddTalk(tData)
end

--活动日志
function Network.RpcSrv2Srv.ActivityLogReq(nSrcServer, nSrcService, nTarSession, tData)
...
--任务日志
function Network.RpcSrv2Srv.TaskLogReq(nSrcServer, nSrcService, nTarSession, tInfo, nType, nTaskID, nTime)
...
--创建帮派日志
function Network.RpcSrv2Srv.CreateUnionLogReq(nSrcServer, nSrcService, nTarSession, tLog)
...
--删除帮派
function Network.RpcSrv2Srv.DelUnionLogReq(nSrcServer, nSrcService, nTarSession, nUnionID)
	local sql = string.format(gtGameSql.sDelUnionSql, nUnionID)
	goMysqlPool:GameQuery(sql)
end
--更新帮派日志
function Network.RpcSrv2Srv.UpdateUnionLogReq(nSrcServer, nSrcService, nTarSession, nUnionID, tParam)
	local sSetSql = ""
	for k, v in pairs(tParam) do
		sSetSql = sSetSql .. string.format("%s='%s',", k, _StrField(v))
	end
...
--创建帮派成员日志
function Network.RpcSrv2Srv.CreateUnionMemberLogReq(nSrcServer, nSrcService, nTarSession, tLog)
...
--更新帮派成员日志
function Network.RpcSrv2Srv.UpdateUnionMemberLogReq(nSrcServer, nSrcService, nTarSession, nRoleID, tParam)
	local sSetSql = ""
	for k, v in pairs(tParam) do
		sSetSql = sSetSql .. string.format("%s='%s',", k, _StrField(v))
	end
...
---------------- Network.CltPBProc ----------------
--玩家行为日志
function Network.CltPBProc.RoleBehaviourReq(nCmd, nServer, nService, nSession, tData)
	do return end  --暂时屏蔽
	if not tData then 
		return 
	end
