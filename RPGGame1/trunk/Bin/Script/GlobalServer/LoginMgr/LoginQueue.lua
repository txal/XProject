...
function CLoginQueue:Ctor()
    ...
    self.m_nTimer = GetGModule("TimerMgr"):Interval(2, function() self:Tick() end)
end

function CLoginQueue:Release()
...
function CLoginQueue:SyncLoginQueue(nAccountID, nRoleID, nRank)
    ...
    Network.PBSrv2Clt("RoleLoginQueueRet", oAccount:GetServer(), nSession, tMsg)
