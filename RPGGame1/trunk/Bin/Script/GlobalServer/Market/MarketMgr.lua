...
function CMarketMgr:Init()
	self:LoadData()
	self.m_nActiveTradeTimer = GetGModule("TimerMgr"):Interval(60, function () self:OnMinuTimer() end)
	self.m_nRoleDataTimer = GetGModule("TimerMgr"):Interval(70, function () self:TickRoleData() end) --70秒检查一次，和其他时间稍微错开
	self.m_nTickTimer = GetGModule("TimerMgr"):Interval(3, function () self:TickSave() end) --数据保存，间隔1秒
end

function CMarketMgr:Release()
	GetGModule("TimerMgr"):Clear(self.m_nActiveTradeTimer)
	GetGModule("TimerMgr"):Clear(self.m_nRoleDataTimer)
	GetGModule("TimerMgr"):Clear(self.m_nTickTimer)
	self:SaveData()
end
