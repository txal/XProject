...
function CNoticeMgr:OnLoaded()
	self.m_nUpdateTimer = GetGModule("TimerMgr"):Interval(60, function() self:OnUpdateTimer() end)
end

function CNoticeMgr:SaveData()
end

function CNoticeMgr:Release()
	-- self:SaveData()
	-- GetGModule("TimerMgr"):Clear(self.m_nSaveTimer)
	-- self.m_nSaveTimer = nil

	GetGModule("TimerMgr"):Clear(self.m_nUpdateTimer)
	self.m_nUpdateTimer = nil

	for nID, nTimer in pairs(self.m_tNoticeTick) do
		GetGModule("TimerMgr"):Clear(nTimer)
	end
...
function CNoticeMgr:DoNotice(nID)
	...
	if tNotice.nVIP > 0 or tNotice.nSource > 0 then
		...
		Network.PBBroadcastExter("ScrollNoticeRet", tSessionList, {sCont=tNotice.sCont})
	else
		Network.PBSrv2All("ScrollNoticeRet", {sCont=tNotice.sCont})
	end
end
...
function CNoticeMgr:RemoveNotice(nID)
	if self.m_tNoticeMap[nID] then
		...
		GetGModule("TimerMgr"):Clear(self.m_tNoticeTick[nID])
...
function CNoticeMgr:OnNoticeTimer(nID)
	...
	if nNowSec >= tNotice.nEndTime then
		...
		GetGModule("TimerMgr"):Clear(self.m_tNoticeTick[nID])
		self.m_tNoticeTick[nID] = GetGModule("TimerMgr"):Interval(tNotice.nInterval, function() self:OnNoticeTimer(nID) end)
	end
end
...
function CNoticeMgr:OnUpdateTimer()
	...
	while oMysql:FetchRow() do
		...
		if not self.m_tNoticeMap[nID] then
			...
			self.m_tNoticeTick[nID] = GetGModule("TimerMgr"):Interval(nTimerInterval, function() self:OnNoticeTimer(nID) end)
...
function CNoticeMgr:SendNoticeReq(sCont)
	...
	Network.PBSrv2All("ScrollNoticeRet", {sCont=sCont})
end
