...
function CRobot:SendMsg(sCmd, tMsg)
    PBNetwork.BsrCmdProc
Network.Clt2Srv(sCmd, self:PacketID(), self.m_nSessionID, tMsg)
end
...
function CRobot:KeepAlive()
    ...
    if nTimeNow - self.m_nLastKeepAlive >= 10 then
        ...
        Network.BsrCmdProc
Network.Clt2Srv("KeepAlive", self:PacketID(), self.m_nSessionID, nTimeNow)
    end
end
...
function CRobot:OnLoginRet(tData)
    ...
    self.m_nTimer = GetGModule("TimerMgr"):Interval(1, function() self:Update() end)
    self.m_bLogon = true
