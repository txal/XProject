...
function CGRole:Tips(sCont, nServer, nSession)
    ...
    Network.PBSrv2Clt(nServer, nSession, "TipsMsgRet", {sCont=sCont})
end

--取物品数量
function CGRole:ItemCount(nType, nID, fnCallBack)
    if fnCallBack then
        Network.oRemoteCall:CallWait("RoleItemCountReq", fnCallBack, self:GetServer(), self:GetLogic(), self:GetSession(), self:GetID(), nType, nID)
    else
        Network.oRemoteCall:Call("RoleItemCountReq", self:GetServer(), self:GetLogic(), self:GetSession(), self:GetID(), nType, nID)
    end
end
...
function CGRole:AddItem(tItemList, sReason, fnCallBack)
    ...
    if fnCallBack then
        Network.oRemoteCall:CallWait("RoleAddItemReq", fnCallBack, self:GetServer(), self:GetLogic(), self:GetSession(), self:GetID(), tItemList, sReason)
    else
        Network.oRemoteCall:Call("RoleAddItemReq", self:GetServer(), self:GetLogic(), self:GetSession(), self:GetID(), tItemList, sReason)
    end
end
...
function CGRole:SubItem(tItemList, sReason, fnCallBack)
    ...
    if fnCallBack then
        Network.oRemoteCall:CallWait("RoleSubItemReq", fnCallBack, self:GetServer(), self:GetLogic(), self:GetSession(), self:GetID(), tItemList, sReason)
    else
        Network.oRemoteCall:Call("RoleSubItemReq", self:GetServer(), self:GetLogic(), self:GetSession(), self:GetID(), tItemList, sReason)
    end
end