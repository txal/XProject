...
function CTeamMatchMgr:Init()
    self.m_nAutoMergeTimer = GetGModule("TimerMgr"):Interval(5, function() self:Tick() end)
end

function CTeamMatchMgr:Release()
    if self.m_nAutoMergeTimer then 
        GetGModule("TimerMgr"):Clear(self.m_nAutoMergeTimer)
    end
    self.m_nAutoMergeTimer = nil
end
...
function CTeamMatchMgr:OnRoleQuitTeam(nRoleID, nTeamID)
    ...
    local oRole = goGPlayerMgr:GetRoleByID(nRoleID)
    if oRole and CUtil:IsRobot(nRoleID) then 
        local nGameType = self.m_tRobotGameTypeMap[nRoleID]
...
function CTeamMatchMgr:BroadcastTeamMatch(nRoleID, nGameType, sGameName)
    ...
    if sGameName then
        ...
        CUtil:SendWorldTalk(nRoleID, sContent, true)
    end
end
