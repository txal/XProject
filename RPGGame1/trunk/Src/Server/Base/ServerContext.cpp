...
bool ServerContext::LoadServerConfig()
{
	...
	{
		XLog(LEVEL_ERROR, "mgr sql conf error!\n");
		return false;
	}
	...
	m_oServerConf.uGroupID = (uint16_t)nGroupID;
	...
	{
		uint16_t uServerID = (uint16_t)m_pMgrMysql->ToInt32("serverid");
		uint16_t uServiceID = (uint16_t)m_pMgrMysql->ToInt32("serviceid");
		uint16_t uServicePort = (uint16_t)m_pMgrMysql->ToInt32("serviceport");
		...
		{
			GateNode oGate;
			oGate.uID = uServiceID;
			oGate.uServer = uServerID;
			oGate.uPort = uServicePort;
			oGate.uMaxConns = 20000;
			oGate.uSecureCPM = 0;
			oGate.uSecureQPM = 300;
			oGate.uDeadLinkTime = 120;

			lua_settop(pState, 0); //注意这里要清下栈，不然服务器一多会溢出崩溃
		...
		//{
		//	LoginNode oLogin;
		//	oLogin.uID = uServiceID;
		//	oLogin.uServer = uServerID;
