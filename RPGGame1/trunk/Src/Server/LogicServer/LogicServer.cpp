...
bool LogicServer::Init(int8_t nServiceID)
{
	...
	return m_oMonitorThread.Create(LogicServer::DeadLoopMonitorFunc, this, false);
}
...
bool LogicServer::RegToRouter(int8_t nRouterServiceID)
{
	...
	INNER_HEADER oHeader(NSSysCmd::ssRegServiceReq, gpoContext->GetServerConfig().GetServerID(), GetServiceID(), 0, poRouter->nService, 0);
...
void LogicServer::DeadLoopMonitorFunc(void* pParam)
{
	LogicServer* poService = (LogicServer*)pParam;
