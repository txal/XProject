...
void MonsterAI::SearchNearTarget(int64_t nNowMS)
{
	...
	SceneBase* poScene = m_poActor->GetScene();
	SceneBase::GameObjMap& oObjMap = poScene->GetGameObjMap();
	SceneBase::GameObjIter iter = oObjMap.begin();
	SceneBase::GameObjIter iter_end = oObjMap.end();
...
{ 
	...
	SceneBase* poScene = m_poActor->GetScene();
	Array<AOIOBJ*>& oObjList = poScene->GetAreaObserveds(m_poActor->GetAOIID(), (OBJTYPE)0);
