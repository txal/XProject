...
{
	uint16_t nWorldServerID = gpoContext->GetServerConfig().GetWorldServerID();
...
{
	static int nLastTime = (int)time(NULL);
	...
	m_oServerClose.CloseServer(gpoContext->GetServerConfig().GetWorldServerID());
}
