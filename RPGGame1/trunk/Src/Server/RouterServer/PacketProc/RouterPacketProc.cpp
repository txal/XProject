...
void NSPacketProc::OnPrepCloseServer(int nSrcSessionID, Packet* poPacket, INNER_HEADER& oHeader, int* pSessionArray)
{
	...
	INNER_HEADER oHeaderRet(NSSysCmd::ssImplCloseServer, gpoContext->GetServerConfig().GetWorldServerID(), poRouter->GetServiceID(), oHeader.uSrcServer, oHeader.nSrcService, 0);
	poPacketRet->AppendInnerHeader(oHeaderRet, NULL, 0);
