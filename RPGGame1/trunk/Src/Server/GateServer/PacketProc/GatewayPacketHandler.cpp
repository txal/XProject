...
void GatewayPacketHandler::OnRecvExterPacket(int nSrcSessionID, Packet *poPacket, EXTER_HEADER& oHeader)
{
	...
	else
	{
		...
		oNavi.uTarServer = gpoContext->GetServerConfig().GetServerID();
		...
		if (oNavi.nTarService == 0)
		{
			...
			{
				oNavi.nTarService = gpoContext->GetServerConfig().RandomLogic(gpoContext->GetServerConfig().GetServerID());
