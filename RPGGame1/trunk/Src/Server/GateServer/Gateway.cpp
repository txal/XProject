...
void Gateway::OnExterNetClose(int nSessionID)
{
	...
	oNavi.uSrcServer = oSrvConf.GetServerID();
	oNavi.nSrcService = gpoContext->GetService()->GetServiceID();
	oNavi.uTarServer = oSrvConf.GetServerID();

	GlobalVector& oGlobalList = oSrvConf.GetGlobalList(oSrvConf.GetServerID(), 0);
	oNavi.nTarService = (int8_t)oGlobalList[0].uID;
	...
	{
		XLog(LEVEL_ERROR, "%s: Send packet fail\n", GetServiceName());
	}
}
...
void Gateway::OnExterNetMsg(int nSessionID, Packet* poPacket)
{
	...
	{
		poClient->m_nLastPacketTime = (int)time(0);
	}
